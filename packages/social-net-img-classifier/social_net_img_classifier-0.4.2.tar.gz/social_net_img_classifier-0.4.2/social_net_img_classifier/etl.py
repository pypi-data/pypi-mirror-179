...
class MainETL:
    ...
    def execute_tw(self, terms=TW_TERMS, max_tweets=MAX_TWEETS_PER_TERM, batch=240):
        ...
        while len(tweets_data) > 0:
            ...
            responses = Responding().twitter(contacts, response=False)
    ...
    def process_batch_tw(self, tweets_data):
        ...
        if lng:
            ...
            if data.shape[0]:
                ...
                contacts = true_cases[["screen_name", "twitter_id", "user_name", "tweet_id"]].to_dict("records")
...
class Responding:
    ...
    def twitter(self, tweets_info, dm=True, response=True):
            ...
            for row in tweets_info:
                if row["screen_name"] not in BRANDS["twitter"]:
                    ...
                    if response and dm_sent == 0:
                        text = f"¡Oye pana! ¿Mucha sed? Refréscate con una Águila 0.0 :)"
