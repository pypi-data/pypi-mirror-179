...
ds_name = 'Automation_assembly_DATABRICKS'
# ds_name = 'sf_ds'
...
class TestDS:
    ...
    def test_get_datasource(self):
        ds_name = self.torch_client.get_datasource(self.ds_name, True)
        assert ds_name is not None

    def test_get_datasource_id(self):
        ds_id = self.torch_client.get_datasource(self.ds_id, False)
    ...
    def test_get_ds_crawler(self):
        ds_id = self.torch_client.get_datasource(self.ds_id, True)
        status = ds_id.get_crawler_status()
        assert status is not None

    def test_start_crawler(self):
        ds_name = self.torch_client.get_datasource(self.ds_name, False)
...
class TestAsset:
    torch_client = TorchClient(**test_const.torch_credentials)
    asset_name = 'AWS-S3.jsonArray'
    # asset_name = 'sf_ds.FINANCE.FINANCE.ACCELDATA_CUSTOMERS_IND'
    # asset_id = 4
    asset_id = 279890
    def test_get_asset(self):
        asset = self.torch_client.get_asset(self.asset_name)
        assert asset is not None

    def test_get_asset_id(self):
        asset = self.torch_client.get_asset(self.asset_id)
        assert asset is not None
