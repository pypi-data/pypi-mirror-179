...
def download_inputs(year, days, input_dir, session_id, session_file, sleep_time):
    for day in days:
        fn = os.path.join(input_dir, f"day{day:02}.txt")
        if os.path.exists(fn):
            continue
        response = requests.get(f"https://adventofcode.com/{year}/day/{day}/input",
                                headers={'User-Agent': 'https://github.com/j0057/pytest-aoc by jjm@j0057.nl'},
...
def pytest_sessionstart(session):
    ...
    session_file = session.config.getoption('aoc_session_file') \
            ...
            or '.cookie'
    sleep_time = session.config.getoption('aoc_sleep_time') \
            or session.config.get_ini('aoc_sleep_time') \
            or '2.5'
    if year:
        create_input_dir(input_dir)
        days = get_available_days(int(year), datetime.datetime.utcnow())
        download_inputs(int(year), days, input_dir, session_id, session_file, float(sleep_time))
