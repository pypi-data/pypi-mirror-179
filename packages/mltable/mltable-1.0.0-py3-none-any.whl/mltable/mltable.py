    ...
    def _format_str_list(values, var_name):
        if values:
            ...
            if not all(isinstance(x, str) for x in values):
                raise ValueError(
                    '`{var_name}` must be a non-empty list of strings or None'.format(var_name))
            if len(values) == 0:
                values = None
        return values
...
def load(uri, storage_options: dict = None):
    ...
    """
    custom_dimensions = {'app_name': 'MLTable'}
...
def from_json_lines_files(paths, invalid_lines="error", encoding="utf8", include_path_column=False):
    ...
    if invalid_lines not in ['error', 'drop']:
        raise ValueError(
            f"Invalid value for invalid_lines, the supported values are ['error', 'drop']")
...
class MLTable:
    ...
    def _mount(self, stream_column="Path", mount_point=None, **kwargs):
        ...
        try:
            ...
            message = str(e)
            if "InvalidUriScheme" in message or \
                    "StreamError(NotFound)" in message or \
                    "DataAccessError(NotFound)" in message or \
                    "DataAccessError(PermissionDenied)" in message:
