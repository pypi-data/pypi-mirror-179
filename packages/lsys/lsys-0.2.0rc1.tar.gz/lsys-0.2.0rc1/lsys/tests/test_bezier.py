...
def test_lsys_bezier_poly(n, k, expected):
    t = numpy.array([0, 0.25, 0.5, 0.75, 1])
    expected = numpy.array(expected, dtype=float)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            (
                numpy.array(
                    [0.0, 0.5, 0.68347576, 0.89173788, 1.10826212, 1.31652424, 1.5, 2.0]
                ),
                numpy.array(
                    [0.0, 0.5, 0.63245961, 0.69868941, 0.69868941, 0.63245961, 0.5, 0.0]
                ),
            ),
        ),
        (
            ...
            None,
            (
                numpy.array([0.5, 0.68347576, 0.89173788, 1.10826212, 1.31652424, 1.5]),
                numpy.array([0.5, 0.63245961, 0.69868941, 0.69868941, 0.63245961, 0.5]),
            ),
        ),
        (
            True,
            0.5,
            (
                numpy.array([0.0, 0.5, 0.676, 0.888, 1.112, 1.324, 1.5, 2.0]),
                numpy.array([0.0, 0.5, 0.62, 0.68, 0.68, 0.62, 0.5, 0.0]),
            ),
        ),
        (
            False,
...
def test_bezier_xy(keep, weight, expected):
    ...
    res = bezier.bezier_xy(pts[:, 0], pts[:, 1], weight=weight, segs=5, keep_ends=keep)
    numpy.testing.assert_allclose(res, expected)
...
@pytest.mark.parametrize(
    ("angle", "guess", "max_iters"), [(90, 0.5, 1), (90, 0.99, 1), (45, 0.1, 1)]
)
def test_find_circular_weight_one_iter(angle, guess, max_iters):
    res = bezier.find_circular_weight(angle, guess=guess, max_iters=max_iters)[0]
    assert res == guess
