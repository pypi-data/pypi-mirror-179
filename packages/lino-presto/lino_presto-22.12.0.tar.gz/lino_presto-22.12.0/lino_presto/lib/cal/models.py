...
class Event(Event, InvoiceGenerator):
    ...
    def get_generators_for_plan(cls, plan, partner=None):
        ...
        if partner is None:
            ...
            if issubclass(pm, rt.models.contacts.Partner):
                ...
                qs = cls.filter_by_invoice_recipient(qs, partner, 'project__'+fldname)
                # qs = pm.add_partner_filter(qs, partner, 'project__')
            else:
                raise Exception("Cannot filter {} using {}".format(partner, settings.SITE.project_model))
    ...
    def get_invoice_items(self, info, invoice, ar):
        ...
        for i in super(Event, self).get_invoice_items(info, invoice, ar):
            # print(i.qty)
            yield i
            for oi in self.owner.get_invoice_items(info, invoice, ar):
                yield oi
