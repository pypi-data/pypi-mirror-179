...
class Menu():
    ...
    def show(self):
        print(f"{Colour.bracketsymbol}[{Colour.plussymbol}+{Colour.bracketsymbol}]{Colour.Yellow} {self.title}{' '*7}",end="")
        if type(self.options) == type([]):
            for index, option in enumerate(self.options, 1):
                print(f"{Colour.Yellow}[{index}] {option}{' '*7}",end="")
...
a = Menu(options=["11","2gsg2", "33dsfs", "4$sfd"])
a.show()
