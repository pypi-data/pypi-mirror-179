...
async def generationTwo(characters,assets,img,adapt,signatureRes,lvl, splash):
    ...
    try:
        if splash:
            await create_picture(frame,person,img,adapt,characters.image.banner.url)
        else:
            await create_picture(frame,person,img,adapt)
        await weaponAdd(weaponRes,characters.equipments[-1],lvl)
        await nameBanner(nameRes,characters,lvl)
        await stats(statRes,characters,assets)
        starsRes = starsAdd(person)
        elementRes = elementIconPanel(person.element.value)
        await constant(constantRes,characters,person)
        await talants(talatsRes,characters)
        await artifacAdd(artifacRes,artifactSet,characters)
