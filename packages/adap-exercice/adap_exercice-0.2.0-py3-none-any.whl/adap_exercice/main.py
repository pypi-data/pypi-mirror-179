...
def original():
    ...
    document()
    typer.secho(
        f"\n\nThe .tex and .pdf files are in the folder C:/Users/{os.environ.get('USERNAME')}",
...
def modify():
    ...
    ghn = typer.prompt(
        "What's the value of high quality numerator of gamma in second period ?",
        default=1,
    )
    ghd = typer.prompt(
        "What's the value of high quality denominator of gamma in second period ?",
        default=4,
    )
    gln = typer.prompt(
        "What's the value of low quality numerator of gamma in second period ?",
        default=1,
    )
    gld = typer.prompt(
        "What's the value of low quality denominator of gamma in second period ?",
        default=2,
    )
    if l > h:
        typer.secho(
            "The value of high quality must be superior or equal to the low quality one",
            fg=typer.colors.RED,
        )
        raise typer.Exit()
    gh = Rational(ghn, ghd)
