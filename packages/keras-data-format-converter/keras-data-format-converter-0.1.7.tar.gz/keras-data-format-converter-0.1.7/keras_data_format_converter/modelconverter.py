...
class ModelConverter:
    ...
    def _fix_args(self, call_args: Tuple, node_input_tensors: List[KerasTensor]) -> Tuple[List, List[KerasTensor]]:
        ...
        for i, arg in enumerate(call_args):
            if isinstance(arg, dict):
                new_arg_dict, node_input_tensors = self._fix_kwargs(arg, node_input_tensors)
                new_tfoplambda_args[i] = new_arg_dict
            if isinstance(arg, list):
                new_arg, node_input_tensors = self._fix_args(arg, node_input_tensors)
                new_tfoplambda_args[i] = new_arg
    ...
    def _fix_kwargs(self, call_kwargs, node_input_tensors):
        new_tfoplambda_kwargs = call_kwargs.copy()
        for key, arg in call_kwargs.items():
            if isinstance(arg, dict):
                new_arg_dict, node_input_tensors = self._fix_kwargs(arg, node_input_tensors)
                new_tfoplambda_kwargs[key] = new_arg_dict
            elif isinstance(arg, list):
                new_arg, node_input_tensors = self._fix_args(arg, node_input_tensors)
                new_tfoplambda_kwargs[key] = new_arg
            elif isinstance(arg, KerasTensor):
                try:
                    tensor_index = self._find_tensor_index_by_name(node_input_tensors, arg.name)
                    ...
                    pass
        return new_tfoplambda_kwargs, node_input_tensors
    ...
    def _default_call_layer_by_args(self,
        ...
        new_tfoplambda_kwargs, input_tensors = self._fix_kwargs(call_kwargs, node_input_tensors)
