...
def mirror():
    ...
    slack_mirror = read_mirrors(read_file(f'{etc_slackpkg}mirrors'))
    ...
    if slack_mirror:
        ...
        print('You do not have any http/s mirror selected in /etc/slackpkg/mir'
...
def config():
    ...
    config_file = f'{conf_path}sun.toml'
...
def os_info():
    ...
    info = (
        ...
        f'Uptime: {uptime}\n'
        '[Memory]\n'
        f'Free: {mem[9]}, Used: {mem[8]}, Total: {mem[9]}\n'
        '[Disk]\n'
        f'Free: {disk[2] // (2**30)}Gi, Used: {disk[1] // (2**30)}Gi, '
        f'Total: {disk[0] // (2**30)}Gi\n'
        f'[Processor]\n'
        f'CPU: {cpu}'
        )
    return info
