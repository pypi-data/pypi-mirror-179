
 

// This file is autogenerated. DO NOT EDIT

#pragma once
#include <robotpy_build.h>


#include <AHRS.h>

#include <wpi/sendable/SendableHelper.h>


#define RPYGEN_DISABLE_InitSendable_RTSendableBuilder

#include <rpygen/frc__Gyro.hpp>
#include <rpygen/wpi__Sendable.hpp>

namespace rpygen {



template <typename CfgBase>
using PyTrampolineCfgBase___AHRS =
    PyTrampolineCfg_frc__Gyro<
    PyTrampolineCfg_wpi__Sendable<
CfgBase
>>;

template <typename CfgBase = EmptyTrampolineCfg>
struct PyTrampolineCfg___AHRS :
    PyTrampolineCfgBase___AHRS< CfgBase>
{
    using Base = ::AHRS;

    using override_base_KGetAngle_v = ::AHRS;
    using override_base_KGetRate_v = ::AHRS;
    using override_base_Reset_v = ::AHRS;
    using override_base_Calibrate_v = ::AHRS;
};


template <typename PyTrampolineBase, typename PyTrampolineCfg>
using PyTrampolineBase___AHRS =
    PyTrampoline_frc__Gyro<
    PyTrampoline_wpi__Sendable<
        PyTrampolineBase
        
        , PyTrampolineCfg
    >
        
        , PyTrampolineCfg
    >
;

template <typename PyTrampolineBase, typename PyTrampolineCfg>
struct PyTrampoline___AHRS : PyTrampolineBase___AHRS<PyTrampolineBase, PyTrampolineCfg> {
    using PyTrampolineBase___AHRS<PyTrampolineBase, PyTrampolineCfg>::PyTrampolineBase___AHRS;



#ifndef RPYGEN_DISABLE_KGetAngle_v
    double GetAngle() const override {
        using LookupBase = typename PyTrampolineCfg::Base;
        using CxxCallBase = typename PyTrampolineCfg::override_base_KGetAngle_v;
        PYBIND11_OVERRIDE_IMPL(PYBIND11_TYPE(double), LookupBase,
            "getAngle", );
        return CxxCallBase::GetAngle();
    }
#endif

#ifndef RPYGEN_DISABLE_KGetRate_v
    double GetRate() const override {
        using LookupBase = typename PyTrampolineCfg::Base;
        using CxxCallBase = typename PyTrampolineCfg::override_base_KGetRate_v;
        PYBIND11_OVERRIDE_IMPL(PYBIND11_TYPE(double), LookupBase,
            "getRate", );
        return CxxCallBase::GetRate();
    }
#endif

#ifndef RPYGEN_DISABLE_Reset_v
    void Reset() override {
        using LookupBase = typename PyTrampolineCfg::Base;
        using CxxCallBase = typename PyTrampolineCfg::override_base_Reset_v;
        PYBIND11_OVERRIDE_IMPL(PYBIND11_TYPE(void), LookupBase,
            "reset", );
        return CxxCallBase::Reset();
    }
#endif

#ifndef RPYGEN_DISABLE_Calibrate_v
    void Calibrate() override {
        using LookupBase = typename PyTrampolineCfg::Base;
        using CxxCallBase = typename PyTrampolineCfg::override_base_Calibrate_v;
        PYBIND11_OVERRIDE_IMPL(PYBIND11_TYPE(void), LookupBase,
            "calibrate", );
        return CxxCallBase::Calibrate();
    }
#endif




};

}; // namespace rpygen
