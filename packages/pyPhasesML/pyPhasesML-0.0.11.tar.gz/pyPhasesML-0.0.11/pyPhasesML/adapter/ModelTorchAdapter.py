...
class ModelTorchAdapter(Model):
    ...
    def evalValidation(self, validationData):
        ...
        validationData.transformToCategorical = False
        validationData.continous = False
        processList = tqdm(validationData, disable=(not self.showProgress))
        processList.set_description("Validation")

        for record in processList:
            x, y = self.prepareDataAdapter(record, validation=True)
    ...
    def train(self, dataset: TrainingSet):
        ...
        self.oneHotDecoded = dataset.trainingData.transformToCategorical
        ...
        while self.maxEpochs is None or i_epoch < self.maxEpochs:
            ...
            processList = tqdm(dataset.trainingData, disable=(not self.showProgress))
            processList.set_description("EPOCH {}".format(i_epoch))

            for trainBatch in processList:
