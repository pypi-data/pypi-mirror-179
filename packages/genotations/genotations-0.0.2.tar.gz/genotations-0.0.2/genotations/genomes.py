...
class Annotations:
    ...
    def exon_features_by_gene_name(self, gene_name):
        ...
        anno = self.by_gene_name(gene_name).protein_coding().exons().annotations_df.select(selection).unique()
    ...
    def transcript_features_by_gene_name(self, gene_name, rc):
        return seq(self.by_gene_name(gene_name).transcripts().annotations_df.select(
    ...
    def with_sequences(self, genome: Genome, rc: bool = False) -> 'Annotations':
        if "sequence" in self.annotations_df.columns:
            ...
            return Annotations(self.annotations_df.with_column(self.coordinate_column.apply(lambda r: genome.get_seq(str(r[0]), r[1], r[2], rc))))
