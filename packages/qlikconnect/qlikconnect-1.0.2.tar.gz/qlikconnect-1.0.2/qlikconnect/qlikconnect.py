...
class SenseConnect:
    ...
    def __init__(self, domain='localhost', port='4848', userdirectory='', userid='', certPath='.', conntype='cert'):
        self.ws = self.create_connection(domain, port, userdirectory, userid, certPath, conntype)
        self.qlikcall = SenseCalls()

    def create_connection(self, domain, port, userdirectory, userid, certPath, conntype):
        ...
        if domain=='localhost':
            ...
            certs = ({"ca_certs"   : r''+certPath+'/root.pem',
                      "certfile"   : r''+certPath+'/client.pem',
                      "keyfile"    : r''+certPath+'/client_key.pem',
    ...
    def app_overview(self, appID=''):
        ...
        if appID!='':
            for doclist in result['result']['qDocList']:
                if doclist['qDocId']==appID:
                    return doclist
            return 'App ID not found.'           
        return result['result']['qDocList']
