...
class Bank(Resource):
    ...
    def resource(self, resource_name):
        resource = self._resources[resource_name].set_bank_id(
            super().get_bank_id()).set_headers(self._nash.get_headers()).set_ipn_object(ipn_object=super().get_ipn_object).set_identity_token(identity_token=super().get_identity_token)
    ...
    def ipn(self, operation=None, payload=None):
        ...
        # If the operation performed is to update an IPN
            ...
            if 'client_id' in super().response().keys() and 'account_number' in super().response().keys() :
                ...
                data = super().get_ipn_object.filter(client_id=super().response().get('client_id', ''),account_number=super().response().get('account_number', ''))
