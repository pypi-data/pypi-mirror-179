...
from crongit.dotfile import Dotfile, save_dotfile
...
class Commands:
    ...
    ALLOWED_METHODS = ('init', 'add', 'remove',)
    ...
    def init(dotfile: Dotfile, args: Dict[str, Any]) -> CommandResult:
        ...
        file: Path =  Path(os.path.join(path, '.crongit'))
        if file.exists():
            return (False, f".crongit file already exists in {path}")
        ...
        if not Path(os.path.join(path, '.git')).is_dir():
            return (True, f"Default .crongit created in {path}")
        else:
            return (True, f"Default .crongit created in {path} but it doesn't look like a git repo")
    ...
    def add(dotfile: Dotfile, args: Dict[str, Any]) -> CommandResult:
        ...
        if dotfile.get_repo(name) is not None:
            return (False, f'Repository {name} already exists in {dotfile.path}.')
    ...
    def remove(dotfile: Dotfile, args: Dict[str, Any]) -> CommandResult:
        ...
        if not dotfile.remove_repo(name):
