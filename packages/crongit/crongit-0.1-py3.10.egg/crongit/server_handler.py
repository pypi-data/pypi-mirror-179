...
from crongit.commands import CallableCommand, Commands
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send (self, data: str, response_code: int = 200):
    ...
    def get_command(self) -> Optional[CallableCommand]:
        ...
        if self.client_address[0] != '127.0.0.1':
            self.reject_request(f'Invalid request (address = {self.client_address})')
            return None

        auth = next((k for k,v in self.headers.items() if k == "Authorization" and v == f"Bearer {secret}"), None)
        if not auth:
            self.reject_request(f'Invalid request (authentication failed)')
            return None

        m = re.match(r'^\/crongit\/([^\/]+)/?', self.path)
        if not m:
            self.reject_request(f'Invalid request')
            return None
    ...
    def do_POST(self) -> None:
        fn = self.get_command()
        if not fn:
            self.reject_request(f"Unrecognized command")
            return
        ...
        try:
            ...
            messages.append({ 'severity': 'success' if success else 'error', 'message': msg })
        except HTTPError as ex:
            logging.exception(ex)
            messages.append({ 'severity': 'error', 'message': str(ex) })
            code = 500
        finally:
            self.send(json.dumps({ 'messages': messages }), code)
