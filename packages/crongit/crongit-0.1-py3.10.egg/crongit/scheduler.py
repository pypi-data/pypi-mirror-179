...
from crongit.dotfile import Dotfile, load_dotfile
...
class SchedulerThread(Thread):
    def __init__(self, dotfile_path: str, reload_delay_seconds: int) -> None:
        super().__init__()
        self.dotfile_path = dotfile_path
        self.reload_delay = reload_delay_seconds
        self._running = False

    def _schedule_next(self, dotfile: Dotfile, since: datetime) -> List[RunInfo]:
        ...
        for repo in dotfile.get_repos().values():
            ...
            for key in ('commit-on', 'pull-on', 'push-on'):
                cron_expr: Optional[str] = repo.get(key, None)
                if not cron_expr:
                    continue

                pending_seconds = CronTab(cron_expr).next(since, default_utc=False)
                next_run = since + timedelta(seconds=pending_seconds)

                info = RunInfo(next_run, key.replace('-on', ''), repo)
                result.append(info)
                logging.debug(f'  - Scheduled {info.action} on {info.datetime}')
    ...
    def _perform_run(self, rinfo: RunInfo) -> bool:
        ...
        if not path:
            logging.warning(f"  - Missing 'path' key!")
            return False

        path = os.path.abspath(os.path.expandvars(path))

        command: Optional[str] = None
        if rinfo.action == 'push':
            remote = repo.get('remote', '')
            command = f'git push {remote}'
        elif rinfo.action == 'pull':
            remote = repo.get('remote', '')
            command = f'git pull {remote}'
        elif rinfo.action == 'commit':
            message = datetime.now().strftime(repo.get('commit-message', ''))
    ...
    def run(self) -> None:
        ...
        while self._running:
            ...
            #
            # We choose to reload all files every `reload_delay` seconds because:
            # - It's a lightweight operation.
            # - It's simpler thant maintaining an up-to-date memory cache of
            #   all files and watch for changes during runtime.

            # Reload metadata if needed
            if next_reload < datetime.now():
                logging.debug('Reloading metadata...')
                dotfile = load_dotfile(self.dotfile_path)
                next_reload = datetime.now() + timedelta(seconds=self.reload_delay)
                next_runs = None
            ...
            if pending_count:
                ...
                if successful_runs != pending_count:
                    logging.warning(f'  - {successful_runs} successful runs.')

                # Schedule next runs.
                #
                # Use schedule since the cached `now` instead of `datetime.now()`
