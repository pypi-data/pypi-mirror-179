...
async def _(matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if "本周" in msg:
        ...
        await lol_today.finish(await LoLMatch.cancel_tournament_group(int(sub_id), event.group_id))
    elif "订阅" in msg:
...
async def match_checker():
    ...
    for (group_id, msg) in msg_container.items():
        ...
        try:
            ...
            logger.warning(f"{__plugin_meta__.name} 向群 {group_id} 发送 {match_result} 失败")
        # 停1秒
