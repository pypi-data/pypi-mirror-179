...
async def get(
    ...
    Examples:
        .. code-block:: sls
            ...
            get-location:
                exec.run:
                    - path: gcp.cloudkms.locations.get
                    - kwargs:
    ...
    if not result["ret"]:
        result["comment"] += (
            hub.tool.gcp.comment_utils.get_empty_comment(
                "gcp.cloudkms.locations", resource_id
            ),
        )
...
async def list_(hub, ctx, project: str = None, filter: str = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        filter(str, Optional):
            ...
            "displayName=tokyo", and is documented in more detail in `AIP-160 https://google.aip.dev/160`__.
