...
async def get(
    ...
    Examples:
        .. code-block:: sls
            ...
            get-key-ring:
                exec.run:
                    - path: gcp.cloudkms.crypto_keys.get
                    - kwargs:
    ...
    if not result["ret"]:
        result["comment"] += (
            hub.tool.gcp.comment_utils.get_empty_comment(
                "gcp.cloudkms.crypto_keys", resource_id
            ),
        )
...
async def list_(
    ...
    Examples:
        .. code-block:: sls
            ...
            list-crypto-keys:
                exec.run:
                   - path: gcp.cloudkms.crypto_keys.list
                   - kwargs:
            ...
            list-crypto-keys-filtered:
                exec.run:
                   - path: gcp.cloudkms.crypto_keys.list
                   - kwargs:
                         key_ring: projects/tango-gcp/locations/global/keyRings/kr-global-test
    ...
    if not result["ret"]:
        result["comment"] += (
            hub.tool.gcp.comment_utils.list_empty_comment(
                "gcp.cloudkms.crypto_keys", key_ring
            ),
        )
