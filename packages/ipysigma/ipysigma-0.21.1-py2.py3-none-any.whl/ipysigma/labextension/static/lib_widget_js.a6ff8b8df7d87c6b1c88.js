...
function createD3Scale(definiton) {
    if (!definiton)
    ...
    if (type === 'pow') {
        const scale = (0, d3_scale_1.scalePow)();
        if (param)
            scale.exponent(param);
        else
            scale.exponent(2);
        return scale;
    }
    if (type === 'sqrt') {
        const scale = (0, d3_scale_1.scaleSqrt)();
        if (param)
...
class CategorySummary {
    ...
    static fromTopValues(name, kind, frequencies, defaultValue, scheme, maxCount = CATEGORY_MAX_COUNT) {
...
class VisualVariableScalesBuilder {
    constructor(visualVariables, maxCategories = CATEGORY_MAX_COUNT) {
    ...
    build() {
        ...
        for (const variableName in this.variables) {
            ...
            // Continuous variables
                ...
                else if (variable.range[0] === variable.range[1] ||
                    extent.isConstant()) {
                    scale = () => isValidNumber(variable.default)
                        ? variable.default
                        : variable.range[0];
                }
                else {
