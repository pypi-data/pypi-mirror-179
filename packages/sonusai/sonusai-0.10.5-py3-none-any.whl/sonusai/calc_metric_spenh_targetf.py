...
class MPGlobal:
    ...
    plot_enable: bool = None
    truth_est_mode: bool = None
    disable_wav: bool = None
...
def _process_mixture(mixid: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    plot_enable = MP_GLOBAL.plot_enable
    truth_est_mode = MP_GLOBAL.truth_est_mode
    disable_wav = MP_GLOBAL.disable_wav
    ...
    metr1 = [mixdb.mixtures[mixid].snr, pesq_mixture, pesq_speech, pesq_impr, pesq_impr_pc, cmape_tg, lerr_tg,
    ...
    if truth_est_mode is True:
        ...
        metr3 = [mixdb.mixtures[mixid].snr, pesq_mixture, pesq_speechtr, pesq_impr_sptr, pesq_impr_pctr, lerr_tgtr,
                 lerr_ntr]
        ...
        mtab4flat_col = ['MXSNR', 'SPLERR Max', 'SPLERR Min', 'SPLERR Avg', 'SPLERR Median',
                         'NLERR Max', 'NLERR Min', 'NLERR Avg', 'NLERR Median']
        tmp = np.insert(np.array(metr4), 0, mixdb.mixtures[mixid].snr).reshape(1, 9)  # Insert MXSNR
...
def main():
    ...
    MP_GLOBAL.plot_enable = plot_enable
    MP_GLOBAL.truth_est_mode = truth_est_mode
    MP_GLOBAL.disable_wav = disable_wav
    ...
    num_mix = len(mixids)
    if num_mix > 1:
        metric_summary_fname = join(predict_location, 'spenh-metrics-summary.txt')
        with open(metric_summary_fname, 'w') as f:
            ...
            csv_name = join(predict_location, 'spenh-metrics-summary.csv')
            header_args = {
                'mode': 'a',
                'encoding': 'utf-8',
                'index': False,
            ...
            csv_name = join(predict_location, 'spenh-metrics-list.csv')
            ...
            csv_name = join(predict_location, 'spenh-metrics-estats-list.csv')
            ...
            if truth_est_mode is True:
                csv_name = join(predict_location, 'spenh-metrics-tru-list.csv')
                pd.DataFrame(['Speech enhancement metrics list:']).to_csv(csv_name, **header_args)
                all_metrics_table_3.round(2).to_csv(csv_name, **table_args)

                csv_name = join(predict_location, 'spenh-metrics-estats-tru-list.csv')
                pd.DataFrame(['Extraction statistics list:']).to_csv(csv_name, **header_args)
