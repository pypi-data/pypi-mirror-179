...
def _genmix_kernel(mixid: int) -> GenMixData:
    targets = MP_GLOBAL.mixdb.mixture_targets(mixid=mixid, force=True)
    noise = MP_GLOBAL.mixdb.mixture_noise(mixid=mixid, force=True)
    write_data = [('targets', targets), ('noise', noise)]

    if MP_GLOBAL.compute_truth:
        truth_t = MP_GLOBAL.mixdb.mixture_truth_t(mixid=mixid, targets=targets, noise=noise, force=True)
