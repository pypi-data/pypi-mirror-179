...
def genmixdb(location: Location,
    ...
    mixdb = MixtureDatabase(MixtureDatabaseConfig(
    ...
    mixdb.load_raw_target_audios(show_progress=show_progress)
    ...
    mixdb.load_augmented_noise_audios(show_progress=show_progress)
...
def _process_mixture(mixid: int) -> MRecord:
    ...
    raw_target_audios = [mp_genmixdb.mixdb.raw_target_audios[mrecord.target_file_index[ti]] for ti in
                         range(len(mrecord.target_file_index))]
    noise_audio = mp_genmixdb.mixdb.augmented_noise_audios[mrecord.noise_file_index][mrecord.noise_augmentation_index]

    mrecord, genmix_data = initialize_mixture(mixdb=mp_genmixdb.mixdb,
                                              mrecord=mrecord,
    ...
    if mp_genmixdb.save_mix or mp_genmixdb.save_ft:
        ...
        if mp_genmixdb.save_ft:
            truth_t = mp_genmixdb.mixdb.mixture_truth_t(mixid=mixid,
                                                        targets=genmix_data.targets,
                                                        noise=genmix_data.noise)
            feature, truth_f = mp_genmixdb.mixdb.mixture_ft(mixid=mixid,
                                                            mixture=genmix_data.mixture,
                                                            truth_t=truth_t)
            write_data.append(('feature', feature))
            write_data.append(('truth_f', truth_f))

            if mp_genmixdb.save_segsnr:
                segsnr = mp_genmixdb.mixdb.mixture_segsnr(mixid=mixid,
                                                          targets=genmix_data.targets,
                                                          noise=genmix_data.noise)
                write_data.append(('segsnr', segsnr))

        mp_genmixdb.mixdb.write_mixture_data(mixid, write_data)
        mp_genmixdb.mixdb.write_mixture_metadata(mixid)
...
def main():
    ...
    logger.info(f'\nCreating mixture database for {location}')
