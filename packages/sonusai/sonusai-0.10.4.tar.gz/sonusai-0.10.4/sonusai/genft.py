...
def genft(mixdb: MixtureDatabase,
          ...
          write: bool = False,
          show_progress: bool = False) -> List[GenFTData]:
    ...
    if mp.current_process().daemon:
        ...
        progress = tqdm(total=len(mixids), desc='genft', disable=not show_progress)
...
def _genft_kernel(mixid: int) -> GenFTData:
    feature, truth_f = mp_genft.mixdb.mixture_ft(mixid=mixid, force=True)
    write_data = [('feature', feature)]

    if mp_genft.compute_truth:
        write_data.append(('truth_f', truth_f))
    else:
        truth_f = None

    if mp_genft.compute_segsnr:
        segsnr = mp_genft.mixdb.mixture_segsnr(mixid=mixid, force=True)
        write_data.append(('segsnr', segsnr))
    else:
        segsnr = None

    if mp_genft.write:
        mp_genft.mixdb.write_mixture_data(mixid, write_data)
        mp_genft.mixdb.write_mixture_metadata(mixid)
