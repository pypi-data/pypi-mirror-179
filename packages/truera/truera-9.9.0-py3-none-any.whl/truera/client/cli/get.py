...
@click.option("--log_level", default=None, help=cli_logger.log_level_message)
def druid_tables(project_name, connection_string, log_level):
    UncaughtExceptionHandler.wrap(
        _get_druid_tables, project_name, connection_string, log_level
    )


def _get_druid_tables(project_name, connection_string, log_level):
    logger = cli_logger.create_cli_logger(log_level)
