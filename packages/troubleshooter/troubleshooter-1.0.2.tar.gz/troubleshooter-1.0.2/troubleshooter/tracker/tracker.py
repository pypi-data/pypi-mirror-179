...
def get_local_reprs(frame):
    ...
    for key, value in frame.f_locals.items():
        ...
        if r.find('nan') != _NO_RESULT and r.find('value=') != _NO_RESULT:
            logger.user_warning("'NAN' is detected, Key is {}, Value is {}".format(key, r))
            return True
    return False
...
class Tracker(Tracer):
    ...
    def __init__(self, level=1, **kwargs):
        ...
        self.check_nan = kwargs.get('check_nan', None)
        ...
        self.pop_key_list = ['event_list', 'func_wl', 'path_wl', 'depth', 'max_variable_length', 'check_nan',
                             'path_bl']
    ...
    def _frame_filter_ms(self, frame, event):
        ...
        if self.level == 2 and (func_name not in ["construct", "wrapper", "_convert_python_data"]):
            return False
        ...
        if self.check_nan == 1:
            check_result = get_local_reprs(frame)
            if check_result:
                exit(0)
        elif self.check_nan == 2:
