...
class EbsVolume(Base):
    ...
    def instance_name_filter(cls, query: Query, value: str):
        instance_name_clause = "regexp_replace(cast(array((select instances_tags.instance_name from instances_tags " \
                               ...
                               "and instances_tags.account_id = disks.account_id)) as varchar), '[\\{{\\}}\"]', '', 'g')=:instance_name_x"
        query = query.filter(text(instance_name_clause)).params(instance_name_x=value)
        return query
            ...
            if key is not None and val is not None:
                if not isinstance(val, str):
                    ...
                    query = query.filter(cast(func.jsonb(subq.scalar_subquery().where(
                        (func.jsonb(cls.attached_to).op('->>')(0) == InstancesTags.instance_id) & (cls.account_id == InstancesTags.account_id)).op('->')(key)), String) == val)
