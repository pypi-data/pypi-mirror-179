...
def _log(tensors, sparse_tensors, num_skipped):
  r'''Log values of gradients.
  Args:
    tensors: list of dense gradients.
    sparse_tensors: list of sparse gradients.
    num_skipped: Number of skipped gradients in aggregation.
  '''
...
def _aggregate(replica_vars, replica_grads, num_skipped, num_buckets):
  ...
  for idx, g in enumerate(replica_grads):
    ...
    if (isinstance(g, ops.IndexedSlices)
        and replica_vars[idx] not in small_weights):
  ...
  _log(dense_grads, sparse_grads, num_skipped)
...
def _aggregate_dense(grads, num_buckets):
  ...
  if not grads:
    return []
  if len(grads) == 1:
    aggregated = Collective.get().allreduce(grads[0])
    return [aggregated]
  # Bucketize gradients.
  ...
  for bucket_id, flattened in enumerate(bucket_tensors):
    aggregated = Collective.get().allreduce(flattened)
...
def aggregate_gradients(grads_and_vars, num_buckets=1):
  ...
  replica_grads = _aggregate(
    replica_vars_to_optimize, replica_grads, len(shard_grads), num_buckets)
