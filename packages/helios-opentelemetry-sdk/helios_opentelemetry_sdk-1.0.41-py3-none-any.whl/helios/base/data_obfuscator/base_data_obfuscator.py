...
TRACE_HEADERS = ['baggage', 'traceparent', 'traceresponse', 'tracestate']
...
class BaseDataObfuscator:
    ...
    def _obfuscate_object(self, object_to_obfuscate: Any) -> Any:
        if isinstance(object_to_obfuscate, dict):
            return {key: value if key in TRACE_HEADERS else self._obfuscate_object(value)
                    for key, value in object_to_obfuscate.items()}
