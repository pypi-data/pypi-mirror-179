...
class UserDefinedFeatureExtractor(object):
    ...
    def __pose_probability_filters(self):
        ...
        results = pd.DataFrame(self.__count_values_in_range(data=self.out_data.filter(self.pcols).values, ranges=np.array([[0.0, 0.1], [0.000000000, 0.5], [0.000000000, 0.75]])), columns=['Low_prob_detections_0.1', 'Low_prob_detections_0.5','Low_prob_detections_0.75'])
