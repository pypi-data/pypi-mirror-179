...
class Company:
    ...
    def get_config(self):
        # get config values
        get_config_query = f"SELECT * FROM {self.tab_name} WHERE {self.field} LIKE '%{self.value}%' AND is_active = 1;"
...
class CompanyManager:
    ...
    def get_companies(self):
        get_companies_query = f"SELECT company FROM {self.tab_name} WHERE is_active = 1;"
    ...
    def disable_company(self):
        logger.info(f'>> disabling {self.company} company configuration')
    ...
    def activate_company(self):
        logger.info(f'>> activate {self.company} company configuration')
