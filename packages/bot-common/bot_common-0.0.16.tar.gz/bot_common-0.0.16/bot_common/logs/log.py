...
class Log(LogObject):
    ...
    @catch_exception
    def logs_writer(self, tab_name):
        logs_db, logs_cursor = db_connect(configuration.logs)
    ...
    @catch_exception
    def write(self):
        tab_name = configuration.logs.table_prefix + self.company
        logger.info('-- WRITING LOGS --')
        try:
            self.logs_writer(tab_name)
...
@catch_exception
def create_logs_table_if_not_exists(company):
    conf = configuration.logs
    db, cursor = db_connect(conf)
    tab_name = conf.table_prefix + company
