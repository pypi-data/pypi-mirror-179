...
+ ### `filter` and `update`
Each row is simply a dictionary with column names as keys.
...
Empty columns are represented as an empty string.
...
- `windowed` overlaps rows. Those overlapped rows are copied before you apply functions in `fold` or `map`. So don't worry about the row modifications. 
