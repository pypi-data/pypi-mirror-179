...
class CheckLSConnectHandler(APIHandler):
    ...
    def get(self):
        ...
        try:
            ...
            print("{}connection url {} with api key {}...".format(PREFIX,LABEL_STUDIO_URL,API_KEY))
            ls = Client(url=LABEL_STUDIO_URL, api_key=API_KEY)
            response=ls.check_connection()
            print("{}check connection response content {}".format(PREFIX,response))            
            if(response["status"]=="UP"):
                print("{}connection clear".format(PREFIX))
                self.finish({"status":"UP"})
            else:
                print("{}connection failed".format(PREFIX))
                raise ValueError
        except:
            print("{}connection failed".format(PREFIX))
            raise ValueError
