...
class Unet3dMediaPipe(MediaPipe):
    ...
    def __init__(self, a_device=None, a_batch_size=1, a_prefetch_count=1, a_num_instances=1, a_instance_id=0, a_pipeline=None, **kwargs):
        """
        Constructor method.
    ...
    def definegraph(self):
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            if self.augment == True:
                ...
                img = self.pad1(img)
                gaussian_k1 = self.gaussian_kernel1()
                ...
                img = self.pad2(img)
                gaussian_k2 = self.gaussian_kernel2()
                ...
                img = self.pad3(img)
                gaussian_k3 = self.gaussian_kernel3()
...
class gaussian_kernel_func(media_function):
    ...
    def create_gaussian_kernel(self):
        ...
        # sigmas= [1.5]*self.batch_size #[0.5, 0.8, 1.2, 1.5]
        gaussianWeights = self.create_oneD_gaussian_kernel(sigmas)
        use_for_channels = []
        for c in range(0, self.batch_size):
            use_for_channels.append((c+1)*self.channels)  # 3,6,8,12
        D1, D2, D3, D4 = 1, self.kSize, 1, self.channels * \
            self.batch_size * self.input_depth
        weightData = np.zeros([D1, D2, D3, D4], dtype=np.float32)
        for outC in range(0, D4):
            for w in range(0, D2):
                groupIdx = outC % self.batch_size
                #print("w, outC,groupIdx:",w, outC,groupIdx)
                sizeOneD = len(gaussianWeights[groupIdx])
                if(w < sizeOneD):
                    weightData[0][w][0][outC] = gaussianWeights[groupIdx][w]
            # end
        # end
        return weightData
