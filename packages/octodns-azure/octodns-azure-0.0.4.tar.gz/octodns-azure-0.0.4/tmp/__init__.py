...
class AzureProvider(BaseProvider):
    ...
    def __init__(self, id, client_id, key, directory_id, sub_id,
        ...
        self._required_zone_ns_records = {}
    ...
    def _check_zone(self, name, create=False):
        ...
        if create:
            ...
            ret = create_zone(self._resource_group, name, Zone(location='global'))
            self._azure_zones.add(name)
            from pprint import pprint
            pprint(ret)

            # We have to fetch the zone after create so that we can record its
            # azure-dns.* root NS records
            ...
            for azrecord in records(self._resource_group, name):
                ...
                if typ != 'NS' or azrecord.name != '@':
                    continue
                data = self._data_for_NS(azrecord)
                pprint(data)
                self._required_zone_ns_records[name] = set()
        ...
        if self._check_zone(zone_name):
            ...
            for azrecord in records(self._resource_group, zone_name):
                ...
                if record._type == 'NS' and record.name == '':
                    # Fine the required azure-dns values
                    required = set([v for v in record.values
                                    if 'azure-dns' in v])
                    self._required_zone_ns_records[zone_name] = required
                    self.log.debug('populate: %s required root NS records=%s',
                                   zone_name, record.values)
    ...
    def _process_desired_zone(self, desired):
        ...
        for record in desired.records:
            if record._type == 'NS' and record.name == '':
                ...
                all_values = desired_values + \
                    self._required_zone_ns_records[zone_name]
