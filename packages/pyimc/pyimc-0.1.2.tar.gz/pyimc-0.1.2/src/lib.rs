...
#[pymethods]
impl Mcd {
    /// Parse an .mcd file, returning an object providing access to IMC data and accompanying metadata. If with_dcm is set to true, then
    /// a temporary file for fast channel image access if one is not present, and
    /// returning an object providing access to IMC data and accompanying metadata
    #[staticmethod]
    pub fn parse(filename: &str, with_dcm: Option<bool>) -> PyResult<Self> {
        let with_dcm = with_dcm.unwrap_or(true);
