...
class ForecastingPipelineWrapperBase(ABC):
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray) -> 'ForecastModelWrapperBase':
        """
    ...
    def _pipeline_rolling_forecast_internal(
        ...
        if self._y_transformer is not None:
            X_fcst[self.forecast_column_name] = \
                self._y_transformer.inverse_transform(X_fcst[self.forecast_column_name].to_numpy())
    ...
    def _fit_naive(self) -> 'ForecastModelWrapperBase':
