...
from pledger.account.money import Money, Currency
...
class Transfer:
    ...
    @abstractmethod
    def deposit(amount: int, date: str = None) -> Transfer:
        assert isinstance(amount, int), "Amount must be an integer"
        return Transfer(amount, Currency.GBP, TransferType.DEPOSIT, date=date)

    @abstractmethod
    def interest(amount: int, date: str = None) -> Transfer:
        assert isinstance(amount, int), "Amount must be an integer"
