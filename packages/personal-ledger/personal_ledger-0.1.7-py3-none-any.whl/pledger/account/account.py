...
class Account:
    ...
    def get_interest_balance(self) -> int:
        """Get total account balance. Initial balance plus the balance of all transfers."""
        return self.transfers.get_interest().amount
    ...
    def payout(self) -> None:
        ...
        # create a new interest transfer with the current date of the payout
        payout_date = self.get_current_day().strftime("%d/%m/%Y")

        self.transfers.append(Transfer.interest(interest.amount, payout_date))
        self.accrued_interest = Transfers()
