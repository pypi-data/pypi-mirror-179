try:
    from pynut_3db.pyNutDB import nutDb as db
except:
    try:
...
# python -m pytest test/test_nutDb.py
...
def fDf_lite_Launch():
    str_pathDb = r'C:\Users\laurent.tupin\OneDrive - IHS Markit\Documents\Github\sola-pcf\db_param.db'
    if not fBl_liteAccessible(str_pathDb):
        print(f' FOLDER is not accessible: {str_pathDb}')
        return None
