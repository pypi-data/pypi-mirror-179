...
def get_minimum_spanning_tree(all_pairs_of_residues_data: Dict[Tuple[str,str], Residue_pair_data]) -> nx.Graph:
    """
    Returns the Minimum Spanning Tree (MST) of a motif by creating a dense graph of all the pairs of residues in the motif and then applying Kruskal's 
...
def get_nodes_position_specific_exchanges(nodes: List[str], residue_type_policy: Union[str, Dict[str,List[str]]], motif_residues_data: Dict[str, Residue]) -> Dict[str,List[str]]:
    ...
    elif residue_type_policy == 'all_residues_at_all_positions':
        for node in nodes:
            node_ID = node[:-1]
...
def get_all_pairs_of_residues_to_check(all_motifs_MST: List[nx.Graph]) -> Dict[Tuple[str,str], Residue_pair_data]:
    ...
    for MST in all_motifs_MST:
        ...
        for pair_of_full_residue_IDs in MST_edges:
            pair_of_full_residue_IDs = get_sorted_2_tuple(pair_of_full_residue_IDs) # Ordering is needed to make sure we only calculate each pair once and take full advantage of memoization, ie avoid calculating the results of (A,B) and (B,A), as they are identical.
...
def update_map_of_PDBs_with_all_residue_pairs(
        map_of_PDBs_with_all_residue_pairs: Dict[str, Set[Tuple[str,str]]], new_residue_pair_data: Dict[str, Set[Tuple[str,str]]], map_of_PDBs_with_all_residue_pairs_n_updates: int
    ) -> Dict[str, Set[Tuple[str,str]]]:
    """
    ...
    """
    if map_of_PDBs_with_all_residue_pairs_n_updates == 0:
        return new_residue_pair_data
    ...
    for PDB_ID in list(map_of_PDBs_with_all_residue_pairs.keys()):
        if PDB_ID not in updated_set_of_PDBs_with_all_residue_pairs:
            del map_of_PDBs_with_all_residue_pairs[PDB_ID] # We remove PDBs that no longer have all the pairs of residues
        else:
            map_of_PDBs_with_all_residue_pairs[PDB_ID].update(new_residue_pair_data[PDB_ID]) # We update the set of pairs of residues of the PDB
...
def get_PDBs_that_contain_the_residue_pair(
    ...
    pair_of_residues = sort_and_join_2_strings(res1_resname, res2_resname) # The index only contains lexycographically sorted pairs of residues (ex: GA doesn't exist, only AG)
...
def filter_out_PDBs_without_a_similar_motif(
        map_of_PDBs_with_all_residue_pairs: Dict[str, Set[Tuple[str,str]]], motif_MST: nx.Graph
    ) -> Dict[str, List[nx.Graph]]:
    """
    """
    # To know if a PDB contains a similar motif, we use subgraph monomorphism to perform a matching between the reference motif's graph
    # and the graph generated from all the selected pairs of residues of a PDB structure. The monomorphism is done based on the residue names, which have to be 
    # identical between the two graphs. Note that we use subgraph monomorphism instead of simple graph monomorphism because PDBs might have more than 1 similar motif, 
    # for example a motif might be present twice in a homodimer, so we need to check all possible subgraphs in the graph generated from the selected pairs of residues.
    ...
    # PDB AF-G5EB01-F1-model_v4 when using a distance threshold > 1.5 and an angle threshold > 10° as it leads to the occurence of an additional AE edge between residue A32A and A35E.
    add_resname_as_node_attribute(motif_MST) # Needed for subgraph monomorphism, see below.

    PDBs_with_similar_motif: Dict[str, List[nx.Graph]] = defaultdict(list)
    for PDB_ID, pairs_of_residues in map_of_PDBs_with_all_residue_pairs.items():
        ...
        for residue_mapping_dict in graph_matcher.subgraph_monomorphisms_iter():
            ...
            PDBs_with_similar_motif[PDB_ID].append(similar_motif_graph)
    
    return PDBs_with_similar_motif

memoization_dict_type_alias: TypeAlias = Dict[Tuple[str,str], Dict[str,Set[Tuple[str,str]]]] # For mypy annotation
def find_PDBs_with_all_pairs_of_residues_in_motif_MST(motif_MST: nx.Graph, memoization_dict: memoization_dict_type_alias,) -> Dict[str, List[nx.Graph]]:
    """
    """
    ...
    map_of_PDBs_with_all_residue_pairs: Dict[str, Set[Tuple[str,str]]] = defaultdict(set)
    map_of_PDBs_with_all_residue_pairs_n_updates: int = 0
    ...
    for pair_of_full_residue_IDs in pairs_of_residues_to_check:
        map_of_PDBs_with_all_residue_pairs = update_map_of_PDBs_with_all_residue_pairs(
            ...
            memoization_dict[pair_of_full_residue_IDs], 
            map_of_PDBs_with_all_residue_pairs_n_updates
        )
        map_of_PDBs_with_all_residue_pairs_n_updates += 1
    
    # After finding PDBs that contain all the residue pairs in their correct geometric arragement, we need to make sure 
    # these pairs are connected and not just scattered.
    # Connected pairs example: [('A1P', 'A4C'), ('A4C', 'A9L'), ('A9L', 'A10K')]
    # Unconnected pairs example: [('A1P', 'A4C'), ('A54C', 'A60L'), ('A19L', 'A22K')]
    PDBs_with_similar_motif = filter_out_PDBs_without_a_similar_motif(map_of_PDBs_with_all_residue_pairs, motif_MST)
...
def get_PDBs_with_similar_motifs(
    ...
    ...
    """
    memoization_dict: memoization_dict_type_alias = {}
    ...
    # The most computationally intensive part of the code is the actual loading of the data for all the pairs of residues and to 
    ...
    # Although we could in theory parallelize each motif MST instead, that would require sharing the memoization dict between the processes,
    ...
    with ProcessPoolExecutor(n_cores) as concurrent_executor:
        ...
        for future in as_completed(submitted_futures):
            if issubclass(type(future.exception()), BaseException):
                raise future.exception() # type: ignore

            pair_of_full_residue_IDs = submitted_futures[future]
            PDBs_that_contain_the_residue_pair = future.result()
            
            memoization_dict[pair_of_full_residue_IDs] = PDBs_that_contain_the_residue_pair

    # Its not worth parallelizing the next bit of code because it is actually quite fast, and the added overhead of
    # parallelizing + having to transfer the results back to the main thread isn't worth it.
    PDBs_with_similar_motifs: Dict[nx.Graph, Dict[str, List[nx.Graph]]] = defaultdict(dict)
    for motif_MST in all_motifs_MST: 
        PDBs_with_all_pairs_of_residues_in_motif_MST = find_PDBs_with_all_pairs_of_residues_in_motif_MST(motif_MST, memoization_dict)
        if PDBs_with_all_pairs_of_residues_in_motif_MST:
            PDBs_with_similar_motifs[motif_MST] = PDBs_with_all_pairs_of_residues_in_motif_MST
    
    return PDBs_with_similar_motifs
