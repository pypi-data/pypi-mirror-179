...
def run_ml_cli_pipeline(args, config_path, ml_cli_template_path):
    ...
    mlcli_template_json = args.datasets_configuration_json
    ...
    if datasets_configuration_json != "" and datasets_configuration_json is not None:
        config["datasets"] = json.loads(datasets_configuration_json)

    if mlcli_template_json is None or mlcli_template_json == "":
        with open(ml_cli_template_path) as mlcli_file:
            mlcli_template = mlcli_file.read()
    else:
        mlcli_template = json.loads(mlcli_template_json)
