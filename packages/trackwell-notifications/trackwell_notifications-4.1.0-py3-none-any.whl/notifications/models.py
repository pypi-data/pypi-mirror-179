...
class Topic(models.Model):
    ...
    def send_notification(
        ...
        Args:
            ...
            extra_filter (Optional[Q], optional): Extra filtering for user devices to send to. Defaults to None.
        """
        default_language = get_default_language()
        devices = Device.objects.filter(user__in=self.subscribers.all())
        if extra_filter is not None:
            devices = devices.filter(extra_filter)
        devices = devices.annotate(
            language=Coalesce(
                Cast('user__profile__prefs__language', output_field=CharField()),
