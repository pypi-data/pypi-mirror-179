...
class FlowAndPortStatsMonitor(app_manager.RyuApp):
    ...
    def _port_stats_reply_handler(self, ev) -> None:
        ...
        for port in body:
            ...
            if self.producer_running:
                self.logger.info(f"PRODUCING TO topic: {constants.TOPIC_NAMES.OPENFLOW_PORT_STATS_TOPIC_NAME}, record: "
                                 f"{port_statistics_dto.to_kafka_record()}")
        ...
        if self.producer_running and len(port_stats) > 0:
            ...
            self.logger.info(f"PRODUCING TO topic: "
                             f"{constants.TOPIC_NAMES.AVERAGE_OPENFLOW_PORT_STATS_PER_SWITCH_TOPIC_NAME}, record: "
                             f"{avg_flow_stats.to_kafka_record()}")
