...
class Raster:
    ...
    def set(self, i, j, val):
        ...
        if not(val is None) and not(math.isnan(val)) and str(val) != str(self._nodata):
            self._checkRasterBounds(i, j)
            self._mesh[i][j] = val
    ...
    def save(self, outputFile):
        ...
        # If there are None values NoData must be set
        if (any(None in row for row in self._mesh)) and (self._nodata is None):
            if(self.min > 0):
                self._nodata = 0
            else:
                self._nodata = self.min - (self.max - self.min)
