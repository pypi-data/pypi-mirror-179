...
class HASC (Raster):
    ...
    @staticmethod
    def cellSideFromArea(area):

        return math.sqrt((2 * area) / (3 * math.sqrt(3)))

    @staticmethod
    def cellSideFromSquaredResolution(square_side):

            return HASC.cellSideFromArea(math.pow(square_side, 2) * 1.134)
