...
def read_channel(file, head=None):
    if head is not None:
        ngx, ngy, ngz = map(int, head.split())
    else:
        ngx, ngy, ngz = map(int, next(file).split())
    ...
    while after is not None and after != head:
        after = next(file, None)
