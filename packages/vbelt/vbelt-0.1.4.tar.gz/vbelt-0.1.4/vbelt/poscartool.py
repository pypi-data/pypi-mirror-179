...
def nudge(opts):
    ...
    with error_catch():
        nudge(opts.poscar, dest, forces, A=opts.amplitude, rattle=opts.rattle)
...
def coord(opts):
    ...
    from .poscar import Poscar, calc_econ, calc_coco
    ...
    if opts.method == "econ":
        fn = calc_econ
    elif opts.method == "coco":
        fn = calc_coco

    with error_catch():
        coord = fn(poscar, at, opts.species)
