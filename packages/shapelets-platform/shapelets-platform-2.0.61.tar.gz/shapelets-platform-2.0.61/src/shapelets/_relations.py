...
class SandBox:
    ...
    def __rewrite_cols(self, ds: DataSet) -> DataSet:
        r"""
        It rewrites column names when necessary. Special characters (i.e. . , : ( ) ? ¿ etc) will be replaced by char '_'
        ...
        It returns a Shapelets DataSet with the column names modified

        """
        col_dict = [ds.rewrite_col(idx,c) for idx, c in enumerate(ds.columns)]
        col_dict = dict(filter(lambda item: item is not None, col_dict))
        
        return (ds.rename_columns(col_dict) if col_dict else ds)
