...
def get_new_trajactory_len(buffers, env_id, ep_len, relabelling_strategy, q_net):
    ...
    if relabelling_strategy == 'cher':
        len_hind = len(achieved_goal_id)
        elem1 = desired_goal_id[0]
        new_achieved_goal_id = []
        for elem0 in achieved_goal_id:
            if ((elem0, elem1) in q_net.adjacent_goal_id_pairs) or ((elem1, elem0) in q_net.adjacent_goal_id_pairs):
                new_achieved_goal_id.append(elem0)
        achieved_goal_id = copy.deepcopy(new_achieved_goal_id)

        if len(achieved_goal_id) == 0:
            return None, None

        # print(len(achieved_goal_id), len_hind)

        # with torch.no_grad():
        #     z_hg, *_ = q_net.encode_goal_w_id(achieved_goal_id)
        #     z_g, *_ = q_net.encode_goal_w_id(desired_goal_id)
        #     d = torch.cdist(z_g, z_hg)
        #     indices = torch.topk(d, min(len(achieved_goal_id), 3)).indices
        #     indices = indices.cpu().numpy()
        #     if len(indices)<len(achieved_goal_id):
        #         print(len(indices), len(achieved_goal_id))
        #     achieved_goal_id = achieved_goal_id[indices].reshape(-1,1)
