...
class TransferL1Data(L1OutputDataBase, GlobusMixin, ObjectStoreMixin):
    ...
    def transfer_output_data(self):
        """Create a Globus transfer for all output data."""
        science_frame_paths: list[Path] = list(self.read(tags=[Tag.output(), Tag.frame()]))
        movie_path: list[Path] = list(self.read(tags=[Tag.output(), Tag.movie()]))
        object_paths: list[Path] = science_frame_paths + movie_path
        transfer_items = []
    ...
    def run(self) -> None:
        """Transfer the data."""
        with self.apm_task_step("Upload Science Frames and Movie"):
            self.transfer_output_data()
