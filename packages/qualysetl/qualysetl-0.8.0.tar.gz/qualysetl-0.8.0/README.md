...
IMPORTANT NOTE: The 0.8.x series is out. Please test before replacing your 0.7.x series code. The key changes are noted in the changelog.

0.8.x is the next BETA Release of Example code from the [Qualys API Best Practices Series](https://blog.qualys.com/tag/api-best-practices-series) 
...
```text
    ...
    qetl_manage_user -u [path] -e etl_host_list_detection -d 1970-01-01T00:00:00Z
     ...
     - qetl_manage_user will download all knowledgebase and vulnerability data. 
     - Knowledgeabase, Host List and Host List Detection will be transformed/prepared
     - JSON, SQLITE and Optionally files prepared for remote database insertion.
     - Resulting SQLite Databse will contain multiple normalized tables:  
