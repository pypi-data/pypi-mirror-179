...
class Dxlfunc:
    ...
    def write(self, axis, INPUT_Adrress, value):
        if axis == 'ALL':
            for i in self.IDs:
            ...
            if INPUT_Adrress in self.one_byte_values:
                self._packetHandler.write1ByteTxRx(self._portHandler, axis, INPUT_Adrress, value)
            elif INPUT_Adrress in self.two_byte_values:
                self._packetHandler.write2ByteTxRx(self._portHandler, axis, INPUT_Adrress, value)
            elif INPUT_Adrress in self.four_byte_values:
                self._packetHandler.write4ByteTxRx(self._portHandler, axis, INPUT_Adrress, value)

    def read(self, axis, INPUT_Adrress):
        ret = 0
        if INPUT_Adrress in self.one_byte_values:
            ret, _, _ = self._packetHandler.read1ByteTxRx(self._portHandler, axis, INPUT_Adrress)
            if ret >= np.power(2, 7):
                ret = ret - np.power(2, 8)
        elif INPUT_Adrress in self.two_byte_values:
            ret, _, _ = self._packetHandler.read2ByteTxRx(self._portHandler, axis, INPUT_Adrress)
            if ret >= np.power(2, 15):
                ret = ret - np.power(2, 16)
        elif INPUT_Adrress in self.four_byte_values:
            ret, _, _ = self._packetHandler.read4ByteTxRx(self._portHandler, axis, INPUT_Adrress)
            if ret >= 2**31:
                ret = ret - 2**32
        return ret
