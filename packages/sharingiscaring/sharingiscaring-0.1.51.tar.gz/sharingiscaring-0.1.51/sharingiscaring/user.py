...
class UserReminder:
    def __init__ (self, status):
        self.status = status
        self.send_next_reminder_after = None
...
class Subscription:
    def __init__(self):
        ...
        self.reminder               = UserReminder(UserReminderStatus.NO_NEED)
...
class User:
    ...
    def prepare_for_subscription_logic(self,
        node: ConcordiumClient,
        ccdscan: CCDScan, 
        ):
        self.get_explorer_transactions(node, ccdscan)
        self.get_explorer_delegators(ccdscan)
        

    def get_explorer_transactions(self, node: ConcordiumClient, ccdscan: CCDScan):
        try:
            explorer_ccd_transactions, _ = ccdscan.ql_get_all_transactions_for_explorer_ccd(None, SubscriptionDetails.EXPLORER_CCD.value)
            self.explorer_ccd_transactions = [Transaction(node).init_from_graphQL(x['node']['transaction']).find_memo_and_amount() for x in explorer_ccd_transactions]
        except:
            self.explorer_ccd_transactions = []
    ...
    def determine_bot_status(self):
        ...
        if self.subscription.reminder.status == UserReminderStatus.NO_NEED:
            # based on the avg per hour, we estimate that in 48 hours, credits are gone.
            if (self.subscription.messages_per_hour * SubscriptionDetails.SUBSCRIPTION_WARN_HOURS_BEFORE.value) >= \
                self.subscription.count_messages:
                self.subscription.reminder.status = UserReminderStatus.RUNNING_OUT_IN_2_DAYS
                self.subscription.reminder.send_next_reminder_after = dt.datetime.now().astimezone(timezone.utc)

        if self.subscription.reminder.status == UserReminderStatus.RUNNING_OUT_IN_2_DAYS:
            # check if we now out of credits.
            if self.subscription.remaining_message_credits <= 0:
                self.subscription.reminder.status = UserReminderStatus.OUT_OF_CREDITS_1
                self.subscription.reminder.send_next_reminder_after = dt.datetime.now().astimezone(timezone.utc)
