...
# If the host is linux or mac, check if the rust program is accessible
    ...
    if not os.path.isfile(TriePath):
        ...
        print("Trie is not in the .local/bin/trie directory. Downloading it from the repository...")
        mainCmd = str("git clone https://github.com/Redempt/anomaly_analysis && cd anomaly_analysis && cargo build --release && mv target/release/trie " + TriePath)
        print(mainCmd)
        os.system(mainCmd)
        #Check if the rust program is accessible
        if not os.path.isfile(TriePath):
