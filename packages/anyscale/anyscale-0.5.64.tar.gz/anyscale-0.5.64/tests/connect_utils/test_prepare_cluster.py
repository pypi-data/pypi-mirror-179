...
def test_check_if_cluster_needs_start_or_create(
    mock_prepare_cluster_block,
    needs_update: bool,
    cluster_exists: bool,
    ...
    if not cluster_name or cluster_state != "Running" or not cluster_exists:
        assert mock_prepare_cluster_block._check_if_cluster_needs_start_or_create(
            "test_project_id", cluster_name, needs_update
        )
    else:
        assert (
            mock_prepare_cluster_block._check_if_cluster_needs_start_or_create(
                "test_project_id", cluster_name, needs_update
            )
            == needs_update
        )
...
@pytest.mark.parametrize("cluster_needs_start_or_create", [True, False])
@pytest.mark.parametrize("cluster_name", ["test_cluster_name", None])
@pytest.mark.parametrize("cluster_compute_name", ["test_cluster_compute_name", None])
@pytest.mark.parametrize("cluster_env_name", ["test_cluster_env_name", None])
def test_init(
    cluster_needs_start_or_create: bool,
    cluster_name: Optional[str],
    cluster_compute_name: Optional[str],
    cluster_env_name: Optional[str],
):
    if not cluster_needs_start_or_create and not cluster_name:
        return

    mock_check_if_cluster_needs_start_or_create = Mock(
        return_value=cluster_needs_start_or_create
    )
    mock_cluster = Mock(
    ...
    with patch.multiple(
        ...
        _check_if_cluster_needs_start_or_create=mock_check_if_cluster_needs_start_or_create,
    ), patch.multiple(
        "anyscale.connect_utils.prepare_cluster",
    ...
    mock_check_if_cluster_needs_start_or_create.assert_called_once_with(
        "test_project_id", cluster_name, False
    )
    if cluster_needs_start_or_create:
