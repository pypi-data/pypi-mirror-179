...
def combine_commands(
    ...
    commands.extend([f'echo $?>/tmp/{code} || echo $?>/tmp/{code}', 'sleep 60', f'rm /tmp/{code} &'])
    command_str = ' && '.join(commands)
    command = f"echo '{command_str}'>{tmpfile}"
    return [
        CommandSettingDetail(Command(command), timeout=Number(10)),
        CommandSettingDetail(Command(f'nohup /bin/bash {tmpfile} >/dev/null 2>&1'), timeout=Number(10)),
        CommandSettingDetail(Command(f'rm {tmpfile}'), timeout=Number(10))
    ], timeout, code
