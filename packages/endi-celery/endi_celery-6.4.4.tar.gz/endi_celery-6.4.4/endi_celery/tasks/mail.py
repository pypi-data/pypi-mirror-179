...
def async_mail_salarysheets(self, job_id, mails, force):
    ...
    for mail_datas in mails:
        ...
        try:
            ...
            if email is None:
                logger.error(u"no mail found for company {0}".format(company_id))
                continue
            else:
            ...
            transaction.abort()
            error_count += 1
            msg = u"Ce fichier a déjà été envoyé {0}".format(mail_datas["attachment"])
...
(mail : {1})".format(
    ...
    messages.append(u"{0} mails n'ont pas pu être envoyés".format(error_count))
