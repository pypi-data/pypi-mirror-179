...
from endi_celery.tasks.accounting_measure_compute import get_measure_compilers
...
class AccountingDataParser(object):
    ...
    def _load_company_id_cache(self):
        query = (
            DBSESSION()
            .query(Company.id, Company.code_compta)
    ...
    def _collect_main_file_infos(self):
        ...
        :rtype: dict
        """
        basename, extension = os.path.splitext(os.path.basename(self.file_path))
        if extension:
    ...
    def process_file(self):
        ...
        if self.force or not self._already_loaded():
            ...
            logger.info(
                u"Storing {0} new operations in database".format(len(operations))
            )
            logger.info(
...
class GeneralLedgerParser(AccountingDataParser):
    _filename_re = re.compile(
        "general_ledger_" "(?P<year>[0-9]+)" "_(?P<month>[^_]+)" "_(?P<doctype>[^_]+)",
        re.IGNORECASE,
    )
    ...
    def _build_operation(self, line_datas):
        ...
        if len(line_datas) >= 6:
            if line_datas[0].strip() not in (
                ...
                if not date:
                    logger.error(u"This line has incorrect datas : %s" % line_datas)
                    return None
            ...
            if line_datas:
                logger.error(u"This line is missing informations : %s" % line_datas)
        return result
...
def send_error(request, mail_addresses, filename, err):
    ...
    if mail_addresses:
        try:
            message = MAIL_ERROR_BODY.format(error=err.message, filename=filename)
    ...
    if mail_addresses:
        try:
            ...
            message = MAIL_UNKNOWN_ERROR_BODY.format(
                error=err.message, filename=filename
            )
            send_mail(
                request,
                mail_addresses,
...
def handle_pool_task(self, force=False):
    ...
    if already_processing:
        ...
        if mail_addresses:
            ...
            logger.info(u"A success email has been sent to {0}".format(mail_addresses))
