...
def get_measure_compilers(data_type, grid_type=None):
    ...
    if data_type == AccountingOperationUpload.ANALYTICAL_BALANCE:
        ...
        elif grid_type == "balance_sheet":
            return [BalanceSheetMeasureCompiler]
        else:
            return [
                TreasuryMeasureCompiler,
                IncomeStatementMeasureCompiler,
...
def compile_measures_task(self, upload_id, grid_type=None):
    ...
    for compiler_factory in compiler_factories:
        operations = None
        if compiler_factory == TreasuryMeasureCompiler:
            ...
            operations = (
                AccountingOperation.query().filter_by(upload_id=upload_id).all()
            )
...
since yesterday"
        ...
        for upload in uploads:
            ...
            measure_compiler_factories = get_measure_compilers(upload_object.filetype)
