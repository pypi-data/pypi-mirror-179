...
class CinnarollPyTorchModel(CinnarollModel):
    def save(self) -> None:
        try:
            self.framework_package.save(self.model_object, self.model_save_path)
        except Exception as e:
            raise ModelSavingError(f"Could not save model: {repr(e)}")

    def load(self) -> None:
        try:
            self.model_object = self.framework_package.load(self.model_save_path)
        except Exception as e:
            raise ModelLoadingError(f"Could not load model: {repr(e)}")
    ...
    def should_be_loaded(self) -> bool:
        ...
        if self.model_save_path.exists():
            try:
                saved_model = self.framework_package.load(self.model_save_path)
            except Exception as e:
                raise ModelLoadingError(
                    f"Error occurred while trying to load model from {self.model_save_path}: {repr(e)}"
                )
