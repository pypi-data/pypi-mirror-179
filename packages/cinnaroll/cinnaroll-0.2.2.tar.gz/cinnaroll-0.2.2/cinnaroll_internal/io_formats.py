...
def find_infer_func_output_format_mismatch_error(
    ...
    if output_format == JSON:
        if not isinstance(infer_func_output, str):
            return InferFuncOutputFormatMismatchError(
                f"When output_format is set to {output_format} infer function must return a string. Currently "
                f"infer function returns an object of type {type(infer_func_output)}."
            )
        try:
            _ = json.loads(infer_func_output)
        except json.JSONDecodeError as e:
            return InferFuncOutputFormatMismatchError(
                f"When output_format is set to {output_format} infer function must return a valid "
                f"JSON-formatted string that you can pass to json.loads() . Here is the error encountered "
