...
class SecondCacheDict(RWDict):
    def __setitem__(self, key, value):
        if isinstance(value, list):
            kv = RWList(rw_name=f"{self.rw_name.replace('second', 'value')}", write_flag=True)
            for v in value:
                kv.append(v)
        else:
            raise ValueError
...
class TopCacheDict(RWDict):
    ...
    def __setitem__(self, key, value):
        if isinstance(value, dict):
            kv = SecondCacheDict(rw_name=self.rw_name.replace("top", "second"), write_flag=True)
            for k in value:
                kv[k] = value[k]
        else:
            raise ValueError
...
CacheObject: Dict[Union[ObjectType, str], dict] = TopCacheDict(rw_name="CacheObject-top", write_flag=True)
