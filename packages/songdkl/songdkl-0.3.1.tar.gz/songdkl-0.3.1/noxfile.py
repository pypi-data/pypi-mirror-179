...
def download_pcb_dataset(session):
    ...
    """
    session.run('python', './scripts/download_pcb_dataset.py')
...
def prep_pcb_dataset(session):
    ...
    """
    session.install(".")
    # TODO: fix this to be cross platform. Need to write a .bat script?
    session.run('bash', './src/scripts/prep-all.sh', external=True)
...
def clean_pcb_prep(session):
    ...
    session.run("rm", "-rf", "results/pcb_data/song_data/*.zarr/", external=True)
...
def clean_pcb_prep(session):
    ...
    session.run("python", "src/scripts/tar_pcb_prep.py")
