...
class KeyboardManager:
    ...
    def register_hotkey(self, keys, count, on_press, trigger, *args, **kwargs):
        """
        :param keys: the key list or tuple.
        :param count: number of pressed times for hotkey with single key.
        :param on_press: True = trigger the hotkey on press; False = trigger the hotkey on release.
        ...
        if 0 == length:
            ...
            return 0
        on_press = bool(on_press)
        if any([on_press == hotkey.on_press and length == len(hotkey.keys) and
                all([k in hotkey.keys for k in keys]) for hotkey in self.__hotkeys]):
            self.__logger.info('【Register -1】Hotkey: {} has been registered.'.format(keys))
            return -1
        hotkey = HotKey(self.__id, keys, count, trigger, on_press, *args, **kwargs)
        self.__hotkeys.append(hotkey)
        self.__id += 1
        self.__logger.info('【Register 1】{}'.format(hotkey))
        return self.__id - 1
    ...
    def unregister_hotkey_by_keys(self, keys, on_press):
        """
        ...
        on_press = bool(on_press)
        for hotkey in self.__hotkeys:
            if on_press == hotkey.on_press and length == len(hotkey.keys) and \
                    all([k in hotkey.keys for k in keys]):
                self.__hotkeys.remove(hotkey)
                self.__logger.info('【Unregister 1】{}'.format(hotkey))
                return True
