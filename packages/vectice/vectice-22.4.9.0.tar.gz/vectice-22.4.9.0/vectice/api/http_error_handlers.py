...
class ClientErrorHandler:
    ...
    def handle_code(self, e: HttpError, reference_type: str, reference: Union[str, int]):
        if e.code == 404:
            ...
            raise ConnectionRefusedError("Bad or missing credentials")
        elif e.code == 403:
    ...
    def handle_get_gql_error(self, error: TransportQueryError, reference_type: str, reference: Union[str, int]):
        ...
        if status_code == 404:
            ...
            raise RuntimeError(f"Can not access {reference_type}: {message}")

    def handle_post_gql_error(self, error: TransportQueryError, reference_type: str, action: str = "create"):
        status_code = error.errors[0]["extensions"]["exception"]["status"]  # type: ignore
        message = error.errors[0]["extensions"]["exception"]["stacktrace"]  # type: ignore
        if status_code == 401:
            raise ConnectionRefusedError("Bad or missing credentials")
            ...
            raise RuntimeError(f"Can not {action} {reference_type}: {message}")
        else:
            return self._graphql_error_formatter(error)

    def handle_put_gql_error(self, error: TransportQueryError, reference_type: str, reference: Union[str, int]):
        status_code = error.errors[0]["extensions"]["exception"]["status"]  # type: ignore
        if status_code == 404:
