...
from typing import Optional, Tuple, Any, BinaryIO, Union, Sequence, cast, NoReturn, List
...
from .json import PagedResponse, AttachmentOutput, DatasetVersionOutput, ModelVersionOutput, CodeVersionOutput
...
class AttachmentApi(RestApi):
    ...
    def _generate_artifact_url_and_id(
        ...
        try:
            if _type == "datasetversion":
                ...
                version = self._get_version_reference(_type, version, project)
                url, object_name = self.get_url_and_id_for_code_version(_type, version, workspace, project)
            elif _type == PROJECT:
    ...
    def get_url_and_id_for_code_version(
        self,
        _type: str,
        version: Union[str, int],
        workspace: Optional[Union[str, int]] = None,
        ...
        version_object: CodeVersionOutput = CodeVersionApi(self.auth).get_code_version(
            version=version, project=project, workspace=workspace
        )
        url = self._build_url(version_object.code.project.id, _type, version_object.id)
        object_name = "CodeVersion with id: " + str(version_object.id)
        return url, object_name
    ...
    def get_attachment(
        ...
        try:
            ...
            response = self._get_attachment(url + f"/{file_id}")
    ...
    def delete_attachment(
        ...
        try:
            url, object_id = self._generate_artifact_url_and_id(_type, version, artifact, workspace, project)
            response = self._delete_attachment(url + f"/{file_id}")
