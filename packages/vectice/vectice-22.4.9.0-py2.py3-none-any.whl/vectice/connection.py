from __future__ import annotations

import logging
import json
from json import JSONDecodeError

from typing import Optional, Union, List

from vectice.api import Client
from vectice.api.json import ActivityTargetType
from vectice.models.workspace import Workspace
from vectice.models.project import Project


_logger = logging.getLogger(__name__)


def _get_last_asset(target_type: ActivityTargetType, client: Client):
    try:
        last_asset = client.get_last_assets([target_type.value], {"index": 1, "size": 1}).list[0]
        return last_asset
    except IndexError as e:
        _logger.debug(f"There were no assets with activity found. Sanity check {e}")
        return


def _get_last_used_assets_and_logging(client: Client):
    assets_logging = ["\nLast Used Assets", "", "", "", ""]
    last_target_types = [
        ActivityTargetType.Project,
        ActivityTargetType.Phase,
        ActivityTargetType.Iteration,
        ActivityTargetType.IterationStep,
    ]
    for target_type in last_target_types:
        asset = _get_last_asset(target_type, client)
        if asset and target_type == ActivityTargetType.Project:
            assets_logging.insert(1, f"\nProject: {asset.name}")
        elif asset and target_type == ActivityTargetType.Phase:
            assets_logging.insert(2, f"\nPhase: {asset.name}")
        elif asset and target_type == ActivityTargetType.Iteration:
            assets_logging.insert(3, f"\nIteration: {asset.id}")
        elif asset and target_type == ActivityTargetType.IterationStep:
            assets_logging.insert(4, f"\nIterationStep: {asset.name}")
    _logger.info("".join(assets_logging))


class Connection:
    """
    This class allows to connect to the Vectice application.
    Once the connection is established, information are logged automatically.
    """

    def __init__(
        self,
        api_token: Optional[str] = None,
        host: Optional[str] = None,
        workspace: Optional[Union[str, int]] = None,
    ):
        """
        :param api_token: Your private api token.
        :param host: The address of the Vectice application.
        :param workspace: The workspace you want to work in.
        """
        logging.getLogger("Client").propagate = True
        self._client = Client(
            workspace=workspace,
            token=api_token,
            api_endpoint=host,
            auto_connect=True,
            allow_self_certificate=True,
        )
        _logger.info("Vectice successfully connected.")
        self._integration_client = None  # Placeholder, can be removed from workspace, project etc
        compatibility = self._client.check_compatibility()
        if compatibility.status != "OK":
            if compatibility.status == "Error":
                _logger.error(f"compatibility error: {compatibility.message}")
                raise RuntimeError(f"compatibility error: {compatibility.message}")
            else:
                _logger.warning(f"compatibility warning: {compatibility.message}")

    def __repr__(self) -> str:
        return (
            "Connection("
            + f"workspace={self._client.workspace.name if self._client.workspace else 'None'}, "
            + f"host={self._client.auth.api_base_url}, "
        )

    def workspace(self, workspace: Union[str, int]) -> Workspace:
        """
        Return the workspace object that id or name matches the reference specified.

        :param workspace: The id or the name of the desired workspace.
        :return: Workspace
        """
        output = self._client.get_workspace(workspace)
        result = Workspace(output.id, output.name, output.description)
        result.__post_init__(self._client, self._integration_client)
        return result

    @property
    def workspaces(self) -> Optional[List[Workspace]]:
        """
        List workspaces.

        Lists the workspaces the user have access to with it's user connection.

        :return: List of Workspaces
        """
        outputs = self._client.list_workspaces()
        results = [Workspace(id=output.id, name=output.name, description=output.description) for output in outputs.list]
        for workspace in results:
            workspace.__post_init__(self._client, self._integration_client)
        return results

    @staticmethod
    def connect(
        api_token: Optional[str] = None,
        host: Optional[str] = None,
        config: Optional[str] = None,
        workspace: Optional[Union[str, int]] = None,
        project: Optional[str] = None,
    ) -> Optional[Union[Connection, Workspace, Project]]:
        """
        Attempts to create a client that connects to Vectice using the api_token, host, workspace or config provided. If
        a config is used the following keys must be set in the json file;
        1. VECTICE_API_TOKEN
        2. VECTICE_API_ENDPOINT
        3. WORKSPACE - this key is optional

        :param api_token: The api token found in the account/keys.
        :param host: The host which the client will try to connect to.
        :param config: A config file which is json and requires the keys VECTICE_API_TOKEN, VECTICE_API_ENDPOINT and an optional WORKSPACE.
        :param workspace: The workspace name or ID you want to use.
        :param project: The project name you want to use.
        :return: Vectice Object
        """
        if config:
            with open(config, "r") as config_file:
                try:
                    config_object = json.load(config_file)
                except JSONDecodeError:
                    raise SyntaxError("The json config is not parsable.")
                workspace = config_object.get("WORKSPACE")
                try:
                    api_token = config_object["VECTICE_API_TOKEN"]
                    host = config_object["VECTICE_API_ENDPOINT"]
                except KeyError as error:
                    raise KeyError(f"The key {error.args[0]} is required in the json config.")
        connection = Connection(api_token=api_token, host=host)
        _get_last_used_assets_and_logging(connection._client)
        if workspace and not project:
            workspace_output = connection.workspace(workspace)
            _logger.info(f"Your current workspace: {workspace_output.name}")
            return workspace_output
        elif workspace and project:
            logging.getLogger("vectice.models.workspace").propagate = False
            workspace_output = connection.workspace(workspace)
            project_output = workspace_output.project(project)
            _logger.info(f"Your current workspace: {workspace_output.name} and project: {project_output.name}")
            return project_output
        elif project and not workspace:
            raise ValueError("A workspace reference is needed to retrieve a project.")
        else:
            return connection
