...
from .json.phase import PhaseOutput, PhaseInput
...
class Client:
    ...
    def delete_attachment(
        ...
        """
        project, workspace = self.get_project_and_workspace_refs_if_project_ref_is_str(project, workspace)
        return AttachmentApi(self.auth).delete_attachment(_type, version, file_id, artifact, workspace, project)
    ...
    def create_phase(
        ...
        project, workspace = self.get_project_and_workspace_references_or_raise_error(project, workspace)
        project_object = self.get_project(project, workspace)
        return PhaseApi(self._gql_client, self.auth).create_phase(phase, project_object.id)
    ...
    def update_phase(self, phase: PhaseInput, phase_id: int) -> PhaseOutput:
        return PhaseApi(self._gql_client, self.auth).update_phase(phase, phase_id)

    def delete_phase(self, phase_id: int) -> None:
        PhaseApi(self._gql_client, self.auth).delete_phase(phase_id)
    ...
    def list_steps(
        ...
        return StepApi(self._gql_client, self.auth).list_steps(phase_id)
