...
class ValiotWorker():
    ...
    def runJob(self, job):
        ...
        log_from_job = self.getProcessLoggerFunction(queue=job['queue'], job=job)
    ...
    def finish_job_gracefully(self, job, job_kwargs, output, errors = []):
        if len(errors) > 0:
            ...
            self.log(LogLevel.INFO, f"finished job with ID {job['id']} with implicit exit")
        elif type(output) == str:
            self.log(LogLevel.INFO, f"finished job with ID {job['id']} with message: \n{output}")
        ...
        if update_job_fn.has_been_called:
            maybe_status = update_job_fn.call_args[0].get("status") if len(update_job_fn.call_args) > 0 else None
            if maybe_status is not None:
                ...
                if (last_status in [JobStatus.FINISHED, JobStatus.ERROR, JobStatus.ABORTED, JobStatus.PAUSED]):
                    ...
                    self.log(LogLevel.INFO, f"job {job['id']} terminated itself correctly")
                    return
    ...
    def actuallyRunEvent(self, queue_name):
        ...
        def safe_fn(**kwargs):
            ...
            try:
                ...
                self.finish_job_gracefully(job=job, job_kwargs=kwargs, output=output, errors=errors)
        p = self.context.Process(target=safe_fn, kwargs=(kwargs))
        p.start()
        self.runningJobs[job['id']] = {
            'process': p,
