...
from psu_base.services import email_service, message_service, downtime_service
...
def get_next_fye_downtime():
    return downtime_service.get_next_fye_downtime()


def cashnet_disabled():
    dt = get_next_fye_downtime()
    if dt and dt.is_fye and dt.is_active():
        return dt
    return False
...
def cashnet_redirect(tx):
    ...
    cn_downtime = cashnet_disabled()
    if cn_downtime:
        message_service.post_error(f"""
            Online payments have been disabled for fiscal year end processing.<br />
            <em>Service is expected to be restored {cn_downtime.end_cd.humanized()}</em>        
        """)
        # Update the status and initiation date
        tx.status_code = "E"
        tx.initiation_date = datetime.now()
        tx.cashnet_error_message = "CC processing disabled for fiscal year-end closing"
        tx.internal_error_message = "CC processing disabled for fiscal year-end closing"
        tx.save()
