...
class NeuralDensePassageRetriever(BaseRetriever):
    """
    ...
    def __init__(
        ...
        **Example:**
                ...
                ```python
                |    # from local path
                |    NeuralDensePassageRetriever(document_store=your_doc_store,
                |                          embedding_model="model_directory/question-encoder",
                |                          params_path="model_directory/context-encoder")
                ```
        ...
        :param params_path: Local path or remote name of passage encoder checkpoint. The format equals the
                                        one used by paddlenlp transformers' models
        ...
        self.set_config(
            ...
            passage_embedding_model=embedding_model,
        ...
        # Init & Load Encoders
        pretrained_model = AutoModel.from_pretrained(embedding_model)
        self.ernie_encoder = SemanticIndexBatchNeg(pretrained_model,output_emb_size=output_emb_size)
        # Load Custom models
        print("Loading Parameters from:{}".format(params_path))
        state_dict = paddle.load(params_path)
        self.ernie_encoder.set_dict(state_dict)
    ...
    def _get_predictions(self, dicts):
        ...
        with tqdm(
            ...
            for batch in data_loader:
                ...
                with paddle.no_grad():
                    cls_embeddings = self.ernie_encoder.get_pooled_embedding(
