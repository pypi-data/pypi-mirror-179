...
class CenterIdentity:
    ...
    def verify_client_challenge(self, challenge_context):
        return self.challenge_request(challenge_context)
    ...
    def get_recovery(self, body):
        ...
        if challenge_result['status'] is True:
            ...
            return self.get_recovery_request(challenge_context)
        return {'status': False}
    ...
    def set_recovery(self, client_username, body):
        ...
        if client_username != body['identity']['username']:
            return {'status': False}
        challenge_context = self.do_challenge(body)
        return self.set_recovery_request(challenge_context)

    def challenge_request(self, data):
        res = requests.post(f'{self.url_prefix}/challenge', json=data)
        return res.json()

    def get_recovery_request(self, data):
        res = requests.post(f'{self.url_prefix}/get-recovery', json=data)
        return res.json()

    def set_recovery_request(self, data):
        res = requests.post(f'{self.url_prefix}/set-recovery', json=data)
        return res.json()
