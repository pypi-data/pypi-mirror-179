...
class MiddleDatasetMgmt(dsmgmt.DatasetMgmt):
    ...
    def _parse_image_count(self, example_proto): # read
        image_feature_description = {
    ...
    def read_images(self, path):# -> images, count:  #TFRecord format
        ...
        parsed_image_count = image_count.map(self._parse_image_count, num_parallel_calls=tf.data.experimental.AUTOTUNE)
        
        count = 0
        for image_count in parsed_image_count:
            count = image_count['count'].numpy()
            break;
    
        return parsed_image_dataset, count
