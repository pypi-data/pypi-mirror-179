...
pub fn chinilla_streamable_macro(input: TokenStream) -> TokenStream {
    ...
    if !fnames.is_empty() {
        let ret = quote! {
            impl Streamable for #ident {
                fn update_digest(&self, digest: &mut clvmr_chinilla::sha2::Sha256) {
                    #(self.#fnames.update_digest(digest);)*
                }
                fn stream(&self, out: &mut Vec<u8>) -> chinilla_error::Result<()> {
                    #(self.#fnames.stream(out)?;)*
                    Ok(())
                }
                fn parse(input: &mut std::io::Cursor<&[u8]>) -> chinilla_error::Result<Self> {
