    ...
    let streamable = quote! {
        ...
        impl #ident {
            ...
            pub fn from_bytes(blob: &[u8]) -> PyResult<Self> {
                ...
                Self::parse(&mut input).map_err(|e| <chinilla_error::Error as Into<PyErr>>::into(e))
            }
            ...
            pub fn parse_rust<'p>(blob: pyo3::buffer::PyBuffer<u8>) -> PyResult<(Self, u32)> {
                ...
                Self::parse(&mut input).map_err(|e| <chinilla_error::Error as Into<PyErr>>::into(e)).map(|v| (v, input.position() as u32))
            }

            pub fn get_hash<'p>(&self, py: Python<'p>) -> PyResult<&'p pyo3::types::PyBytes> {
                let mut ctx = <clvmr_chinilla::sha2::Sha256 as clvmr_chinilla::sha2::Digest>::new();
                Streamable::update_digest(self, &mut ctx);
                Ok(pyo3::types::PyBytes::new(py, clvmr_chinilla::sha2::Digest::finalize(ctx).as_slice()))
            }
            pub fn to_bytes<'p>(&self, py: Python<'p>) -> PyResult<&'p pyo3::types::PyBytes> {
                let mut writer = Vec::<u8>::new();
                self.stream(&mut writer).map_err(|e| <chinilla_error::Error as Into<PyErr>>::into(e))?;
