...
from encord.orm.dataset import CreateDatasetResponse, DataRow, StorageLocation
...
def upload_annotation(
    ...
    data_hash_to_label_hash: Dict[str, str],
    data_unit: DataRow,
    id_to_obj: Dict[int, Object],
):
    data_hash = data_unit["data_hash"]
    if data_hash not in data_hash_to_label_hash.keys():
        lr_dict = project.create_label_row(data_hash)
        label_hash = lr_dict["label_hash"]
    else:
        label_hash = data_hash_to_label_hash[data_hash]
...
def build_local_lr(
    ...
    img_file = Path(images[lr["data_title"]].file_name)
...
class CocoImporter:
    ...
    def create_project(self, dataset: Dataset, ontology: Ontology) -> Project:
        ...
        data_hash_to_label_hash = {lr.data_hash: lr.label_hash for lr in project.list_label_rows()}
        ...
        for data_unit in tqdm(dataset.data_rows, desc="Uploading annotations"):
            data_hash, lr = upload_annotation(project, self.annotations, data_hash_to_label_hash, data_unit, id_to_obj)

            lr_path = self.project_dir / "data" / lr["label_hash"] / "label_row.json"
            build_local_lr(self.images, self.images_dir, data_hash, lr, lr_path)
