...
def create_new_project_on_encord_platform(
    ...
    meta_file_path: Path = st.session_state.project_dir / "project_meta.yaml"
    ...
    #  1. CHECK project_meta.yaml FILE
    ssh_key_path = yaml.safe_load(meta_file_path.read_text())["ssh_key_path"]
    if not Path(ssh_key_path).expanduser().is_file():
    ...
    original_project_hash = yaml.safe_load(meta_file_path.read_text())["project_hash"]
    ...
    try:
        ...
        st.error("Please check your project_hash in project_meta.yaml file")
        return
