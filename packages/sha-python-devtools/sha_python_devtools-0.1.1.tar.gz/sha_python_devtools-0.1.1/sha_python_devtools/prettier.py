...
# 通用生成器类型(这不是详尽的:像chain这样的东西没有包括进来以避免导入)
...
class PrettyFormat:
    ...
    def __call__(self, value: 'Any', *, indent: int = 0, indent_first: bool = False, highlight: bool = False) -> str:
        ...
        if highlight and pygments:
            ...
            s = pygments.highlight(
                s, lexer=pyg_lexer, formatter=pyg_formatter).rstrip('\n')
        return s
    ...
    def _render_pretty(self, gen: 'Iterable[Any]', indent: int) -> None:
        ...
        for v in gen:
            if isinstance(v, int) and v in {-1, 0, 1}:
                ...
                pretty_value = v.get(PRETTY_KEY, MISSING) if (
                    isinstance(v, dict) and len(v) == 1) else MISSING
    ...
    def _format_dataclass(self, value: 'Any', _: str, indent_current: int, indent_new: int) -> None:
        self._format_fields(value, value.__dict__.items(),
                            indent_current, indent_new)
