...
class TimerResult:
    ...
    def str(self, dp: int = 3) -> StrType:
        if self._name:
            return f'{self._name}: {self.elapsed():0.{dp}f}s 消逝'
        else:
            return f'{self.elapsed():0.{dp}f}s 消逝'
...
class Timer:
    ...
    def __call__(self, name: Optional[str] = None, verbose: Optional[bool] = None):
        if name:
            self._name = name
    ...
    def start(self, name: Optional[str] = None, verbose: Optional[bool] = None):
