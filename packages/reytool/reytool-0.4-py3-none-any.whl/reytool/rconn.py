# !/usr/bin/env python
# -*- coding: utf-8 -*-

'''
═════════════════════════════
@Time   : 2022/11/24 14:31:49
@Author : Rey
@Contact: reyxbo@163.com
@Explain: Rey's database connect type
═════════════════════════════
'''


import re
from pandas import DataFrame
from sqlalchemy import create_engine, text, connectors
from sqlalchemy.engine.cursor import LegacyCursorResult
from sqlalchemy.sql.elements import TextClause
from types import MethodType

from . import rbasic


class RConn(object):
    '''
    Rey's database connect type
    '''

    def __init__(
        self,
        user: 'str'=None,
        password: 'str'=None,
        host: 'str'=None,
        port: 'str'=None,
        database: 'str'=None,
        charset: 'str'=None,
        only_once: 'bool'=True
    ) -> 'None':
        '''
        Set database connect parameters
        '''

        rbasic.check_parm(user, str, None)
        rbasic.check_parm(password, str, None)
        rbasic.check_parm(host, str, None)
        rbasic.check_parm(port, str, int, None)
        rbasic.check_parm(database, str, None)
        rbasic.check_parm(charset, str, None)
        rbasic.check_parm(only_once, bool)

        self.user = user
        self.password = password
        self.host = host
        self.port = port
        self.database = database
        self.charset = charset
        self.only_once = only_once
        self.conn = None
    
    def create_conn(
        self,
        user: 'str'=None,
        password: 'str'=None,
        host: 'str'=None,
        port: 'str | int'=None,
        database: 'str'=None,
        charset: 'str'=None,
        only_once: 'bool'=True
    ) -> connectors:
        '''
        Get database connect engine
        '''

        rbasic.check_parm(user, str, None)
        rbasic.check_parm(password, str, None)
        rbasic.check_parm(host, str, None)
        rbasic.check_parm(port, str, int, None)
        rbasic.check_parm(database, str, None)
        rbasic.check_parm(charset, str, None)

        user = rbasic.get_first_notnull(user, self.user)
        password = rbasic.get_first_notnull(password, self.password)
        host = rbasic.get_first_notnull(host, self.host)
        port = rbasic.get_first_notnull(port, self.port)
        database = rbasic.get_first_notnull(database, self.database)
        charset = rbasic.get_first_notnull(charset, self.charset, default='utf8')
        only_once = rbasic.get_first_notnull(only_once, self.only_once, default='utf8')

        if self.conn and [user, password, host, port, database] == [None] * 5:
            return self.conn

        rbasic.check_parm(user, str)
        rbasic.check_parm(password, str)
        rbasic.check_parm(host, str)
        rbasic.check_parm(port, str, int)
        rbasic.check_parm(database, str)
        rbasic.check_parm(only_once, bool)
        
        try:
            url = f'mysql+mysqldb://{user}:{password}@{host}:{port}/{database}?charset={charset}'
            engine = create_engine(url)
        except ModuleNotFoundError:
            url = f'mysql+pymysql://{user}:{password}@{host}:{port}/{database}?charset={charset}'
            engine = create_engine(url)
        conn = engine.connect(close_with_result=only_once)
        if not self.conn and not only_once:
            self.conn = conn
        return conn
    
    def fill_sql_none(
        self,
        sql: 'str | TextClause',
        parms: 'dict | list(dict,)',
        fill_field: 'bool'=True,
        none_values: 'list'=['', b'', [], (), {}, ' ']
    ) -> 'dict/ list(dict,)':
        '''
        Fill missing parameters according to contents of sqlClause object of sqlalchemy module

        Parameters
        ----------
        sql: str or sqlalchemy.sql.elements.TextClause
            SQL in sqlalchemy.text format or return of sqlalchemy.text

        parms: dict or list(dict,)
            parameters set for filling sqlalchemy.text

        fill_field: bool
            whether fill missing fields

        none_values: list(object,)
            values to be converted to None
        
        Returns
        -------
        dict/ list(dict,): filled parameters
            when parameter parms is dict, then return dict
            when parameter parms is list(dict,), then return list(dict,)
        '''

        rbasic.check_parm(sql, str, TextClause)
        rbasic.check_parm(parms, dict, list)
        rbasic.check_parm(fill_field, bool)
        rbasic.check_parm(none_values, list)

        if type(text) == TextClause:
            text = text.text
        pattern = '(?<!\\\):(\w+)'
        sql_keys = re.findall(pattern, sql)
        if type(parms) == dict:
            for key in sql_keys:
                if fill_field:
                    val = parms.get(key)
                else:
                    val = parms[key]
                if val in none_values:
                    val = None
                parms[key] = val
        else:
            for parm in parms:
                for key in sql_keys:
                    if fill_field:
                        val = parm.get(key)
                    else:
                        val = parm[key]
                    if val in none_values:
                        val = None
                    parm[key] = val
        return parms

    def fetch_table(self, data: 'LegacyCursorResult | iter', fields: 'iter'=None) -> 'list[dict,]':
        '''
        Fetch SQL result to list[dict{field: value,},] table format
        '''

        if type(data) == LegacyCursorResult:
            if fields == None:
                fields = data.keys()
            table = [dict(zip(fields, [rbasic.get_datetime_str(val) for val in row])) for row in data]
        else:
            data_fields_len = max([len(row) for row in data])
            if fields == None:
                data = [list(row) + [None] * (data_fields_len - len(row)) for row in data]
                field_range = range(data_fields_len)
                table = [dict(zip(field_range, [rbasic.get_datetime_str(val) for val in row])) for row in data]
            else:
                field_len = len(fields)
                if data_fields_len > field_len:
                    fields += list(range(data_fields_len - field_len))
                data = [list(row) + [None] * (field_len - len(row)) for row in data]
                table = [dict(zip(fields, [rbasic.get_datetime_str(val) for val in row])) for row in data]
        return table

    def fetch_df(self, data: 'LegacyCursorResult | iter', fields: 'iter'=None) -> 'DataFrame':
        '''
        Fetch SQL result to DataFrame object
        '''

        if type(data) == LegacyCursorResult:
            if fields == None:
                fields = data.keys()
            else:
                fields_len = len(data.keys())
                fields = fields[:fields_len]
            df = DataFrame(data, columns=fields)
        else:
            if fields == None:
                df = DataFrame(data)
            else:
                data_fields_len = max([len(row) for row in data])
                field_len = len(fields)
                if data_fields_len > field_len:
                    fields += list(range(data_fields_len - field_len))
                data = [list(row) + [None] * (field_len - len(row)) for row in data]
                df = DataFrame(data, columns=fields)
        return df

    def fetch_sql(self, data: 'LegacyCursorResult | iter', fields: 'iter'=None) -> 'str':
        '''
        Fetch SQL result to SQL sentence
        '''

        if type(data) == LegacyCursorResult:
            if fields == None:
                fields = data.keys()
            sqls = [[repr(rbasic.get_datetime_str(val)) if val else 'NULL' for val in row] for row in data]
        else:
            if fields == None:
                data_fields_len = max([len(row) for row in data])
                fields = ['field_%d' % i for i in range(data_fields_len)]
            sqls = [[repr(rbasic.get_datetime_str(val)) if val else 'NULL' for val in row] + ['NULL'] * (data_fields_len - len(row)) for row in data]
        sqls[0] = 'SELECT ' + ','.join(['%s AS `%s`' % (val, fie) for val, fie in list(zip(sqls[0], fields))])
        sqls[1:] = ['SELECT ' + ','.join(sql) for sql in sqls[1:]]
        sql = ' UNION ALL '.join(sqls)
        return sql

    def execute(
        self,
        sql: 'str',
        parms: 'dict | list(dict,)'=None,
        fill_field: 'bool'=True,
        none_values: 'list'=['', b'', [], (), {}, ' '],
        only_once: 'bool'=True,
        **kwargs: 'object'
    ) -> LegacyCursorResult:
        '''
        Execute SQL and add methods fetch_table and fetch_df

        Parameters
        ----------
        sql: str
            SQL in sqlalchemy.text format

        parms: dict or list(dict,)
            parameters set for filling sqlalchemy.text

        fill_field: bool
            whether fill missing fields

        none_values: list(object,)
            values to be converted to None
        
        only_once: bool
            whether the database connect engine is one-time (i.e. real time creation)

        Returns
        -------
        LegacyCursorResult object of alsqlchemy package
        '''

        rbasic.check_parm(sql, str)
        rbasic.check_parm(parms, dict, list, None)
        rbasic.check_parm(fill_field, bool)
        rbasic.check_parm(none_values, list)
        rbasic.check_parm(only_once, bool)

        only_once = rbasic.get_first_notnull(only_once, self.only_once)

        if self.conn:
            conn = self.conn
        else:
            conn = self.create_conn(only_once=only_once)
        if parms != None or kwargs != {}:
            if parms != None and kwargs != {}:
                if type(parms) == list:
                    for parm in parms:
                        parm.update(kwargs)
                else:
                    parms.update(kwargs)
            elif kwargs != {}:
                parms = kwargs
            parms = self.fill_sql_none(sql, parms, fill_field, none_values)
            sql = text(sql)
            result = conn.execute(sql, parms)
        else:
            result = conn.execute(sql)
        fetch_table = lambda _self, fileds=None: self.fetch_table(_self, fileds)
        fetch_df = lambda _self, fileds=None: self.fetch_df(_self, fileds)
        fetch_sql = lambda _self, fileds=None: self.fetch_sql(_self, fileds)
        result.fetch_table = MethodType(fetch_table, result)
        result.fetch_df = MethodType(fetch_df, result)
        result.fetch_sql = MethodType(fetch_sql, result)
        if self.only_once:
            del self.conn
            self.conn = None
        return result

    def execute_select(
            self,
            table: 'str',
            database: 'str'=None,
            fields: 'str | iter'=None,
            where: 'str'=None,
            order: 'str'=None,
            limit: 'int | str | iter'=None,
            print_sql: 'bool'=False
        ) -> 'LegacyCursorResult':
        '''
        Execute select SQL

        Parameters
        ----------
        table: str
            table name
        
        database: str
            database name
        
        fields: None or str or iterator
            select syntax content
            when is None, then is 'SELECT `*`'
            when is str, then join as 'SELECT str'
            when is iterator(str,) then join as 'SELECT `str`, ...'

        where: str
            where syntax content
            join as 'WHERE str'

        order: str
            order by syntax content
            join as 'ORDER BY str'

        limit: int or str or iterator(str/int, str/int)
            limit syntax content
            when is int or str, then join as 'LIMIT int/str'
            when is iterator(int/str, ...) with length of 1 or 2, then join as 'LIMIT int/str, ...'

        print_sql: bool
            whether print SQL

        Returns
        -------
        LegacyCursorResult object of alsqlchemy package
        '''

        rbasic.check_parm(table, str)
        rbasic.check_parm(database, str, None)
        rbasic.check_parm(where, str, None)
        rbasic.check_parm(order, str, None)

        database = rbasic.get_first_notnull(database, self.database)

        rbasic.check_parm(database, str)

        sqls = []
        if fields == None:
            fields = '*'
        elif rbasic.if_iterable(fields):
                fields = ','.join(['`%s`' % field for field in fields])
        select_sql = (
            f'SELECT {fields}\n'
            f'FROM `{database}`.`{table}`'
        )
        sqls.append(select_sql)
        if where != None:
            where_sql = 'WHERE %s' % where
            sqls.append(where_sql)
        if order != None:
            order_sql = 'ORDER BY %s' % order
            sqls.append(order_sql)
        if limit != None:
            list_type = type(limit)
            if list_type in [str, int]:
                limit_sql = f'LIMIT {limit}'
            else:
                if len(limit) in [1, 2]:
                    limit_content = ','.join([str(val) for val in limit])
                    limit_sql = 'LIMIT %s' % limit_content
                else:
                    error = ValueError('The length of the limit parameter value must be 1 or 2')
                    raise error
            sqls.append(limit_sql)
        sql = '\n'.join(sqls)
        if print_sql:
            rbasic.rprint(sql, title='SQL', format_module=None)
        result = self.execute(sql)
        return result


    def execute_update(
        self,
        data: 'LegacyCursorResult | list(dict,) | dict',
        table: 'str',
        database: 'str'=None,
        primary_key: 'str'=None,
        print_sql: 'bool'=False
    ) -> 'None':
        '''
        Update the data of table in the datebase
    
        Parameters
        ----------
        data: LegacyCursorResult or list(dict,) or dict
            updated data
            
        table: str
            table name
        
        database: str
            database name
        
        primary_key: str or None
            where syntax content
            primary_key is str, then is f'WHERE `{str}` = :{str}'
            primary_key is None, then is 'WHERE `%s` = :%s' % (first field name and value of each item)
            
        print_sql: bool
            whether print SQL

        Returns
        -------
        LegacyCursorResult object of alsqlchemy package
        '''

        rbasic.check_parm(table, str)
        rbasic.check_parm(data, LegacyCursorResult, list, dict)
        rbasic.check_parm(database, str, None)
        rbasic.check_parm(primary_key, str, None)

        database = rbasic.get_first_notnull(database, self.database)

        rbasic.check_parm(database, str)

        data_type = type(data)
        if data_type == LegacyCursorResult:
            data = self.fetch_table(data)
        elif data_type == dict:
            data = [data]
        for row in data:
            if primary_key == None:
                primary_key = list(row.keys())[0]
            set_content = ','.join(['`%s` = :%s' % (key, key)for key in row if key != primary_key])
            sql = (
                f'UPDATE `{database}`.`{table}`\n'
                f'SET {set_content}\n'
                f'WHERE `{primary_key}` = :{primary_key}'
            )
            if print_sql:
                rbasic.rprint(sql, title='SQL', format_module=None)
            self.execute(sql, row, only_once=False)
        if self.only_once:
            self.conn == None

    def execute_insert(
        self,
        data: 'LegacyCursorResult | list(dict,) | dict',
        table: 'str',
        database: 'str'=None,
        duplicate_method: 'str("ignore") | str("update")'=None,
        print_sql: 'bool'=False
    ) -> 'None':
        '''
        Insert the data of table in the datebase

        Parameters
        ----------
        data: LegacyCursorResult or list(dict,) or dict
            updated data
            
        table: str
            table name
        
        database: str
            database name
        
        duplicate_method: 'ignore' or 'update'
            syntax method when constraint error
            duplicate_method is None, then no syntax
            duplicate_method is 'ignore', then is 'UPDATE IGNORE INTO'
            duplicate_method is 'update' then is 'ON DUPLICATE KEY UPDATE'
            
        print_sql: bool
            whether print SQL

        Returns
        -------
        LegacyCursorResult object of alsqlchemy package
        '''
        
        rbasic.check_parm(table, str)
        rbasic.check_parm(data, LegacyCursorResult, list, dict)
        rbasic.check_parm(database, str, None)

        database = rbasic.get_first_notnull(database, self.database)

        rbasic.check_parm(database, str)

        data_type = type(data)
        if data_type == LegacyCursorResult:
            data = self.fetch_table(data)
        elif data_type == dict:
            data = [data]
        fields = list({key for row in data for key in row})
        fields_str = ','.join(['`%s`' % field for field in fields])
        fields_str_position = ','.join([':' + field for field in fields])
        if duplicate_method == 'ignore':
            sql = (
                f'INSERT IGNORE INTO `{database}`.`{table}`({fields_str})\n'
                f'VALUES({fields_str_position})'
            )
        elif duplicate_method == 'update':
            update_content = ','.join(['`%s` = VALUES(`%s`)' % (field, field) for field in fields])
            sql = (
                f'INSERT INTO `{database}`.`{table}`({fields_str})\n'
                f'VALUES({fields_str_position})\n'
                'ON DUPLICATE KEY UPDATE\n'
                f'{update_content}'
            )
        else:
            sql = (
                f'INSERT INTO `{database}`.`{table}`({fields_str})\n'
                f'VALUES({fields_str_position})'
            )
        if print_sql:
            rbasic.rprint(sql, title='SQL', format_module=None)
        self.execute(sql, data)