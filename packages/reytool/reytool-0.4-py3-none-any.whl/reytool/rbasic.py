...
def fill_width(text: 'str', char: 'str', width: 'int', align: 'str("right" | "left" | "center")'='right') -> 'str':
    '''
...
def rprint(
        *args: 'object',
        ...
        format_module: 'str("pprint") | str("reytool") | None'='pprint'
    ) -> 'None':
    '''
    ...
    *args: object
    ...
    format_module: 'pprint' or 'reytool' or None
        content format module
        when is 'pprint', then use function pformat
        when is 'reytool', then use function format_data
        when is None, then not format
    '''
...
def get_datetime_str(obj: 'object'='now_datetime') -> 'str | object':
    '''
    Get datetime string
    '''
    ...
    if obj == 'now_datetime':
        ...
        ret = now_datetime.strftime('%Y-%m-%d %H:%M:%S')
    elif obj == 'now_date':
        now_datetime = datetime.datetime.now()
        ret = now_datetime.strftime('%Y-%m-%d')
    elif obj == 'now_time':
        now_datetime = datetime.datetime.now()
        ret = now_datetime.strftime('%H:%M:%S')
        ...
        ret = time.strftime('%H:%M:%S', time.gmtime(obj.seconds))
    else:
        ret = obj
    return ret
...
def get_paths(path: 'str | None'=None, target: 'str'='all', recursion: 'bool'=True) -> 'list':
    ...
    target: str, 'all' or 'file' or 'folder'
        when 'all', then get file and folder path
        when 'file, then get file path
        when 'folder', then get folder path
