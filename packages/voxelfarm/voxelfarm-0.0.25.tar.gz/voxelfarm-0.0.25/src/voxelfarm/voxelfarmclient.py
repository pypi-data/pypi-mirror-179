...
class rest:
    ...
    def workflow_get_timings(self, project_id):
        ...
        for collection in collections:
            if "workflow_product" in collections[collection]:
                entities = []
                version_queue_time = 0
                version_processing_time = 0
                version_workflow_time = 0
                for entity_id in collections:
                    if "file_folder" in collections[entity_id] and collections[entity_id]["file_folder"] == collections[collection]["ID"]:
                        ...
                        if processing_time > 0:
                            ...
                            workflow_time = file_date - int(collections[collection]["capture_date"])
                            entities.append({
                            ...
                            version_processing_time += processing_time
                            version_queue_time += queue_time
                            version_workflow_time += workflow_time
                if version_processing_time is not None and version_processing_time > 0:
                    workflow_name = collections[collection]["workflow_product"].replace("META_", "")
                    workflow_index = -1
                    ...
                    workflows[workflow_index]["versions"].append({
                        "name": (''.join(filter(str.isdigit, collections[collection]["name"]))),
                        "id": collections[collection]["ID"],
                        "workflow_time": version_workflow_time,
                        "queue_time": version_queue_time,
                        "processing_time": version_processing_time,
                        "entities": entities
                    })

        return pd.DataFrame.from_dict(workflows)        
