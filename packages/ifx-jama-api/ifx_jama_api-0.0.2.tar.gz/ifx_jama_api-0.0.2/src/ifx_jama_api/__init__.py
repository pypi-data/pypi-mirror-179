...
#            self.__loadProjectItems()
    ...
    def post_item(self, resource, payload):
        url = self.baseURL + resource
        response = requests.post(url, json=payload,
                                auth=self.__auth, verify=False)
        daten=response.json()
    ...
    def post_alt(self, resource, name, item_type, child_item_type, parent_id, set_key):

        payload = {
            "project": self.__prj_id,
            "itemType": item_type,
...
def create_TC_for_ReQ(parent_id_dict,Jama):
    req_TC_list=[]

    #create set of TC for set of ReQ:
    for element in parent_id_dict:
        element_info=Jama.getItemByID(element)
        status, req_set_id = Jama.post_alt("items", "TCs for '" + element_info["fields"]["name"] + "'", 31, 26, element, "TC") #posts TC set
...
def create_all_TCs(req_TC_list, Jama, project_id):
    for req_id, set_id in req_TC_list:
        ...
        for dictionary in req_children:
            ...
            if dictionary["itemType"]==32:
                status, tc_folder_id=Jama.post_alt("items", dictionary["fields"]["name"], 32, 26, set_id, "TC")
                ...
                for item in req_folder_children:
                    ...
                    if  item["itemType"]==57:
                        ...
                        status, tc_id=Jama.post_item("items", payload)
                        status_response=Jama.setRelationship(item["id"],tc_id,  15)
                        print(status)
                        print("Relationship:", status_response)
...
class JamaAPI:
    ...
    def generateTC(self, MainCompID):
        ...
        req_TC_list = create_TC_for_ReQ(parent_id_dict, self.Jama)
        create_all_TCs(req_TC_list, self.Jama, self.project_id)
