...
class AsyncKafkaSubscriber(AsyncSubscriber):
    ...
    def _process_received_task(self, payload: bytes, meta: ConsumerRecord) -> Task:
        ...
        task._kafka_subscriber = (meta.topic, meta.partition, meta.offset)
        return task
    ...
    async def commit(self, task: Task, timeout: float = None):
        ...
        :return:
        """
        assert hasattr(task, '_kafka_subscriber'), "cannot commit task not received by this subscriber"
        tp = TopicPartition(task._kafka_subscriber[0], task._kafka_subscriber[1])
        offset = task._kafka_subscriber[2]
