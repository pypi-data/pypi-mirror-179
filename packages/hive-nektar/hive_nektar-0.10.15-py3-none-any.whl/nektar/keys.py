...
class Address(object):
    ...
    def __bytes__(self):
        ...
        if self._address is None:
            return as_bytes(self.derivesha512address())
        else:
            return as_bytes(self._address)
...
class PublicKey(object):
    ...
    def compressed(self):
        ...
        p = ecdsa.VerifyingKey.from_string(
            as_bytes(self), curve=ecdsa.SECP256k1
        ).pubkey.point
        x_str = ecdsa.util.number_to_string(p.x(), order)
        compressed = hexlify(as_bytes(chr(2 + (p.y() & 1)), "ascii") + x_str).decode(
            "ascii"
        )
        return compressed
    ...
    def unCompressed(self):
        ...
        if prefix == "04":
            return public_key
        if prefix in ("02", "03"):
            raise NektarException("Error encountered in deriving uncompressed key.")
...
class PrivateKey(object):
    ...
    def __init__(self, wif=None, prefix="STM"):
        ...
        self.uncompressed.address = Address(
            pubkey=self.pubkeyuncompressedhex, prefix=prefix
        )
