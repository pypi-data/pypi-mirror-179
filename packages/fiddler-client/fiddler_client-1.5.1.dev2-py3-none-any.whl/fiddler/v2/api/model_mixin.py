...
from fiddler.v2.api.model_artifact_upload import ModelUploader, MultiPartModelUploader
...
class ModelMixin:
    ...
    def add_model_surrogate(
        ...
        response = self.client.post(url='surrogate-model/generate', data=payload)
    ...
    def _upload_model_artifact(
        ...
        """
        Upload model artifact of an existing model
        :param model_name: Model name
        :param project_name: Project name
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Choose uploader based on archive file size
            if os.path.getsize(file_path) < MULTI_PART_CHUNK_SIZE:
                uploader_class = ModelUploader
            else:
                uploader_class = MultiPartModelUploader
            ...
            job_uuid = uploader.upload(file_path=Path(file_path))
    ...
    def add_model_artifact(
        ...
        :return: Async job uuid
        """
        return self._upload_model_artifact(
            project_name=project_name,
            model_name=model_name,
