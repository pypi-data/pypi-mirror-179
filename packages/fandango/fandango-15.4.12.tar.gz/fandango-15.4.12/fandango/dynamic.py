                ...
                if device in devs_in_server:
                    ...
                    if full_name not in self._external_attributes:
                        self.debug(
                            ...
                            #a = ft.TAU.Attribute(full_name)
                            ## full_name = ft.get_model_name(a) #If the host is external it must be specified in the formula
                            ...
                            #) and self.check_attribute_events(
                                #self._locals.get("ATTRIBUTE")
                            #):
                                ## If Attribute has events, evalAttr() 
                                ## will be called at every event_received
                                ...
                                #self._external_listeners[ft.get_model_name(a)].add(
                                    #self._locals["ATTRIBUTE"]
                                #)
    ...
    def event_received(self, source, type_, attr_value):
        ...
        if clmatch("config|attr_conf", etype):
            ...
            try:
                ...
                full_name = ft.get_model_name(source)  # .get_full_name()
