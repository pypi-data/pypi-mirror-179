...
from tickerdax.formatting import truncate_datetime, convert_timeframe_to_seconds, get_string_range
...
class Downloader(ConfigBase):
    ...
    def display_missing_ranges(self):
        gaps = {}
            ...
            for missing_value in missing_values:
                ...
                if last_missing_timestamp:
                    ...
                    if missing_value['id'] != incremented_timestamp:
                        print('Found gap!')
                        gaps[route].append(get_string_range(gap_start_timestamp, last_missing_timestamp))
                        gap_start_timestamp = None
                last_missing_timestamp = missing_value['id']
            gaps[route].append(get_string_range(gap_start_timestamp, last_missing_timestamp))

        return '\n\t\t* '.join([f'{key}\n\t\t\t{value}' for key, value in gaps.items()])
    ...
    def download(self):
        ...
        if routes:
            ...
            print(
                f'\nDownload Summary:'
                f'\n\t- {cached_items} of the requested items were already cached.'
                f'\n\t- {downloaded_items} items were downloaded.'
                f'\n\t- {missing_items} items are missing.'
                f'\n\t\t* {self.display_missing_ranges()}'
            )
