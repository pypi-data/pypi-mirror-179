...
class JoTTS:
	...
	def __prepare_model__(self, force_model_download, model_name):
		DOWNLOAD_URL_ROOT = "https://github.com/padmalcom/jotts/releases/download/{folder}/{file}.pt"
		download_url = DOWNLOAD_URL_ROOT.format(folder=model_name, file=model_name)
		...
		MODEL_FILE = os.path.join(MODEL_FILE, "{file}.pt".format(file=model_name))
	...
	def __init__(self, force_model_download=False, model_name="v0.1"):
		logger.debug("Initializing JoTTS...")
		
		MODEL_FILE = self.__prepare_model__(force_model_download, model_name);
