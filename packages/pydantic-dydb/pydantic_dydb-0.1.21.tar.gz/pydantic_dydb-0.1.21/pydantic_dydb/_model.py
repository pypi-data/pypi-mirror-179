...
from pydantic_dydb._initialization import get_table_data
...
def paginator(func: callable, request_body):
    ...
    while continue_key is not None:
        ...
        resp = func(_json.byte_dumps(request_body))
        handle_error(resp)
...
class TableBaseModel(pydantic.BaseModel):
    ...
    def save(self):
        ...
        resp = table_data.dydb.put_item(_json.byte_dumps(body))
        handle_error(resp)
    ...
    @classmethod
    def get(cls, *, hash_key, range_key: Optional[str]):
        cls._setup_class()
        table_data = get_table_data()
        ...
        resp = table_data.dydb.get_item(body)
        handle_error(resp)
    ...
    @classmethod
    def batch_get(cls, items: Union[list[str] | list[tuple[str, str]]]):
        ...
        for chunk in chunks:
            ...
            resp = table_data.dydb.batch_get(_json.byte_dumps(body))
            handle_error(resp)
    ...
    @classmethod
    def query(cls, primary_key_value: str, filter_range=False):
        # todo implement retries if request limit exceeded
        cls._setup_class()
        table_data = get_table_data()
    ...
    @classmethod
    def delete(cls, hash_key: str, range_key: Optional[str] = None):
        cls._setup_class()
        table_data = get_table_data()
        ...
        resp = table_data.dydb.delete(_json.byte_dumps(body))
        handle_error(resp)
...
def batch_delete_obj(items: list[TableBaseModel]):
    # todo implement multiple delete calls if unprocessed items
    # todo implement retries if request limit exceeded
    table_data = get_table_data()
...
def batch_write(items: list[TableBaseModel]):
