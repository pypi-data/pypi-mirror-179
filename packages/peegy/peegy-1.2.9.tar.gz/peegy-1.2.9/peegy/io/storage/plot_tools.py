...
def plot_time_frequency_responses(dataframe: pd.DataFrame = None,
    ...
    for _id, ((_current_row_group, _current_col_group), _group) in enumerate(groups):
        ...
        _columns_legends = []
        _colors = plt.cm.get_cmap('viridis', _group.shape[0])
        for _col in _group:
            if _col in ['x', 'y']:
                continue
            if len(_group[_col].unique()) > 1:
        ...
        for _i, (_, _row) in enumerate(_group.iterrows()):
            ...
            if y_unit_to is None:
                y_unit_to = y_unit.unit
            if x_unit_to is None:
                x_unit_to = x_unit.unit
            y = (_row['y'] * y_unit).to(y_unit_to)
            x = (_row['x'] * x_unit).to(x_unit_to)
            fs = _row['x_fs']
            ...
            if _domain == 'time':
                ...
                ax.set_xlabel('Time [{:}]'.format(x_unit))
            if _domain == 'frequency':
                ...
                ax.set_xlabel('Frequency [{:}]'.format(x_unit_to))
