...
from netapp_ontap.resources import Svm, Volume, FcInterface
from netapp_ontap.resources import Igroup, LunMap
from netapp_ontap.resources import IgroupInitiator, Node, IpInterface, Lun, Software
...
class ontap():
    def __init__(self, hostname, user, passwd, timeout=None, san_dev=None,):
        ...
        config.CONNECTION = HostConnection(self.hostname, username=self.user, password=self.passwd,
                                           verify=False, poll_timeout=timeout)
    ...
    def lun_info(self, lun_name):
        ...
        if not lun_name:
            ...
            return None
        if not self.luns or lun_name not in self.luns:
    ...
    def get_nodes(self, ip=None):
        ...
        try:
            for node in Node.get_collection():
                ...
                nodes[node.name]['version'] = '%s.%s' % (node.version.generation, node.version.major)
        ...
        try:
            for igroup in Igroup.get_collection(**{"svm.name": vm_name}, fields="uuid"):
                ...
                if vm_name == igroup.svm.name:
                    if isinstance(h_wwpn, list):
                        if all(wwpn in igroups[igroup.name]['members'] for wwpn in h_wwpn):
    ...
    def get_initiators(self, vm_name, igroup_name):
        ...
        if 'uuid' in igroup:
            ...
            _print("FAIL: Could not find the igroup uuid with %s in svm %s" % (igroup_name, vm_name))
            return init_name
    ...
    def get_fc_interface(self, svm=None, interface_name=None,):
        target_lif_fc_ports = {}
        try:
            for port in FcInterface.get_collection():
                ...
                target_lif_fc_ports[port.name]['location'] = {'node': port.location.port.node.name,
    ...
    def mod_lif_fc_port(self, lif_name, enable=True):
        try:
            ...
            if lif_fc_port.patch(poll=True):
                _print("INFO: update the lif fc port %s to %s" % (lif_name, enable))
        except NetAppRestError as error:
            _print("FAIL: Error:- %s" % error.http_err_response.http_response.text)
            _print("FAIL: Exception caught :" + str(error))
            return False
    ...
    def lun_create(self, name, size, os_type='linux', wwid=None):
        ...
        if self.vol:
            ...
            vol = self.get_sa_conf()['vol']

        if "/vol/" + vol not in name:
            lun_full_path = "/vol/" + vol + "/" + name
        else:
            lun_full_path = name
        sname = lun_full_path.split('/')[-1]
        _print("INFO: will crate new lun with vol %s  verver %s" % (vol, svm))
    ...
    def lun_map(self, lun_full_name,):
    ...
    def sa_ctrler_t_wwpns(self,  ctrler_ip,):
        ...
        _print("DEBUG: vsm %s FC interface info %s" % (svm, all_fc_interface))
        _print("DEBUG: the IP %s is from node name %s" % (ctrler_ip, node_name))
    ...
    def system_node_power_on(self, node_name):
        ...
        response = CLI().execute(
            ...
            body={"node": node_name}, privilege_level="diagnostic")
        time.sleep(3)
        response = CLI().execute(
    ...
    def system_node_power_off(self, node_name):
        response = CLI().execute(
            "system node power off", body={"node": node_name}, privilege_level="diagnostic")
        time.sleep(3)
        response = CLI().execute(
