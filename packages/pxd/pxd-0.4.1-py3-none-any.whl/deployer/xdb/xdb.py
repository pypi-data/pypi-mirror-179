...
class Xdb:
    ...
    def _generate_xdb_node_mem_limit(self, node):
        if node.role == 'Logger':
            return min(self.mem_limit, constant.LOGGER_MAX_MEM)
    ...
    def storage_type(self):
        ...
        if self.engine_type == constant.XCLUSTER_ENGINE:
            if self.leader_only:
                return constant.STORAGE_TYPE_XCLUSTER_LEADER_ONLY
            else:
                return constant.STORAGE_TYPE_XCLUSTER
        elif self.engine_type == constant.GALAXY_ENGINE:
            if self.leader_only:
                return constant.STORAGE_TYPE_GALAXY_LEADER_ONLY
            else:
