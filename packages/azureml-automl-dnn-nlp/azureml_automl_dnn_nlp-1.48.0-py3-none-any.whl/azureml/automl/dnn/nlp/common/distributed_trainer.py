...
class DistributedTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        Function to override the HF trainer's evaluate method, ensuring it only triggers when the
        process is main process.

        :param eval_dataset: dataset to use for evaluation, if the default self.eval_dataset is not desired.
        :return: the dictionary of metrics summarizing the evaluation results.
        """
        if is_main_process():
    ...
    def predict(self, test_dataset: Dataset) -> PredictionOutput:
        """
        Function to override HF trainer's predict method, ensuring it only triggers when the
        process is the main process.

        :param test_dataset: dataset to use for prediction.
        :return: a named tuple representing the prediction output, with predictions and optionally metrics.
        """
        if is_main_process():
