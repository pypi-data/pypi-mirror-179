...
class MainWindow(QtWidgets.QMainWindow, QtCore.QObject):
    ...
    def __openViewGraphics__(self):
        self.viewGraphics = ViewGraphicsDialog(self.centralwidget , not self.profileDockX1.isHidden(), not self.profileDockX2.isHidden(), not self.profileDockY1.isHidden() ,not self.profileDockY2.isHidden(), not self.descriptionData.isHidden(), not self.counterDock1.isHidden(), not self.counterDock2.isHidden())
        ...
        self.viewGraphics.getUI().checkBoxCounter1.stateChanged.connect(lambda :self.__changeHiddenModeDock__(self.counterDock1))
        self.viewGraphics.getUI().checkBoxCounter2.stateChanged.connect(lambda :self.__changeHiddenModeDock__(self.counterDock2))
