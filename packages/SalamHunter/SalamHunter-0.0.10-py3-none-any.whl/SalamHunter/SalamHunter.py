...
class create_IG():
	   def __init__(self) -> None:
	       ...
	       self.password = "SalamHunter"+str(random.randint(0,30))
	       self.name = "SalamHunter ."
	       self.register()
	   def register(self):
	       	...
	       	self.user = "T5B55_"+''.join(random.choice(string.digits)for i in range(5))
	       	...
	       	if self.confirmation.json()['status'] == "ok":
                ...
                'force_sign_up_code': signup_code})
	       	   if '"account_created": false' in self.create.text:
	       	       ...
	       	       return {'status':'Success','username':str(self.user),'password':str(self.password),'email':str(self.maile)}
	       	else:
	       	   return {'status':'error'}
	   def getmail(self):
