...
# from coti_wallet.base import base.http_pool_manager, base.api_call_times_logger
# from coti_wallet.crypto_helper import HashAndSign, PrivateKeyFromSeed, PublicKeyFromPrivateKey, HashKeccak256, \
#     SignDigest
...
def get_nodes_details(node_manager_address):
    ...
    res = base.http_pool_manager.request('GET', node_manager_address + "/wallet/nodes", headers=headers)
    ...
    base.api_call_times_logger.info("\t\t---> get_nodes_details: %s seconds ---" % round((time.time() - start_time), 3))
...
def get_account_balance(address_id, full_node_address):
    ...
    headers = {'Content-Type': "application/json"}
    res = base.http_pool_manager.request("POST", full_node_address + "/balance", body=payload, headers=headers)
    ...
    base.api_call_times_logger.info("\t\t---> get_account_balance: %s seconds ---" % round((time.time() - start_time), 3))

    return data["addressesBalance"][address_id]["addressBalance"]


def create_full_node_fee(full_node_address, user_hash, private_key, original_currency_hash, amount, context,
                         fee_included):
    start_time = time.time()

    original_amount = Decimal(amount).normalize(context)

    fnf_req_msg = bytes.fromhex(original_currency_hash) + str(format(original_amount, "f")).encode()
    signed_fnf_req_msg, _ = crypto_helper.HashAndSign(bytearray.fromhex(private_key), fnf_req_msg)
    signature_data = {"r": signed_fnf_req_msg[0], "s": signed_fnf_req_msg[1]}

    body = {
        "originalAmount": format(original_amount, "f"),
    ...
    headers = {'Content-Type': "application/json"}
    res = base.http_pool_manager.request('PUT', full_node_address + "/fee", body=json.dumps(body), headers=headers)
    ...
    base.api_call_times_logger.info("\t\t---> create_full_node_fee: %s seconds <---" % round((time.time() - start_time), 3))
...
def validate_network_fee(trust_score_address, user_hash, full_node_fee, network_fee_data):
    ...
    res = base.http_pool_manager.request('POST', trust_score_address + "/networkFee",
                                         body=json.dumps(network_fee_request), headers=headers)
    ...
    base.api_call_times_logger.info("\t\t---> validate_network_fee: %s seconds <---" % round((time.time() - start_time), 3))
...
def create_network_fee(trust_score_address, public_key, full_node_fee):
    ...
    res = base.http_pool_manager.request('PUT', trust_score_address + "/networkFee",
    ...
    base.api_call_times_logger.info("\t\t---> create_network_fee: %s seconds <---" % round((time.time() - start_time), 3))

    return data['networkFeeData']


def build_receiver_base_transaction(instant_time, instant_time1, amount, currency_hash, destination_address):
    start_time = time.time()
...
def get_trust_score_data(trust_score_address, public_key, private_key, transaction_hash):
    ...
    signed_transaction_hash = crypto_helper.SignDigest(bytearray.fromhex(private_key), bytearray.fromhex(transaction_hash))
    body = {
        "userHash": public_key,
    ...
    headers = {'Content-Type': "application/json"}
    res = base.http_pool_manager.request('POST', trust_score_address + "/transactiontrustscore",
    ...
    base.api_call_times_logger.info("\t\t---> get_trust_score_data: %s seconds <---" % round((time.time() - start_time), 3))

    return data['transactionTrustScoreData']


def create_input_base_transaction(address_private, currency_hash, full_node_fee,
                                  instant_time, instant_time1, network_fee_data,
    ...
    ibt_msg = bytearray.fromhex(source_address) \
        ...
        + instant_time1.to_bytes(8, byteorder='big') + bytearray.fromhex(currency_hash)
    ibt_hash = crypto_helper.HashKeccak256(ibt_msg)
    transaction_hash_bytes = ibt_hash + full_node_fee["hash"] + network_fee_data["hash"] \
        + receiver_base_transaction["hash"]
    transaction_hash = crypto_helper.HashKeccak256(bytearray.fromhex(transaction_hash_bytes))
    address_signed_transaction_hash = crypto_helper.SignDigest(bytearray.fromhex(address_private),
...
def call_apis_to_prepare_a_tx(source_address_public_key_crc, destination_address, amount, currency_hash, seed,
                              address_private,
                              transaction_description, full_node_address, trust_score_address,
                              fee_included):
    start_time = time.time()

    instant_time = int(datetime.datetime.now().timestamp())
    instant_time1 = instant_time * 1000

    context = init_context()
    private_key = crypto_helper.PrivateKeyFromSeed(bytearray.fromhex(str(seed)))
    public_key, _ = crypto_helper.PublicKeyFromPrivateKey(bytearray.fromhex(str(private_key)))

    full_node_fee = create_full_node_fee(full_node_address, public_key, private_key, currency_hash, amount,
                                         context, fee_included)
    ...
    receiver_base_transaction = build_receiver_base_transaction(instant_time, instant_time1, amount, currency_hash,
                                                                destination_address)

    input_base_transaction, transaction_hash = create_input_base_transaction(
        address_private, currency_hash, full_node_fee,
        instant_time, instant_time1,
        network_fee_data,
    ...
    return full_node_fee, input_base_transaction, instant_time, instant_time1, network_fee_data, private_key, \
        public_key, receiver_base_transaction, transaction_description, transaction_hash, \
        transaction_trust_score_data


def create_transaction(full_node_address, full_node_fee, input_base_transaction, instant_time, instant_time1,
                       network_fee_data,
    ...
    st, _ = crypto_helper.HashAndSign(bytearray.fromhex(private_key), tx_bytes)
    ...
    res = base.http_pool_manager.request('PUT', full_node_address + "/transaction",
                                    body=json.dumps(body), headers=headers)
    if res.status not in http_ok_codes:
        raise Exception('error return code: ' + str(res.status) + ', data: ' + str(res.data))
    ...
    base.api_call_times_logger.info("\t\t---> create_transaction: %s seconds <---" % round((time.time() - start_time), 3))
