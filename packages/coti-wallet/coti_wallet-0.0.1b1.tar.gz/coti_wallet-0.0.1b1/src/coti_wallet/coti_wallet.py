...
from base import http_pool_manager, api_call_times_logger
from crypto_helper import HashAndSign, PrivateKeyFromSeed, PublicKeyFromPrivateKey, HashKeccak256, \
    SignDigest
...
def get_account_balance(address_id, full_node_address):
    ...
    headers = {'Content-Type': "application/json"}
    res = http_pool_manager.request("POST", full_node_address + "/balance",
...
def create_full_node_fee(full_node_address, user_hash, private_key, original_currency_hash, amount, context,
    ...
    headers = {'Content-Type': "application/json"}
    res = http_pool_manager.request('PUT', full_node_address + "/fee",
...
def validate_network_fee(trust_score_address, user_hash, full_node_fee, network_fee_data):
    ...
    res = http_pool_manager.request('POST', trust_score_address + "/networkFee",
