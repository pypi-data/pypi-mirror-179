...
from federatedcore.misc.enums import Framework
...
def mean(parameter):
    ...
    if paramter_framework is Framework.torch:
        from federatedcore.compatibility.framework.torch import mean
        return mean(parameter)

    if paramter_framework is Framework.tensorflow:
        from federatedcore.compatibility.framework.tensorflow import mean
        return mean(parameter)


def inject_weight_gradient_func(net_func):
    if detect_framework(net_func) is Framework.torch:
        from federatedcore.compatibility.framework.torch import get_weights, set_weights, get_gradients, apply_gradients
    ...
    if detect_framework(net_func) is Framework.tensorflow:
        from federatedcore.compatibility.framework.tensorflow import get_weights, set_weights, get_gradients, apply_gradients
