...
class Topology:
    ...
    def _wfs_at_kpt(self, kpt):
        ...
        """
        modified_wf = []
        dy_mt, all_freqs, eig_vecs = self.model.solve_dynamical_matrix_kpath(
            [kpt], k_num=1
        )
        dim = len(all_freqs[0])
        for i in range(dim):
    ...
    def berry_curvature(self, kpt, band_indices, delta=1e-9):
        ...
        d, freqs, eig_vecs = self.model.solve_dynamical_matrix_kpath([kpt], k_num=1)
        ...
        if self.dim == 2:
            ...
            d_x, _, _ = self.model.solve_dynamical_matrix_kpath(
                [kxp],
                k_num=1,
            )
            d_y, _, _ = self.model.solve_dynamical_matrix_kpath(
                [kyp],
                k_num=1,
            )
            delta_x = (d_x - d) / delta
            ...
            for i in range(num_deg):
                for j in range(num_deg):
                    ...
                    for m in range(num_bands):
                        ...
                        if abs(fi - fm) < 1e-11:
                            raise Exception(
                                "band {} and {} are degenerate".format(i, m)
                            )
                        if abs(fj - fm) < 1e-11:
                            raise Exception(
                                "band {} and {} are degenerate".format(j, m)
                            )
                        prod1, prod2 = np.zeros(2, dtype=complex), np.zeros(
                            2, dtype=complex
                        )
            ...
            d_z, _, _ = self.model.solve_dynamical_matrix_kpath(
                [kzp],
                k_num=1,
            )
            delta_x = (d_x - d) / delta
            ...
            for i in range(num_deg):
                for j in range(num_deg):
                    ...
                    for m in range(num_bands):
                        ...
                        if abs(fi - fm) < 1e-15:
                            warnings.warn(
                                "band {} and {} are likely to be degenerate at {}".format(
                                    band_indices[i], m, kpt
                                )
                            )
                        if abs(fj - fm) < 1e-15:
                            warnings.warn(
                                "band {} and {} are likely to be degenerate at {}".format(
                                    band_indices[j], m, kpt
                                )
                            )
                        prod1, prod2 = np.zeros(3, dtype=complex), np.zeros(
                            3, dtype=complex
                        )
                        prod1[0] = np.dot(wfi.conjugate(), np.dot(delta_x, wfm))
                        prod1[1] = np.dot(wfi.conjugate(), np.dot(delta_y, wfm))
            ...
            raise Exception(
                "To calculate berry curvatures, the dimension must\
                            be 2 or 3"
            )
