...
"""
from gTixi import alib, waFile, waText
aWaEnv=alib('[sWal|WalArtmin]')

a支持的平台=alib('''[Python3|3.6+在电脑和GT0上运行的]
[QPython3|在安卓平台上运行的]
[microPython|在ESP32等微控制器上运行的]''')

a支持的设备=alib('''[oDraw|[#|小屏幕控制]
...
class SpSmp(Shipei): #树莓派适配
    def __init__(self,aenv):
        ...
        i2c=self.oi2c
        self.i2c=alib(dict(write_byte_data=i2c.write_byte_data, read_byte_data=i2c.read_byte_data, read_i2c_block_data=i2c.read_i2c_block_data))
        self.hasSsd=None
        self.oSvg=None
        self.InitDisplay()
...
class SpBdz(Shipei): #GT1 adapter
    def __init__(self,aenv):
        ...
        try:
            ...
            self.oi2c=SMBus(6)
            i2c=self.oi2c
            self.i2c=alib(dict(write_byte_data=i2c.write_byte_data, read_byte_data=i2c.read_byte_data))
        except:
...
##################################################
DEBUG=False
aI2c=alib('''[#comment|此体系中器件有统一i2c地址]
...
print(aWaEnv.ToString(''))

if __name__ == "__main__":
    pass
    print(wa.DrawStdLines("34234\nfdgfg\n"))
    print(SpSmp(aWaEnv).CheckKey())
