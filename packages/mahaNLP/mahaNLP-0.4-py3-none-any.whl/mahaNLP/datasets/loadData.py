...
def checkdir(modelname):
    ...
    if not os.path.isdir(rootPath):
        os.makedirs(os.getcwd()+'/.cache/mahaNLP/'+modelname,exist_ok=True)
        return os.getcwd()
    else:
        rootPath = os.path.expanduser('~\.cache/')
        os.chdir(rootPath)
        os.makedirs('mahaNLP/'+modelname,exist_ok=True)
        return '~'
...
def downloadMahasent():
    ...
    childPath = os.path.expanduser(rootPath+'/.cache/mahaNLP/mahaSent')
    os.chdir(childPath)
...
def downloadMahaner():
    ...
    childPath = os.path.expanduser(rootPath+'/.cache/mahaNLP/mahaNER/IOB')
    os.chdir(childPath)
    for key in iob:
    ...
    childPath = os.path.expanduser(rootPath+'/.cache/mahaNLP/mahaNER/NON_IOB')
    os.chdir(childPath)
    for key in non_iob:
...
def downloadMahahate():
    ...
    childPath = os.path.expanduser(rootPath+'/.cache/mahaNLP/mahaHate/2-class')
    os.chdir(childPath)
    for key in class_2:
    ...
    childPath = os.path.expanduser(rootPath+'/.cache/mahaNLP/mahaHate/4-class')
    os.chdir(childPath)
    for key in class_4:
