...
class HostManagerServicer(csle_collector.host_manager.host_manager_pb2_grpc.HostManagerServicer):
    ...
    def _get_filebeat_status() -> bool:
        ...
        if err is not None:
            err_output = err.decode()
        filebeat_running = not ("not" in status_output)
        logging.info(f"Got filebeat status, output:{status_output}, err output: {err_output} ")
    ...
    def _start_filebeat() -> None:
        ...
        if err is not None:
            err = err.decode()
        logging.info(f"Started filebeat, output:{output}, err output: {err} ")
    ...
    def _stop_filebeat() -> None:
        ...
        if err is not None:
            err = err.decode()
        logging.info(f"Stopped filebeat, output:{output}, err output: {err} ")
    ...
    def set_filebeat_snort_module_config() -> None:
        ...
        if err is not None:
            err = err.decode()
        logging.info(f"Module enabled, output: {output}, err output: {err}")
