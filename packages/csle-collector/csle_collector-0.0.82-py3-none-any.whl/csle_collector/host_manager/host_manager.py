...
class HostManagerServicer(csle_collector.host_manager.host_manager_pb2_grpc.HostManagerServicer):
    ...
    def _get_filebeat_status() -> bool:
        ...
        logging.info(f"Got filebeat status, output:{status_output}, err output: {err.decode()} ")
    ...
    def _start_filebeat() -> None:
        ...
        p = subprocess.Popen(constants.FILEBEAT.FILEBEAT_START, stdout=subprocess.PIPE, shell=True)
        (output, err) = p.communicate()
        p.wait()
        logging.info(f"Started filebeat, output:{output.decode()}, err output: {err.decode()} ")
    ...
    def _stop_filebeat() -> None:
        ...
        p = subprocess.Popen(constants.FILEBEAT.FILEBEAT_STOP, stdout=subprocess.PIPE, shell=True)
        (output, err) = p.communicate()
        p.wait()
        logging.info(f"Stopped filebeat, output:{output.decode()}, err output: {err.decode()} ")
    ...
    def set_filebeat_snort_module_config() -> None:
        ...
        (output, err) = p.communicate()
        p.wait()
        logging.info(f"Module enabled, output: {output.decode()}, err output: {err.decode()}")
