...
class encryption(object):
    ...
    def __init__(cls, auth_key: str) -> ...:
        if not 32 == len(auth_key) : raise IndexError('[SORRY]: LEN YOUR AUTH IS NOT \'32\'')
        cls.auth_key, cls.iv = bytearray((''.join(list(map(lambda a: chr((a - 97 + 9) % 26 + 97), (str(auth_key[16:24] + auth_key[:8] + auth_key[24:32] + auth_key[8:16]).encode('latin-1')))))), 'utf-8'), bytearray.fromhex(str('0' * len(auth_key)))
...
class Tools(
    ...
    def antiLinkDomain(
        ...
        cleaner_link_domain = lambda : list(map(lambda message: (app.deleteMessages([message['last_message']['message_id']], cls.target_guid) if message.get('object_guid') == cls.target_guid and Tools(cls.auth).checkLink(message['last_message']['text']) else ...), app.getChatsUpdate()))
