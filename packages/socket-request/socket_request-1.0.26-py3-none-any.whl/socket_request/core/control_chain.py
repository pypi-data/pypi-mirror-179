...
class ControlChain(ConnectSock):
    ...
    def __init__(
        ...
        super().__init__(unix_socket=unix_socket, timeout=timeout, debug=debug, headers=self.headers, wait_socket=wait_socket, http_version="1.1")
        self.url = url
    ...
    def reset(self, blockheight=None, cid=None):
        ...
        color_print(f"Reset height={hex(blockheight)}({blockheight:,}), blockHash={payload.get('blockHash')}")
        ##  TODO  #######################################
        if cid:
            self.cid = cid
        ...
        res = self.request(
            url=f"/chain/{self.cid}/reset", payload=payload, method="POST")
        self.start(cid, wait_state=False)
        return res
