...
class Account(object):
    ...
    def glue_get_jobs(self):
        ...
        jobs_df = pd.DataFrame(columns=glue_job_df_columns)
        for page in page_iterator:
            for job in page["Jobs"]:
                data = [[job["Name"], job["CreatedOn"], job["LastModifiedOn"]]]
                df_row = pd.DataFrame(data, columns=glue_job_df_columns)
                jobs_df = jobs_df.append(df_row)
    ...
    def glue_get_job_history(self, job_name, no_of_runs=1):
        ...
        job_run_df = pd.DataFrame(columns=glue_run_df_columns)
        ...
        for page in page_iterator:
            for jobrun in page["JobRuns"]:
                ...
                row_df = pd.DataFrame(
                    ...
                    columns=glue_run_df_columns,
                )
                job_run_df = job_run_df.append(row_df, ignore_index=True)
    ...
    def lambda_get_functions(self):
        ...
        functions_df = pd.DataFrame(columns=dataframe_columns)
        ...
        for page in page_iterator:
            for func in page["Functions"]:
                ...
                row_df = pd.DataFrame(row, columns=dataframe_columns)
                functions_df = functions_df.append(row_df)
    ...
    def lambda_get_metrics_list(self, namespace="AWS/Lambda"):
        ...
        metrics_df = pd.DataFrame(columns=dataframe_columns)
    ...
    def lambda_get_invocations(self, lambda_name, start_date=None, end_date=None):
        ...
        metric_data_df = pd.DataFrame(columns=dataframe_columns)
            ...
            for metric_data_result in page["MetricDataResults"]:
                if metric_data_result["Id"] == "myrequest":
                    ...
                    for datapoint in datapoints:
                        ...
                        row_df = pd.DataFrame(row, columns=dataframe_columns)
                        metric_data_df = metric_data_df.append(row_df)
    ...
    def sfn_get_statemachines(self):
        ...
        stepfunctions_df = pd.DataFrame(columns=dataframe_columns)
        ...
        for page in page_iterator:
            for statemachine in page["stateMachines"]:
                ...
                row_df = pd.DataFrame(row, columns=dataframe_columns)
                stepfunctions_df = stepfunctions_df.append(row_df)
    ...
    def ec2_get_instance_id(self, hostname):
        ...
        ec2_instances_df = pd.DataFrame(columns=df_columns)
        ...
        for page in page_iterator:
            for ec2_instances in page["Reservations"]:
                ...
                for instance in ec2_instances["Instances"]:
                    ...
                    row_df = pd.DataFrame(row, columns=df_columns)
                    ec2_instances_df = ec2_instances_df.append(row_df)
