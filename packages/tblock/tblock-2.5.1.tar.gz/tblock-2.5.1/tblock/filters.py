...
class Filter:
            ...
            if not fetch_file(self.source, self.id, self.tmp_file, self.quiet):
                for mirror in self.mirrors.keys():
                    if "compression" in self.mirrors[mirror].keys():
                        if self.retrieve_mirror(mirror, compression=self.mirrors[mirror]["compression"]):
                            break
                    else:
    ...
    def update(self, force: bool = False) -> None:
        ...
        elif self.cache_is_up_to_date() and self.get_rules_count() == self.rules_count and not force:
            if not self.quiet:
    ...
    def add_custom(self, custom_syntax: str = None) -> None:
        ...
        if self.exists:
            ...
            raise FilterError("filter list cannot have the same ID than user rules: {0}".format(self.id))
    ...
    if subscribing_only and not_subscribing_only or from_repo_only and custom_only:
        ...
        with sqlite3.connect(Path.DATABASE) as conn:
            if subscribing_only and from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1 AND on_repo=1 ORDER BY id ASC;")
            elif subscribing_only and custom_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1 AND on_repo=0 ORDER BY id ASC;")
            elif subscribing_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1 ORDER BY id ASC;")
            elif not_subscribing_only and from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0 AND on_repo=1 ORDER BY id ASC;")
            elif not_subscribing_only and custom_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0 AND on_repo=0 ORDER BY id ASC;")
            elif not_subscribing_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0 ORDER BY id ASC;")
            elif custom_only:
                data = conn.execute("SELECT id FROM filters WHERE on_repo=0 ORDER BY id ASC;")
            elif from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE on_repo=1 ORDER BY id ASC;")
            else:
...
def subscribe(filter_lists: list, do_not_prompt: bool = False, quiet: bool = False,
    ...
    if not sync_repo:
        if get_current_repo_index_version() == 0 and not quiet:
            print(f"{Fore.YELLOW}Warning: repository index is not synced. Run tblock -Y to sync now.{Style.RESET_ALL}")
...
def subscribe_custom(filter_lists: list, do_not_prompt: bool = False, quiet: bool = False,
                     sync_repo: bool = False, full_update: bool = False,
                     permissions: FilterPermissions = Var.DEFAULT_PERMISSIONS_CUSTOM, force: bool = False,
...
def list_filter_lists(custom_only: bool = False, on_repo_only: bool = False, subscribing_only: bool = False,
                      not_subscribing_only: bool = False, quiet: bool = False) -> None:
    """
...
def show_search_info_filter_list(filter_list: Filter, quiet: bool = False):
    if quiet:
        ...
        subscribing = f"{Font.BOLD}{Fore.GREEN}[subscribing]{Style.RESET_ALL}" if filter_list.subscribing else ""
        if len(filter_list.metadata["description"]) > 90:
            desc = filter_list.metadata["description"][:90] + " ..."
        else:
            desc = filter_list.metadata["description"]
        print(f"{Font.BOLD}{filter_list.id}{Font.DEFAULT}: "
...
def search_filter_lists(query: str, custom_only: bool = False, on_repo_only: bool = False,
                        subscribing_only: bool = False, not_subscribing_only: bool = False,
...
def get_active_filter_lists_count() -> int:
    with sqlite3.connect(Path.DATABASE) as db:
        return db.cursor().execute('SELECT COUNT() FROM filters WHERE subscribing=1;').fetchone()[0]
...
def sync_filter_list_repo(quiet: bool = False, force: bool = False, lock_database: bool = True) -> None:
    ...
    for mirror in Var.REPO_MIRRORS:
        if fetch_file(mirror, "filter list repository",
                      os.path.join(Path.TMP_DIR, "index.json"), quiet=quiet) and \
           fetch_file(mirror + ".sha512", "filter list repository checksum",
                      os.path.join(Path.TMP_DIR, "index.json.sha512"), quiet=quiet):
            break
    else:
        ...
        for _filter_list in data["filters"]:
            ...
            # Be who you are and who you want to be. You are perfect.
                ...
                try:
                    ...
                    conflict_id = cursor.execute("SELECT id FROM filters WHERE source=?", (source,)).fetchone()[0]
                    if not quiet:
