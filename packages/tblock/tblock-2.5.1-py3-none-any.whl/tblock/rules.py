...
class Rule:
    ...
    def __init__(self, domain: str, user_rule: bool = False) -> None:
        ...
        with sqlite3.connect(Path.RULES_DATABASE) as conn:
            data = conn.cursor().execute(
                'SELECT "policy", "ip" FROM r WHERE domain=?;',
                (self.domain,)
            ).fetchone()
    ...
    def wildcard_exists(self) -> bool:
        with sqlite3.connect(Path.RULES_DATABASE) as conn:
            data = conn.cursor().execute("SELECT domain FROM r WHERE domain LIKE '%*%';")
        for rule in data:
            if re.match(re.compile(rule[0].replace(".", "\\.").replace("*", ".*")), self.domain):
                return True
        with sqlite3.connect(Path.DATABASE) as conn:
            data2 = conn.cursor().execute("SELECT domain FROM rules WHERE domain LIKE '%*%';")
        for rule in data2:
            if re.match(re.compile(rule[0].replace(".", "\\.").replace("*", ".*")), self.domain):
                return True
        else:
    ...
    def add(self, policy: str, ip: str = None, quiet: bool = False) -> None:
        ...
        """
        if self.exists and self.policy == policy and self.ip == ip and self.filter_id == USER_RULE_PRIORITY:
            if not quiet:
                    ...
                    if policy == RulePolicy.ALLOW or not self.wildcard_exists():
                        ...
                        if policy == RulePolicy.REDIRECT:
                            ...
                            if self.exists and self.filter_id == USER_RULE_PRIORITY:
                                ...
                                if self.exists:
                                    with sqlite3.connect(Path.DATABASE) as conn:
                                        conn.cursor().execute("DELETE FROM rules WHERE domain=?", (self.domain,))
                                with sqlite3.connect(Path.RULES_DATABASE) as conn:
                                    conn.cursor().execute(
...
def __remove_allowed_matches(quiet: bool = False) -> dict:
    ...
    wildcards = cursor.execute(
        'SELECT domain FROM rules WHERE policy=? AND domain LIKE "%*%";', (RulePolicy.ALLOW,)
    ).fetchall()
    conn.close()
    conn = sqlite3.connect(Path.RULES_DATABASE)
    cursor = conn.cursor()
    wildcards += cursor.execute('SELECT domain FROM r WHERE policy=? AND domain LIKE "%*%";', (RulePolicy.ALLOW,))
    conn.close()
    ...
    if not quiet:
        print(" {0} Checking wildcards rules: {1}/{2}".format(Icon.SUCCESS, cnt.value, len(wildcards)))
    matches = {}
...
def check_wildcard_domains(i) -> dict:
    ...
    cnt.value += 1
    if not quiet_mode:
        print(" {0} Checking wildcards rules: {1}".format(loading_icon(cnt.value), cnt.value), end="\r")
    conn = sqlite3.connect(Path.DATABASE)
    cursor = conn.cursor()
    query = cursor.execute("SELECT domain, ip FROM rules WHERE domain LIKE ? AND policy != ?;",
                           (i[0].replace("*", "%"), RulePolicy.ALLOW)).fetchall()
    for q in query:
    ...
    conn = sqlite3.connect(Path.RULES_DATABASE)
    cursor = conn.cursor()
    query = cursor.execute("SELECT domain, ip FROM r WHERE domain LIKE ? AND policy != ?;",
                           (i[0].replace("*", "%"), RulePolicy.ALLOW)).fetchall()
    for q in query:
...
def allow_domains(domains: list, do_not_prompt: bool = False, force: bool = False, quiet: bool = False,
                  also_update_hosts: bool = False) -> None:
    """
...
def get_all_rules(from_filter_lists: list = None, user_only: bool = False, standard_only: bool = False) -> list:
