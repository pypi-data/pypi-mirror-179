...
class MetricsPusher:
    ...
    def set_metrics_pod_ips(self):
        ...
        """
        try:
            socket_info_list = socket.getaddrinfo(self.metrics_service_name, self.metrics_service_port)
            self.metrics_pod_ips = {
                f'{result[-1][0]}:{self.metrics_pod_port}'
    ...
    def push_metrics(self, registry: CollectorRegistry):
        for gateway in self.metrics_pod_ips:
            try:
                ...
                LOGGER.exception(f'Failed to push to pushgateway {gateway}')
