...
class Note:
    def __init__(self, project: str):
        ...
        self.body = '''<html> <head> <style> body { font-family: "Helvetica Neue", Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.42857143; color: #333; background-color: #fff; } </style> </head> <body> <h1>Notes Notes for project {}</h1> </body> </html>'''
    ...
    def add_attachment(self, project):
        ...
        screenshots = [os.path.join(os.path.expanduser('~'), 'Desktop', f) for f in os.listdir(os.path.expanduser('~') + '/Desktop') if f.endswith('.png')]

        # get the latest screenshot
        latest_screenshot = max(screenshots, key=os.path.getctime)

        # move the latest screenshot to the project folder
        os.rename(latest_screenshot, os.path.join(p, os.path.basename(latest_screenshot)))
