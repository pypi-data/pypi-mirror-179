...
def run_cache_server(filename: str, remove_file: bool) -> None:
    """
    ...
    """
    if remove_file:
        signal.signal(signal.SIGTERM, lambda _, __, filename=filename: _try_unlink(filename))
        atexit.register(lambda filename=filename: _try_unlink(filename))
