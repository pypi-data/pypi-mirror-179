...
def extract_from_html(msg_body):
    """
    In message, emojis may be present and so
    message has to be encoded by unicode-escape algorithm,
    which converts emojis to unicode characters which will handled
    correctly in html parser.

    On returning the result, result string should be decoded by
    unicode-escape algorithm, which converts unicode characters to emojis
    """
    msg_body = msg_body.encode('unicode-escape')
    """
    ...
    return result.encode('utf8').decode('unicode-escape')
