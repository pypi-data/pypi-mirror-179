...
tab_uni =  pd.read_csv(os.path.join(location, 'datasets', 'HUMAN_9606_idmapping.dat.gz'),sep='\t')
...
def update_mapping_datasets():
	...
	with open(os.path.join(location, 'datasets', 'Homo_sapiens.gene_info.gz','w'))as f:
		f.write(ncbidb)
	with open(os.path.join(location, 'datasets', 'HUMAN_9606_idmapping.dat.gz','w')) as f:
		f.write(tab_uni)
