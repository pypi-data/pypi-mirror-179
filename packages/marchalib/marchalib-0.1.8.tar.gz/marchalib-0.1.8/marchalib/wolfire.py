...
class wolfire(object):
	...
	def eff03(self):
		ne = self.ne().value
		return 4.9e-2 / (1. + 4.e-3*(self.G0*self.T**0.5/ne/self.phi)**0.73) + ((3.7e-2*(self.T/1.e4)**0.7)/(1. + 2.0e-4*(self.G0*self.T**0.5/ne/self.phi)))
	
	def eff95(self):
		param = self.G0*self.T**0.5/self.ne().value
		return 4.9e-2 / (1. + (param/1925.)**0.73) + ((3.7e-2*(self.T/1.e4)**0.7)/(1. + (param/5000.))) #HERACLES
	
	def effBT(self):
		param = self.G0*self.T**0.5/self.ne().value
		return 4.87e-2 / (1. + 4.e-3*(param)**0.73) + ((3.65e-2*(self.T/1.e4)**0.7)/(1. + 2.e-4*param))
