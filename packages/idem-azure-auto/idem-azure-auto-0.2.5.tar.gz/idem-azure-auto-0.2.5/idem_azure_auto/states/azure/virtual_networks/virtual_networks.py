    ...
    if not response_get["result"]:
        if response_get["status"] == 404:
            if ctx.get("test", False):
                ...
                payload = hub.tool.azure.virtual_networks.virtual_networks.convert_present_to_raw_virtual_network(
                    subscription_id=subscription_id,
                    address_space=address_space,
                    location=location,
                    bgp_communities=bgp_communities,
                ...
                result[
                    "new_state"
                ] = hub.tool.azure.virtual_networks.virtual_networks.convert_raw_virtual_network_to_present(
        ...
        new_payload = hub.tool.azure.virtual_networks.virtual_networks.update_virtual_network_payload(
            subscription_id,
            response_get["ret"],
            {
                "address_space": address_space,
    ...
    async for page_result in hub.tool.azure.request.paginate(
        ...
        if resource_list:
            for resource in resource_list:
                ...
                resource_translated = hub.tool.azure.virtual_networks.virtual_networks.convert_raw_virtual_network_to_present(
                    resource=resource,
