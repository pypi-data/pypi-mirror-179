...
@aug.command()
@click.option("-d", "--dataset", prompt="数据集目录，支持COCO格式", help="数据集目录，支持COCO格式", required=True)
@click.option("-c", "--aug_file", prompt="增强配置yml文件", help="增强配置yml文件", required=True)
@click.option("-m", "--mode", type=click.Choice(['train', 'trainval']), help="增强依据，当同时存在train.txt、trainval.txt文件时，必指定其一;若只存在一个，则默认以存在的为依据")
def det_coco(dataset, aug_file, mode):
    ...
    \b
    使用示例：csp datatool aug det-coco -d '数据集目录' -c 'aug.yaml' -m 'trainval'
    """
    from csp.datatool.image_detection.aug import det_aug
    try:
...
@click.option('-d', '--dataset', multiple=True, help="数据集目录", required=True)
def voc(output, dataset):
    """
    VOC数据集合并命令

    \b
    使用示例：csp datatool merge voc -o '保存目录' -d '数据集1' -d '数据集2' ...
    """
    from csp.datatool.merge import voc
    try:
        if len(dataset) <= 1:
            raise FileNotFoundError("待合并数据集至少为两个, csp datatool merge voc -o '保存目录' -d '数据集1' -d '数据集2' ...")
        voc(output, *dataset)
        print("合并完成")
    except Exception as ae:
        print(ae)
...
@click.option("-o", "--output", prompt="合并结果保存目录", help="合并结果保存目录", required=True)
@click.option('-d', '--dataset', multiple=True, help="数据集目录", required=True)
def coco(output, dataset):
    """
    coco数据集合并命令

    \b
    使用示例：csp datatool merge coco -o '保存目录' -d '数据集1' -d '数据集2' ...
    """
    from csp.datatool.merge import coco
    try:
        if len(dataset) <= 1:
            raise FileNotFoundError("待合并数据集至少为两个, csp datatool merge coco -o '保存目录' -d '数据集1' -d '数据集2' ...")
        coco(output, *dataset)
        print("合并完成")
    except Exception as ae:
        print(ae)
...
@click.option("-o", "--output", prompt="合并结果保存目录", help="合并结果保存目录", required=True)
@click.option('-d', '--dataset', multiple=True, help="数据集目录", required=True)
def ner(output, dataset):
    """
    文本实体抽取数据集合并命令

    \b
    使用示例：csp datatool merge ner -o '保存目录' -d '数据集1' -d '数据集2' ...
    """
    from csp.datatool.merge import ner
    try:
        if len(dataset) <= 1:
            raise FileNotFoundError("待合并数据集至少为两个, csp datatool merge ner -o '保存目录' -d '数据集1' -d '数据集2' ...")
        ner(output, *dataset)
        print("合并完成")
    except Exception as ae:
        print(ae)
