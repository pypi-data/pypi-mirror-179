...
## 资源列表查看
@resource.command()
@click.option("-n", "--name", help="资源名称，不传则默认展示一级资源目录", default=None)
def list(name):
    """
    资源列表命令

    \b
    使用示例：csp resource list -n '资源名称'
    """
...
## 资源下载
@resource.command()
@click.option("-n", "--name", help="资源名称", required=True)
@click.option("-c", "--charset", help="资源编码，一般不传即可", default="UTF-8", show_default=True)
@click.option("-o", "--output", help="保存目录", required=True)
