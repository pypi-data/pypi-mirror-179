            ...
            for item in ["folder", "filename", "path", "size", "source", "segmented"]:
                ...
                if len_item > 1:
                    ...
                    with open(self.base_info_error_txt, "a+", encoding="utf-8") as fw:
                        result = {"file_name": xml_name, "message": "'{}' appears more than once".format(item), "data": {}}
                        fw.write(json.dumps(result, ensure_ascii=False))
                        fw.write("\n")
                elif len_item == 0:
                    # 字段不存在
                    flag_base = False
                    with open(self.base_info_error_txt, "a+", encoding="utf-8") as fw:
                        result = {"file_name": xml_name, "message": "no '{}'".format(item), "data": {}}
                        fw.write(json.dumps(result, ensure_ascii=False))
                        fw.write("\n")
                else:
                    ...
                    if not text:
                        flag_base = False
                        with open(self.base_info_error_txt, "a+", encoding="utf-8") as fw:
                            result = {"file_name": xml_name, "message": "'{}' is empty".format(item), "data": {}}
                            fw.write(json.dumps(result, ensure_ascii=False))
                            fw.write("\n")
            try:
    ...
    def is_num_break(self):
        ...
        for item in self.img_folder_names:
            ...
            if os.path.exists(json_path) and os.path.exists(img_path):
                ...
                logger.info("there are {} annotations in {}, and {} images in {}".format(num_anno, json_path, num_img,
                                                                                         img_path))
