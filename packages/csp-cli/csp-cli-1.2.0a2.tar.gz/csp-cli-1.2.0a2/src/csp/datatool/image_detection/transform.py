...
class Voc2Coco:
    ...
    def check_xml(self, data_types):
        ...
        if os.path.exists(os.path.join(self.voc_txt_dir, "trainval" + '.txt')):
            if num_trainval != len(xmls_in_anno):
                logger.warning("标注文件数量与trainval.txt中文件数量不等!")
            if num_trainval != len(img_in_folder):
                logger.warning("图片文件数量与trainval.txt中文件数量不等!")

        if os.path.exists(os.path.join(self.voc_txt_dir, "test" + '.txt')):
            if num_test != len(xmls_in_anno):
                logger.warning("标注文件数量与test.txt中文件数量不等!")
            if num_test != len(img_in_folder):
                logger.warning("图片文件数量与test.txt中文件数量不等!")
