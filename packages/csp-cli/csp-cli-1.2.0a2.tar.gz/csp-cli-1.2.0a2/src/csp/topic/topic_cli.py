...
@click.option("-d", "--dataset", type=click.STRING, help="数据集名称", default=None, show_default=True)
...
@click.option("-d", "--dataset", type=click.STRING, help="模型关联数据集名称", default=None)
def upload(name, model_type, model_source, dataset):
    """
    主题模型镜像上传命令

    \b
    使用示例：csp topic upload -n "镜像资源库:镜像tag" -t "镜像类型" -s "local" -d "镜像来源"
    """
    try:
        if model_type == "general" and dataset:
            raise ValueError("当 -t/--model_type 为 general 时，-d/--dataset 应不传为空")
        if model_type == "bussiness" and not dataset:
            raise ValueError("当 -t/--model_type 为 bussiness 时，-d/--dataset 必传不能为空")
