...
def gen_readme():
  ...
  from GBQ table `{get_global_configuration()['gbq_resource_name']}.{get_global_configuration()['gbq_datset_name']}.{get_global_configuration()['gbq_table_name']}`
  as looker explore '{get_global_configuration()['looker_name']}'
  using LookMLHelper {VERSION}
  Date of generation: {date_time}

  Your next step after generation: search for comments with "£££" i.e. pound sign and add/fill in any missing pieces
  """
  return text
...
def get_dimensions_tables_link(field):
  ...
  elif field.unnested_dimension_name == "altmetrics_id":
    return f"""
    link: {{
      label: "Altmetric"
