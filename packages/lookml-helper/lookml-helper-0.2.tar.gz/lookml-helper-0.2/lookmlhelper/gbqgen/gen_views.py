...
def create_all_raw_array_dimensions(looker_metadata, verbose=False):
  ...
  if len(top_level_array_struct_fields)> 0:
...
def write_raw_array_view(output_path, looker_metadata):
    ...
    all_raw_array_dimensions = create_all_raw_array_dimensions(looker_metadata)

    if all_raw_array_dimensions != "":
...
view: arrays_{get_global_configuration()['looker_name']} {{
  ...
  sql_table_name: `{get_global_configuration()['gbq_resource_name']}.{get_global_configuration()['gbq_datset_name']}.{get_global_configuration()['gbq_table_name']}`;;

{all_raw_array_dimensions}
...
def create_unnested_dimension(field):
  ...
  if data_type in ["INT64", "FLOAT64", "BIGNUMERIC", "NUMERIC"]: 
    ...
    looker_type = "yesno"
  elif (data_type in ["DATE", "TIMESTAMP", "DATETIME"]) or field.column_name.endswith("date") or field.column_name.startswith("date_") :
    looker_type = "date_time"
    ...
    looker_type = "string"
    sql = f"TO_JSON_STRING({sql}) # £££: GBQ TYPE 'JSON' NOT PROPERLY SUPPORTED. CODING SIMPLY CONVERTS JSON TO STRING"
  elif data_type == "BYTES":
    looker_type = "string"
    sql = f"SAFE_CONVERT_BYTES_TO_STRING({sql})  # £££: GBQ TYPE 'BYTES' NOT PROPERLY SUPPORTED. CODING SIMPLY TRIES TO CONVERT TO STRING AS BEST AS POSSIBLE"
  elif data_type == "GEOGRAPHY":
    looker_type = "location"
    standard_sql = False
    sql = f"""# £££: GBQ TYPE 'GEOGRAPHY' NOT PROPERLY SUPPORTED. CODING WORKS WELL ONLY IF THERE ARE ONLY NULL and 1-POINT VALUES
    sql_longitude: SAFE.ST_X({sql});;
    sql_latitude:  SAFE.ST_Y({sql});;"""
  else:
    printDebug("Unknown GBQ data type", data_type, " found for ", field.field_path, "!")
    looker_type = "string"
    sql = f"CAST({sql} TO STRING)  # £££: GBQ TYPE '{data_type}' NOT SUPPORTED. CODING ONLY TRIES TO CAST TO STRING WHICH MAY FAIL"

  if field.field_path == "id":
    group_label= "Identifiers"
    extras = f"""
    primary_key: yes"""
    label = " ID" # --- LEADING SPACE ---
  ...
  {dimension_type}: {dimension_name} {{
    ...
    {sql}{get_global_configuration()['link_func'](field)}
  }}
  """
  ...
  dimension: full_name {{
    type:         string
    label:        "Full Name"
    group_label:  "{group_label}"  #£££ EDIT IF NEEDED
...
def write_raw_fields_view(output_path, looker_metadata):
    all_raw_fields_dimensions = create_all_raw_fields_dimensions(looker_metadata)
    if all_raw_fields_dimensions:
      raw_fields_lml = f"""
##############
...
def write_unested_array_view(output_path, looker_metadata):
  ...
  all_array_unnested_views = create_all_array_unnested_views(looker_metadata)
  if all_array_unnested_views:
...
def create_all_base_field_measures(looker_metadata, verbose=False):
  fields = looker_metadata[looker_metadata.data_type.isin(['INT64', 'FLOAT64', "BOOL"])& # avg & sums are only possible for these types
          (~looker_metadata.field_path.str.contains("year")) & # averages and sums for attributes that are numerical but include years or ids don't make sense
          (looker_metadata.field_path != "id") & 
          (~looker_metadata.field_path.str.endswith("_id")) & 
...
def write_base_view(output_path, looker_metadata):
    all_base_field_measures = create_all_base_field_measures(looker_metadata)
    if all_base_field_measures:
      all_base_field_measures =f"""

  #######################
  #  MEASURES FOR NUMERIC FIELDS
  #
  {all_base_field_measures}
      """

    all_link_measures = create_all_link_measures(looker_metadata)
    if all_link_measures:
      all_link_measures = f"""
  ...
  {all_link_measures}
      """
      # If there are any arrays we need also include the array view
      include_array_view = f"arrays_{get_global_configuration()['looker_name']}, "
    else:
      include_array_view = ""
...
view: {get_global_configuration()['looker_name']}_base {{
  ...
  extends: [{include_array_view}fields_{get_global_configuration()['looker_name']}]
  sql_table_name: `{get_global_configuration()['gbq_resource_name']}.{get_global_configuration()['gbq_datset_name']}.{get_global_configuration()['gbq_table_name']}` ;;
  ...
  # £££: EDIT DEFAULT & DETAILS DRILL FIELDS 
  drill_fields: [id]
...
def write_unnested_view(output_path, looker_metadata):
    ...
    all_unnested_measures = create_all_unnested_measures(looker_metadata)
    if all_unnested_measures:
