...
def create_training_patterns(
    ...
    Args:
        ...
        use_gam (bool): If use GAM to create the models
    ...
    for subset_pattern_id, subset_pattern_df in patterns.groupby(
        ...
        for class_id_i, class_id in enumerate(class_ids):
            ...
            for j, band_name in enumerate(band_names):
                ...
                if use_gam:
                    # Create a GAM (Generalized Adversial Model)
                    # and use it on the data to find GAM model parameters
                    gam = LinearGAM(s(0))
                    gam.gridsearch(
                        x_days, y_values, progress=False)

                    # Plot values for the GAM
                    x_grid = gam.generate_X_grid(term=0, n=500)
                    axes[i, j].plot(x_grid, gam.predict(x_grid), 'r')
                else:
                ...
                # Predict band values for each of the prediction dates
                # with the GAM. Gives smoother results than using just
                # the values itself
                if use_gam:
                    all_patterns_per_class_per_band.append(
                        gam.predict(predict_dates))
                else:
...
def get_characteristic_timeseries(
    ...
    """ Extract timeseries from every roi and then use timeseries clustering
        ...
        Args:
            ...
            use_gam (bool): Use Generalized Additive Models for Values
                (smoothing them)
