...
from azureml.automl.dnn.vision.object_detection.eval.evaluator import OdEvaluator
from azureml.automl.dnn.vision.object_detection_yolo.eval.evaluator import YoloEvaluator
...
def test_single_image_onegt_onepred_exact_match(dataset_processing_type, yolo, eval_type):
    ...
    if yolo:
        evaluator = YoloEvaluator(training_settings_defaults, val_index_map=val_index_map,
                                  dataset_processing_type=dataset_processing_type,
                                  dataset_wrapper=dataset_wrapper)
    else:
        evaluator = OdEvaluator(training_settings_defaults, val_index_map=val_index_map,
                                dataset_processing_type=dataset_processing_type,
                                dataset_wrapper=dataset_wrapper)

    evaluator.initialize(enable_evaluation=True)
...
def test_multi_image_three_gt_three_pred_single_match_masks(dataset_processing_type, eval_type):
    meta_info_per_image = [
        ...
        {"width": 6400, "height": 6400,
         "original_width": 6400, "original_height": 6400,
         "filename": "image_2.jpg", "areas": [60000],
         "iscrowd": np.array([False, False, False])},
        {"width": 64000, "height": 64000,
         "original_width": 64000, "original_height": 64000,
    ...
    gt_objects_per_image = [
        ...
        # second image
        {
            ...
            "masks": [
                get_mask_from_bbox([10, 0, 20, 100], 6400, 6400),
                get_mask_from_bbox([20, 0, 30, 100], 6400, 6400),
                get_mask_from_bbox([30, 0, 40, 100], 6400, 6400),
            ],
            "labels": torch.tensor([0, 1, 2]),
            "scores": None
        },
        # third image
        {
            ...
            "masks": [
                get_mask_from_bbox([100, 0, 200, 100], 64000, 64000),
                get_mask_from_bbox([200, 0, 300, 100], 64000, 64000),
                get_mask_from_bbox([300, 0, 400, 100], 64000, 64000),
            ],
            "labels": torch.tensor([0, 1, 2]),
            "scores": None
        }
    ...
    predicted_objects_per_image = [
        ...
        # second image
        {
            ...
            "masks": get_mask_from_bbox([20, 0, 30, 100], 6400, 6400),
            "labels": torch.tensor([1]),
            "scores": torch.tensor([0.5]),
        ...
        # third image
        {
            ...
            "masks": get_mask_from_bbox([300, 0, 400, 100], 64000, 64000),
