...
def test_log_per_label_metrics():
    ds_mock = DatastoreMock('some_ds')
    ws_mock = WorkspaceMock(ds_mock)
    ...
    utils.log_per_label_metrics(metrics, mock_run, val_index_map=[0, 1, 2])
    run_metrics = mock_run.metrics

    assert run_metrics[MetricsLiterals.PER_LABEL_METRICS] == \
        metrics[MetricsLiterals.PER_LABEL_METRICS]

    assert run_metrics[MetricsLiterals.PER_LABEL_METRICS + '_train'] == \
        metrics[MetricsLiterals.PER_LABEL_METRICS + '_train']

    assert len(run_metrics.keys()) == 2
    assert all([key.startswith(MetricsLiterals.PER_LABEL_METRICS) for key in run_metrics])
