...
def log_all_metrics(computed_metrics: Dict[str, Any], azureml_run: Run, add_to_logger: bool = False,
    ...
    for metric_name, value in computed_metrics.items():
        ...
        # Need them to be logged as a table. moving their logging to log_per_label_metrics.
        if metric_name.startswith(MetricsLiterals.PER_LABEL_METRICS):
            continue
        if is_verbose:
...
def log_per_label_metrics(best_model_metrics: Dict[str, Any], azureml_run: Run,
                          val_index_map: List = []) -> None:
    """
    Logs per label metrics in a table to the run history of the given run
    ...
    :param azureml_run: The run object.
    :type azureml_run: azureml.core.run
    :param val_index_map: List of class names
    :type val_index_map: List
    """
    for metric_name in best_model_metrics:
        if not metric_name.startswith(MetricsLiterals.PER_LABEL_METRICS):
            continue
        for label_index, class_metrics in best_model_metrics[metric_name].items():
            azureml_run.log_row(metric_name,
                                class_name=val_index_map[label_index],
                                precision=class_metrics[MetricsLiterals.PRECISION],
                                recall=class_metrics[MetricsLiterals.RECALL],
