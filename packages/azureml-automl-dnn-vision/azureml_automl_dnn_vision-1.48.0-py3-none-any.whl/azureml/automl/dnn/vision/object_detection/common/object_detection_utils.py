...
def _update_with_voc_metrics(current_metrics, cumulative_per_label_metrics, voc_metrics, is_train=False):
    ...
    def _alter_name(name):
        return name + "{train}".format(train="_train" if is_train else "")
    ...
    current_metrics[_alter_name(MetricsLiterals.PRECISION)] = _round(voc_metrics[MetricsLiterals.PRECISION])
    current_metrics[_alter_name(MetricsLiterals.RECALL)] = _round(voc_metrics[MetricsLiterals.RECALL])
    current_metrics[_alter_name(MetricsLiterals.PER_LABEL_METRICS)] = _round_per_label_metrics(
