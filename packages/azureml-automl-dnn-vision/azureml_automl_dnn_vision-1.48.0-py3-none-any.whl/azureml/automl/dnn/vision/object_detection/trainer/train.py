...
def train_one_epoch(model, optimizer, scheduler, train_data_loader,
                    ...
                    grad_clip_type: str, evaluator: OdEvaluator):
...
def validate(
    model, val_data_loader, device, system_meter, distributed, evaluator: OdEvaluator
):
...
def train(model, optimizer, scheduler, train_data_loader, val_data_loader,
    ...
    if log_training_metrics and training_dataset_processing_type == DatasetProcessingType.IMAGES_AND_TILES:
        ...
        logger.warning("Training Metrics won't be computed when training\
                        dataset processing is set to images_and_tiles")
    ...
    # Initialize the evaluation helpers
    train_evaluator = OdEvaluator(settings=settings, val_index_map=val_index_map,
                                  dataset_processing_type=train_data_loader.dataset.dataset_processing_type,
                                  dataset_wrapper=train_data_loader.dataset)
    validation_evaluator = OdEvaluator(settings=settings, val_index_map=val_index_map,
                                       dataset_processing_type=train_data_loader.dataset.dataset_processing_type,
                                       dataset_wrapper=train_data_loader.dataset)
    ...
    for epoch in range(number_of_epochs):
        ...
        train_evaluator.initialize(enable_evaluation)
        avg_loss = \
        ...
        if train_evaluator.enabled:
            ...
            compute_metrics(train_evaluator.eval_bounding_boxes, train_evaluator.val_metric_type,
                            train_evaluator.val_coco_index, incremental_voc_evaluator,
        ...
        if epoch % eval_freq == 0 or final_epoch:
            ...
            if val_metric_type != ValidationMetricType.NONE:
                validation_evaluator.initialize(enable_evaluation=True)
                avg_loss = \
                ...
                map_score = compute_metrics(validation_evaluator.eval_bounding_boxes,
                                            ...
                                            validation_evaluator.val_coco_index,
        ...
        if master_process and azureml_run is not None:
            ...
            # log label wise metrics in table format.
            if (final_epoch or stop_early) and best_model_metrics:
                utils.log_per_label_metrics(best_model_metrics, azureml_run, val_index_map)
