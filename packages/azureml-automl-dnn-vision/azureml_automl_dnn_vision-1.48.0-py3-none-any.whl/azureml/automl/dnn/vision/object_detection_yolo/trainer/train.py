...
def validate(
    model, validation_loader, device, system_meter, evaluator: YoloEvaluator, print_freq=100,
    distributed=False,
):
...
def train(model_wrapper, optimizer, scheduler, train_loader, validation_loader,
    ...
    train_evaluator = YoloEvaluator(settings=settings, val_index_map=val_index_map,
                                    dataset_processing_type=train_loader.dataset.dataset_processing_type,
                                    dataset_wrapper=train_loader.dataset)
    validation_evaluator = YoloEvaluator(settings=settings, val_index_map=val_index_map,
    ...
    for epoch in range(number_of_epochs):
        ...
        train_evaluator.initialize(enable_evaluation=enable_evaluation)
