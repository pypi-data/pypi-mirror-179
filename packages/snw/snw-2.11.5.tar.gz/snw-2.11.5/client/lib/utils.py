...
def _lookup_repository(url, auth, service, remote_path, entity):
    ...
    for fileObj in result:
        if fileObj.get('alias_names') and isinstance(fileObj['alias_names'], list):
            lookup_cache[id].extend(fileObj['alias_names'])
        elif fileObj.get('key'):
            lookup_cache[id].append(fileObj['key'])
        ...
        if not status:
            ...
            for f in result:
                ...
                if f.endswith(CORPUS_SUFFIX):
                    ...
                    test_name = test_parts[len(test_parts) - 1]
                    res.append(
                        ("%s:%s.%s" % (storage_name, f, src_lang), "pn9_testtrans:" +
                         model + "/" + storage_name + "/" + real_path + "/" + test_name + "."+src_lang+'.' + tgt_lang))
                    count += 1
            if entity == "CONF_DEFAULT":
                entity = ""
