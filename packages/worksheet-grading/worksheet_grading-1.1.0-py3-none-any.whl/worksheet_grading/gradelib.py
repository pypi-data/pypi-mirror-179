...
class Exercise:
    ...
    def html_string(self, collapse=True):
        ...
        s = "<p>\n<b>Exercise {} (max {}P): {}P</b>".format(self.ex_num, self.max_points, self.act_points)
        if not collapse:
            s += "<ul>"
...
class Frame:
    ...
    def update_feedback(self):
        self.csv_df["feedback"] = [self.data[r["group"]].sheet.exercises[int(r["exercise"])-self.first_ex].csv_feedback() for _,r in self.csv_df.iterrows()]
        self.csv_df["points"] = [self.data[r["group"]].sheet.exercises[int(r["exercise"])-self.first_ex].csv_points() for _,r in self.csv_df.iterrows()]
