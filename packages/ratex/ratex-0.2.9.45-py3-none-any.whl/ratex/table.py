    ...
    def _headers(self) -> str:
        for i in range(len(self.headers)):
            if i != len(self.headers) - 1:
                self.headers[i] = f"\\begin{{basetext}}{self.headers[i]}\\end{{basetext}}&"
            else:
                self.headers[i] = f"\\begin{{basetext}}{self.headers[i]}\\end{{basetext}}\\\\\\hline"
        ...
        for i in range(len(self.data)):
            for j in range(len(self.data[i])):
                if j != len(self.data[i]) - 1:
                    self.data[i][j] = f"\\begin{{basetext}}{self.data[i][j]}\\end{{basetext}}&"
                else:
                    self.data[i][j] = f"\\begin{{basetext}}{self.data[i][j]}\\end{{basetext}}\\\\\\hline"
