...
def _process_push_row(session: Session, status: Status, push_context: PushContext, index: object, row_dict: dict):
    ...
    if 'Signal' in row_dict['Type']:
        ...
        condition_input.parameters = _process_formula_parameters(
            condition_input.parameters, push_context.workbook_context.workbook_id, push_context.push_results)
        row_dict['Formula Parameters'] = condition_input.parameters
        if len(condition_input.parameters) > 0:
            push_context.push_results.at[index, 'Formula Parameters'] = condition_input.parameters
        ...
        if _common.present(row_dict, 'ID'):
            ...
            condition_input.sync_token = push_context.sync_token
            setattr(condition_input, 'dataframe_index', index)
            status.df['Condition'] += _add_no_dupe(push_context.condition_batch_input.conditions, condition_input)
