...
def decode_amr(amr, output):
    import pilk
    amr_name = os.path.splitext(os.path.basename(amr))[0]
    wav_path = os.path.join(output, amr_name + '.wav')
    try:
        pilk.silk_to_wav(amr, wav_path)
    except:
        print(f'{amr} 转换失败！')
    return wav_path
...
def main2():
    """weamr 解码 微信 amr 语音文件"""
    try:
        import pilk
    except ImportError:
        print(f'weamr({__version__}) requires pilk>=0.2.2')
        return
    import argparse
    parser = argparse.ArgumentParser(
        description='解码 QQ/微信 语音文件 amr 到 WAV',
        epilog=f'weamr({__version__}) by foyou(https://github.com/foyoux/foyou-wedat)'
    )
    parser.add_argument('file', nargs='*', help='QQ/微信 语音文件 amr，可任意多个')
    parser.add_argument('-v', '--version', dest='version', help='show weamr version', action="store_true")
    parser.add_argument('-d', '--dir', dest='dir', type=str, help='folder of amr files')
    parser.add_argument('-o', '--out', dest='out', type=str, default='.', help='output folder')
    parser.add_argument('-i', '--ignore', dest='ignore', help='ignore suffix', action="store_true")
    args = parser.parse_args()

    if args.version:
        print('weamr version', __version__)
        return
    ...
    # 处理 files
    for file in files:
        x = decode_amr(file, output)
        print(x)

    f: os.DirEntry
    for f in os.scandir(folder):
        if args.ignore or f.name.endswith('.amr'):
            x = decode_amr(f.path, output)
            print(x)
