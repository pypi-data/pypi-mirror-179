...
class MyTUI:
    ...
    def __display_top(cls, title: str, spaces: int) -> None:
        ...
        if title:
            ...
            print(on_blue(" ") + black_on_gray(f'┌{HORIZONTAL_BAR*((bar_length//2))}┤  ') + red_on_gray(title) + black_on_gray(f'  ├{HORIZONTAL_BAR*((bar_length//2)-1+bonus)}┐') + on_blue("  "))
        else:
            print(on_blue(" ") + black_on_gray(f'┌{HORIZONTAL_BAR*(width()-5)}┐') + on_blue("  "))
    ...
    def __display_bottom(cls, spaces: int, extra_line: bool = True, yesno: bool = False, menu: bool = False, input: bool = False) -> None:
        if extra_line:
            print(f'{on_blue(" ") + black_on_gray(VERTICAL_BAR + " "*(width()-5) + VERTICAL_BAR) + on_black(" ") + on_blue(" ")}')
        print(on_blue(" ") + black_on_gray(f'└{HORIZONTAL_BAR*(width()-5)}┘') + on_black(" ") + on_blue(" "))
