...
class RedisUid:
    ...
    def __init__(
        ...
        :param prefix: 前缀(为空，则没有前缀拼接)
        :param seq_name: 序列名称，作为redis存储键(为空时默认取prefix，但两者不能同时为空)
        :param seq_beg: 序列开始，默认为0
        :param seq_len: 序列长度，默认为9(不足则0填充)
        :param date_fmt: 日期格式(为空，则没有日期拼接)
        """
    ...
    def gen_uid(self, seq_step: int = 1, ex: datetime = None):
        """
        生成唯一id
        ...
        if ex and not isinstance(ex, datetime):
            raise TypeError("'ex'只支持datetime类型")
        if self.redis_cli.ttl(self.seq_name) < 0:
            ex = self._set_ex(ex)
            self.redis_cli.set(self.seq_name, self.seq_beg)
            self.redis_cli.expireat(self.seq_name, ex)
        ...
        uid = "{prefix}{date_value}{seq_value}".format(
            prefix=_prefix,
            date_value=_date_value,
            seq_value=str(_seq_value).zfill(self.seq_len))
        return uid
