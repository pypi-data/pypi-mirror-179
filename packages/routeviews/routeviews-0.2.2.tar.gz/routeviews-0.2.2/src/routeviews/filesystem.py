...
def latest_file(path: str):
    ...
    files = glob.glob(f'{path}/*')
    if len(files) == 0:
        raise OSError(f'No files/directories found at "{path}"')
...
def writefile(*file_path: str, text: str) -> int:
    mkdirs_for_file(*file_path)
    return pathlib.Path(*file_path).write_text(text)
