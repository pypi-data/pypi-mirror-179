...
def _clear_packages(env: Type["Environment"]) -> None:
    from azureml.core.conda_dependencies import CondaDependencies
    pkg_set = set()
    pkg_set.update(env.python.conda_dependencies.conda_packages)

    for pkg in pkg_set:
        # python must not be removed from conda dependencies
        if pkg.startswith('python') and \
                CondaDependencies._get_package_name(pkg) == 'python':
            continue
        env.python.conda_dependencies.remove_conda_package(pkg)

    pkg_set.clear()
    pkg_set.update(env.python.conda_dependencies.pip_packages)
...
def _get_curated_environment_conda_list_packages() -> Tuple[List[str], List[str], Set[str]]:
    ...
    for pkg in output_json:
        ...
        if pkg_name not in added_set and pkg_name not in constants.Dependencies.PACKAGE_DELETION_LIST:
            ...
            if is_valid_conda_pkg or pkg_channel in ['conda-forge', 'anaconda', 'cerebis', 'pytorch']:
                conda_list.append(pkg_name + "=" + pkg_version)
            else:
                pip_list.append(pkg_name + "==" + pkg_version)

    return pip_list, conda_list, added_set
