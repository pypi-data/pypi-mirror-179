...
def _all_conda_and_pip_dependencies_list() -> Tuple[List[str], List[str], Set[str]]:
    """
    Retrieve the packages using 'conda list' command.
    ...
    for pkg in output_json:
        ...
        if pkg_name not in added_set:
            added_set.add(pkg_name)
            is_valid_conda_pkg = pkg_name in constants.Dependencies.CONDA_PREFERRED_LIST
            if is_valid_conda_pkg or pkg_channel in ['conda-forge', 'anaconda', 'cerebis']:
