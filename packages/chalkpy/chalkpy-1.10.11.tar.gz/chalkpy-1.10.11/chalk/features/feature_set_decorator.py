...
from chalk.streams._windows import WindowedMeta
        ...
        if isinstance(annotation, Windowed):
            ...
            if wind is None or not isinstance(wind, Windowed):
                raise TypeError(
                    ...
                    f"'{name}: Windowed[{str(annotation._kind)}] = windowed('10m', '30m')."
                )
            wind_with_buckets = cast(Windowed, wind)
            annotation._buckets = wind._buckets
            for bucket in wind_with_buckets._buckets:
                feat = wind_with_buckets._get_feature(bucket=bucket, default_name=name)
                feat._buckets = wind_with_buckets._buckets
                setattr(cls, feat.name, feat)
                cls_annotations[feat.name] = wind_with_buckets._kind
