...
def check_fortran(self, *k, **kw):
    self.check_cc(
        fragment=FC_FRAGMENT, compile_filename='test.f', features='fc fcprogram', msg='Compiling a simple fortran app'
    )
...
if os.name == 'nt':
    LINKFLAGS_IGNORED.extend([
        r'-lfrt*', r'-luser32', r'-lkernel32', r'-ladvapi32', r'-lmsvcrt', r'-lshell32', r'-lmingw', r'-lmoldname'
    ])
else:
...
def check_gfortran_o_space(self):
    ...
    try:
        self.check_fc(
            msg='Checking if the -o link must be split from arguments', fragment=FC_FRAGMENT, features='fc fcshlib'
        )
    except self.errors.ConfigurationError:
        self.env.revert()
    else:
