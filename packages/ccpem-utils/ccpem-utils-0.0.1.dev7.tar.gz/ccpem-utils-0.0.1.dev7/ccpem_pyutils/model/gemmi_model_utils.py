...
def shift_coordinates(
    in_model_path, out_model_path, trans_matrix, remove_charges=False, input_obj=False
):
    """
                ...
                for atom in residue:
                    ...
                    if remove_charges:  # remove negative charges?
                        if atom.charge != 0:
                            atom.charge = 0
    if not input_obj:
        if os.path.splitext(in_model_path)[-1].lower() in [".cif", ".mmcif"]:
            if not os.path.splitext(out_model_path)[-1].lower() in [".cif", ".mmcif"]:
                out_model_path = os.path.splitext(out_model_path)[0] + ".cif"
            write_structure_as_mmcif(structure, out_model_path)
        else:
            write_structure_as_pdb(structure, out_model_path)


def get_bfactors(in_model_path=None, structure=None, out_json=None):
    """
    ...
    for model in structure:
        for chain in model:
            ...
            if not polymer:
                continue
            if chain.name not in dict_chain:
...
def calc_bfact_deviation(in_model_path=None, structure=None, out_json=None):
    if in_model_path is not None:
        structure = gemmi.read_structure(in_model_path)
    dict_deviation = {}
    for dist in [3.0, 5.0, 10.0]:
        ...
        for model in structure:
            for chain in model:
                ...
                if not polymer:
                    continue
                if chain.name not in dict_deviation[dist]:
                ...
                for residue in chain.get_polymer():
                    ...
                    for atom in residue:
                        if atom.name == "CA":
                            ...
                            marks = subcells.find_neighbors(
                                atom, min_dist=0.1, max_dist=dist
                            )
                            for mark in marks:
                            ...
                            try:
                                avg_neigh = sum(list_neigh_bfact) / len(
                                    list_neigh_bfact
                                )
                    ...
                    if len(list_neigh_bfact) > 0:
                        dict_deviation[dist][chain.name][residue_id] = abs(
                            ca_bfact - avg_neigh
                        )
            break  # ignore other models
    if out_json is not None:
...
def get_coordinates(
    in_model_path: Union[str, pathlib.Path] = None,
    gemmi_structure_instance: gemmi.Structure = None,
    ...
    ---------
        ...
        out_json: str, optional
            json file to save the output coordinate dictionary (if return_list is False)
    Returns
    -------
    ...
    list_coords = []
    polymertype = None
    structure = check_structure_inputs(in_model_path, gemmi_structure_instance)
        ...
        for chain in model:
            ...
            for residue in chain:
                ...
                if isinstance(atom_selection, list):  # specified atoms only
                    list_residue_coords = get_selected_atom_coordinates(
                ...
                if return_list:
                    ...
                    if len(list_residue_coords) == 1:
                        residue_coords = list_residue_coords[0]
                    else:
...
def check_structure_inputs(in_model_path, gemmi_structure_instance=None):
    """
    ...
    """
    if gemmi_structure_instance is not None:
        structure = gemmi_structure_instance
    elif in_model_path is not None and os.path.isfile(in_model_path):
        structure = gemmi.read_structure(in_model_path)
    else:
...
def get_selected_atom_coordinates(residue: gemmi.Residue, atom_selection: list):
    """
...
def get_representative_coordinate(residue: gemmi.Residue, polymertype: str = None):
    """
...
def get_backbone_coordinates(residue: gemmi.Residue, polymertype: str = None):
    """
...
def get_residue_centre(gemmi_residue_instance: gemmi.Residue):
    """
        ...
        for chain in model:
            ...
            for residue in chain:
                ...
                for atom in residue:
                    ...
                    count_atom += 1
    structure.write_pdb(
        os.path.splitext(os.path.basename(in_model_path))[0] + "_attr.pdb"
    )


def get_sequence_from_atom_records(input_structure):
    """
    Get polymer sequence from atom records
    """
    structure = check_structure_inputs(input_structure)
    dict_seq = {}
    structure.add_entity_types(overwrite=True)
    structure.assign_subchains()  # assigns subchains in each chain
    structure.ensure_entities()  # each subchain is linked to one of Entity objects
...
def get_sequence_resnum_from_atom_records(input_structure):
    """
