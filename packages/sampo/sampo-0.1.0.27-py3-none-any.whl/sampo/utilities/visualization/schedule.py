...
def schedule_gant_chart_fig(schedule_dataframe: pd.DataFrame,
    ...
    print('min_xrange', schedule_dataframe.loc[:, 'start'].min() - timedelta(days=2), schedule_dataframe.loc[:, 'start'].min())
    print('max_xrange', schedule_dataframe.loc[:, 'finish'].max() + timedelta(days=25), schedule_dataframe.loc[:, 'finish'].max())

    fig.update_layout(autosize=True, font_size=12, margin_r=100)
