...
def _get_entries_from_commit(
    repo: Repo, commit: Commit, file_path: str, regex: str
) -> list[Entry]:
    try:
        file_text = str(
    ...
    return Entry.parse(file_path, file_text, commit, regex)
            ...
            for commit1, commit2 in commit_tuples:
                ...
                for return_value in _get_changed_files_and_their_status(
                    ...
                    if status == "D":
                        continue
                    entries.extend(
                        _get_entries_from_commit(repo, commit1, file_path, regex)
                    )
