...
class WeakNetworkConfig(object):
    ...
    def _connect_android(self, serial):
        ...
        if str(ip).startswith("192.168.0"):
            self._device_ip_list.append(ip)
            return True
        else:
            return False
    ...
    def _connect_ios(self, device):
        ...
        while True:
            ...
            if self._device_ip:
                self._device_ip_list.append(self._device_ip)
    ...
    def set_net_config(self, config_json):
        ...
        """
        for device_ip in self._device_ip_list:
            url = "{host}{uri}".format(host=self._atc_host,
                                       uri=WeakNetworkConfig.ATC_SHAPE_URI.format(device_ip))
