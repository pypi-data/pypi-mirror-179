    ...
    for dither in dither_list:
        ...
        if orders is not None:
            ...
            if background_orders is not None:
                ...
                o_empty.scene = Scene(instrument.telescope.tel_name, input=build_empty_scene(), webapp=webapp)
...
def configure_coronagraphy(input, strategy, scene_configuration, warnings, webapp=False):
    ...
    scene = Scene(strategy.instrument.telescope.tel_name, input=scene_configuration, webapp=webapp)
...
def calculate_sn(input, webapp=False):
    ...
    if contrast:
        ...
        scene = Scene(instrument.telescope.tel_name, input=scene_configuration, webapp=webapp)
        warnings.update(scene.warnings)
...
def calculate_exposure_time(input, webapp=False, **kwargs):
    ...
    if strategy.method=="taspec":
        # The TASpec strategy uses one DetectorSignal and DetectorNoise, but 
        # multiple extraction boxes. We need to make copies of the DetectorSignals.
        my_detector_signal_list = [copy.deepcopy(my_detector_signal_list[0]) for x in range(strategy.num_segments)]
        my_detector_noise_list = [copy.deepcopy(my_detector_noise_list[0]) for x in range(strategy.num_segments)]
        my_detector_saturation_list = [copy.deepcopy(my_detector_saturation_list[0]) for x in range(strategy.num_segments)]

    # Use the strategy to get the extracted signal/noise products
    extracted_sn_list = strategy.extract(my_detector_signal_list, my_detector_noise_list)
    warnings.update(extracted_sn_list[0]['warnings'])

    # Detector Scattering.  If calculation_config specifies to apply it, do that. If not, use the
    # predefined data value.
    # 
    # This detector effect is unusual for two reasons: One, it's defined as a fraction of the rate, but must be added
    # to the rate_plus_bg; Two, it's only present in the noise.
    scatter = instrument.the_detector.scatter
    if calc_config.noise['scatter'] is not None:
        scatter = calc_config.noise['scatter']
    if scatter:
    ...
    for signal in range(len(my_detector_signal_list)):
        new_time = my_detector_signal_list[signal].the_detector.scale_exposure_time(extracted_sn_list[signal], input)
