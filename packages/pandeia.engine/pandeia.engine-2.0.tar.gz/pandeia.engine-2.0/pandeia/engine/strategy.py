...
from .constants import EXTRACTION_NSPLIT
    ...
    **kwargs: list of keyword/value pairs
    ...
    def __init__(self, instrument=None, webapp=False, config={}, **kwargs):
        ...
        **kwargs: keyword/value pairs
        ...
        # If units are not in the engine standard of arcsecs, convert them first.
        #  This conversion will then be undone after the extraction is complete.
        if "units" in self.api_parameters and self.units != "arcsec":
            self.convert_extraction_units(self.units, "arcsec")
    ...
    def extract(self, my_detector_signal_list, my_detector_noise_list):
        ...
        if "units" in self.api_parameters and self.units != "arcsec":
            # The user defined the extraction height in something other than arcseconds.
            #  We converted to arcseconds in __init__, so now convert back to the original units.
            self.convert_extraction_units("arcsec", self.units)
    ...
    def _error_sum(self, a_ij, product_subscripts, my_detector_signal, my_detector_noise):
        ...
        exposure_products = {
            ...
            'extracted_noise_source': np.array(var_flux_products),
            ...
            'masks_products': masks_products,  # ARR - FOR DEBUGGING ONLY
            'saturation_products': {
    ...
    def _make_rectangle(self, grid, width, height, xoff=0., yoff=0., raise_except=False, name="Region"):
        ...
        """
        region = grid.rectangular_mask(width=width, height=height, xoff=xoff, yoff=yoff)
    ...
    def _make_segment(self, grid, signal, segment, raise_except=False, name="Region"):
        ...
        wmin_ind = np.searchsorted(signal.wave_pix,wmin, side="right")
        wmax_ind = np.searchsorted(signal.wave_pix,wmax, side="left") -1
...
class SpecSegmentedPhot(SpecApPhot):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        segment_config = self.instrument.slit_config[slit]['range']['default'][disperser][cenwave]
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        if self.scan_extraction_length == 'pixel':
            ...
            if self.instrument.dispersion_axis == 'y':
                # The cross-dispersion axis is x, which is the first value in the tuple.
...
class TASpec(SpecSegmentedPhot):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def __init__(self, instrument, webapp=False, config={}, **kwargs):
        ...
        self.order=instrument.disperser_config[self.disperser_key].get("orders", [None])[0]
        self.segment_config = self.instrument.slit_config[slit]["range"]["default"][self.disperser_key][cenwave]
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        if self.num_segments == 1:
            # The only ways this is 1 is when a COS FUV SpecAcq calculation is done with G140L, or when it's COS
            # NUV SpecAcq and the acq_mode is not acq_peakxd. In all cases, we need to combine any segments we have
            # into one big mask.
            # Combine the various segments by selecting the max value from any given pixel in the list of masks.
            # Could add them, but in the event that segments overlap, that makes less sense.
    ...
    def _make_segment(self, grid, signal, segment, raise_except=False, name="Region"):
        ...
        if self.mode == "fuv_specacq" and "acq_windows" in list(segment.keys()):
            ...
            # acq windows span shorter wavelength ranges than an entire segment.
            # Thus we can zero out the region delivered by the superclass and
            # just add the window masks to it.
            ...
            for window in acq_windows:
                ...
                # get the indices of those bounds
                wmin__window_ind = np.searchsorted(signal.wave_pix, wmin_window, side="right")
                wmax_window_ind = np.searchsorted(signal.wave_pix, wmax_window, side="left") - 1

                # # grid.rectangular_mask requires the location to be described as a width and offset from perfectly centered
                width_ind = (wmax_window_ind - wmin__window_ind)
                x_center_ind = ((width_ind / 2.0) + wmin__window_ind)
                width = (width_ind - 1) * grid.xsamp
                xoff = (x_center_ind * grid.xsamp) - grid.row.max()

                # make window, add it to segment region, and update total size
                window_mask = grid.rectangular_mask(width=width, height=height, xoff=xoff, yoff=yoff)
                region += window_mask
                window_size = np.sum(window_mask)
                reg_size += window_size
    ...
    def _add_exposure_products(self, exposure_products_list):
        ...
        return exposure_products_list[0]
