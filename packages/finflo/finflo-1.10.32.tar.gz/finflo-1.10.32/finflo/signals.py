...
def creates_finflo_work_models(sender, instance, created , **kwargs):
    ...
    if created:
        ...
        gets_transition = FinFlotransition(t_id = obj.id , type =   a.capitalize() )
        obj2 = workflowitems.objects.create(transitionmanager = obj ,model_type = a.capitalize() , initial_state = states.description ,interim_state = states.description, final_state = states.description)
        workevents.objects.create(workflowitems = obj2 , type = a.capitalize() , record_datas = gets_transition.get_record_datas(), initial_state = states.description ,interim_state = states.description,  final_state = states.description)
    pass
