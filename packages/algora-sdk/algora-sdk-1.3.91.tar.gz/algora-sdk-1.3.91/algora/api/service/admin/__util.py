...
def _get_users_request_info(limit: int) -> dict:
    return {
        "endpoint": "users",
        "params": {"max": limit},
        "url_key": "keycloak_admin"
...
def _create_offline_token_request_info(user_id: str, refresh_token: str, exp: Optional[int] = None) -> dict:
    return {
...
def _update_offline_token_request_info(user_id: str, refresh_token: str, exp: Optional[int] = None) -> dict:
    return {
        "endpoint": f"config/offline_token/{user_id}",
        "json": {
            "user_id": user_id,
